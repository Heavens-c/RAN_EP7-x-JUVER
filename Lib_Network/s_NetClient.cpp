...
CNetClient::CNetClient() 
	...
	, bRandomStringRcv(FALSE) //userpanel
	, m_pRcvManager(NULL)
{
	...
	::memset(m_szRandomString, 0, sizeof(char) * (USR_CAPTCHA+1));//userpanel
...
int CNetClient::GetClientVer()
{
	...
	FILE* sfp = ::fopen ( str.GetString(), "rb" );
	if ( !sfp )
	{
		return NET_ERROR;
	}
...
int CNetClient::GetGarbageMsg()
{
	...
	while(1)
	{
		...
		strcpy( szTempChar, GARBAGE_DATA[garbageNum] );
		...
		{
			strcpy( m_szSendGarbageMsg[m_dwGarbageNum], szTempChar );
