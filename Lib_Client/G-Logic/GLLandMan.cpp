...
BOOL GLLandMan::DropGenItem ( D3DXVECTOR3 &vPos, const SNATIVEID &sNID, EMITEMGEN emGENTYPE, EMGROUP emGroup, DWORD dwHoldGID )
{
	...
	DropItem ( vPos, &sITEM_NEW, emGroup, dwHoldGID, TRUE );

	return TRUE;
}

BOOL GLLandMan::DropItem ( D3DXVECTOR3 &vPos, SITEMCUSTOM *pItemCustom, EMGROUP emGroup, DWORD dwHoldGID, BOOL bMobGen )
{
	//	Note : 떨어질 위치 노드를 찾는다.
	//
...
DWORD GLLandMan::DropCrow ( GLMobSchedule *pMobSch )
{
	...
	if ( pNewCrow->m_pCrowData->m_sAction.m_dwActFlag&EMCROWACT_KNOCK )
	{
		...
		if ( szMAP_NAME )
		{
			...
			GLGaeaServer::GetInstance().SENDTOAGENT ( &NetMsg );*/
			GLMSG::SNET_EMCROWACT_KNOCK NetMsgCrowKnock;
			NetMsgCrowKnock.nidBossID = sNativeID;
			NetMsgCrowKnock.nidMapID = m_sMapID;
			//StringCchCopy ( NetMsgCrowKnock.szMapName, CHAR_SZNAME, szMAP_NAME );	
			//StringCchCopy ( NetMsgCrowKnock.szBossName, CHAR_SZNAME, pNewCrow->m_pCrowData->GetName() );	
			GLGaeaServer::GetInstance().SENDTOALLCLIENT ( &NetMsgCrowKnock );
		}
	}

	/*crow notice, Juver, 2017/11/25 */
	if ( pNewCrow->m_pCrowData->m_dwNotice& EMCROW_NOTICE_RESPAWN ||
		pNewCrow->m_pCrowData->m_dwNotice&EMCROW_NOTICE_RESPAWN2 )
	{
		const char *szMAP_NAME = GLGaeaServer::GetInstance().GetMapName ( m_sMapID );
		if ( szMAP_NAME )
		{
			CString strKey;
			CString strTEXT;
			strKey.Format ( "CROW_RESPAWN_%03d_%03d", pNewCrow->m_pCrowData->sNativeID.wMainID, pNewCrow->m_pCrowData->sNativeID.wSubID );
			strTEXT.Format ( ID2SERVERTEXT(strKey.GetString()), pNewCrow->m_pCrowData->GetName(), szMAP_NAME );

			GLMSG::SNET_SERVER_GENERALCHAT NetMsg;
			NetMsg.SETTEXT ( strTEXT.GetString() );
			//NetMsg.bUseAdminDisplay = BOOL( pNewCrow->m_pCrowData->m_dwNotice&EMCROW_NOTICE_RESPAWN2 );
...
DWORD GLLandMan::DropCrow ( GLMobSchedule *pMobSch, SNATIVEID sNID, FLOAT fPosX, FLOAT fPosY, FLOAT fScale )
{
	...
	//	Note : 떨어질 위치 노드를 찾는다.
	//
	LANDQUADNODE* pLandNode = m_LandTree.FindNode ( vPos.x, vPos.z );
	if ( !pLandNode )	return	UINT_MAX;
	...
	if ( pNewCrow->m_pCrowData->m_sAction.m_dwActFlag&EMCROWACT_KNOCK )
	{
		...
		if ( szMAP_NAME )
		{
			...
			GLGaeaServer::GetInstance().SENDTOAGENT ( &NetMsg );*/
			GLMSG::SNET_EMCROWACT_KNOCK NetMsgCrowKnock;
			NetMsgCrowKnock.nidBossID = sNID;
			NetMsgCrowKnock.nidMapID = m_sMapID;
			//StringCchCopy ( NetMsgCrowKnock.szMapName, CHAR_SZNAME, szMAP_NAME );	
			//StringCchCopy ( NetMsgCrowKnock.szBossName, CHAR_SZNAME, pNewCrow->m_pCrowData->GetName() );	
			GLGaeaServer::GetInstance().SENDTOALLCLIENT ( &NetMsgCrowKnock );
...
BOOL GLLandMan::MoveOutInstantMap( PGLCHAR pChar )
{
	if ( !pChar )
	{
		CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "!pChar" );
		return FALSE;
	}
	...
	if( !pOutGateMan )
	{
		CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pOutGateMan", pChar->m_dwCharID, pChar->m_szName );
		return FALSE;
	}
	...
	if( !pOutGate )
	{
		...
		if ( !pMapNode )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pOutGate !pMapNode %d %d", pChar->m_dwCharID, pChar->m_szName, sMoveMapId.wMainID, sMoveMapId.wSubID );
			return FALSE;
		}
	}else{
		GLLandMan* pInLandMan = NULL;
		...
		if ( !pInLandMan )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pInLandMan %d %d", pChar->m_dwCharID, pChar->m_szName, pOutGate->GetToMapID().wMainID, pOutGate->GetToMapID().wSubID );
			return FALSE;
		}
		...
		if ( !pInGateMan )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pInGateMan", pChar->m_dwCharID, pChar->m_szName );
			return FALSE;
		}
		...
		if ( !pInGate )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pInGate %d %d %d", pChar->m_dwCharID, pChar->m_szName, pOutGate->GetToMapID().wMainID, pOutGate->GetToMapID().wSubID, pOutGate->GetToGateID() );
			return FALSE;
		}
		...
		if ( !pMapNode )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pInGate %d %d", pChar->m_dwCharID, pChar->m_szName, sMoveMapId.wMainID, sMoveMapId.wSubID );
			return FALSE;
		}
	...
	for ( int i=0; i<SKILL_SUMMON_MAX_CLIENT_NUM; ++i )
		glGaeaServer.DropOutSummon ( pChar->m_dwSummonGUID_FLD[i] );
	...
	if ( pMapNode->dwFieldSID!=glGaeaServer.GetFieldSvrID() )
	{
		...
		if( !bOK )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s Instant Map Must Leave Failed %d %d", pChar->m_dwCharID, pChar->m_szName, m_sMapID.wMainID, m_sMapID.wSubID );
			return FALSE;
		}
...
void GLLandMan::FrameMoveInstantMap( float fElapsedTime )
{
	...
	if( IsPartyInstantMap() )
	{
		...
		for ( ; pCharNode; pCharNode = pCharNode->pNext )
		{
			...
			if( pChar->m_bInstanceDropOut ) 
			{
				pChar->m_fInstanceDropTimer += fElapsedTime;
				if ( pChar->m_fInstanceDropTimer >= (float) INSTANT_MAP_CHAR_DROP_TIME )
				{
					pChar->m_bInstanceDropOut = FALSE;
					pChar->m_fInstanceDropTimer = 0.0f;
				}
			}
			else
			{
				...
				{
					BOOL bMove = MoveOutInstantMap( pChar );	
					if ( !bMove )
					{
						CDebugSet::ToFileWithTime( "instancemap.txt", "GLLandMan::FrameMoveInstantMap IsPartyInstantMap() failed to moveout character[%d %s] [%d~%d]", 
							pChar->m_dwCharID, pChar->m_szName, m_sMapID.wMainID, m_sMapID.wSubID );
					}

					pChar->m_bInstanceDropOut = TRUE;
					pChar->m_fInstanceDropTimer = 0.0f;
				}
			}
		...
		for ( ; pCharNode; pCharNode = pCharNode->pNext )
		{
			...
			{		
				if( pChar->m_dwGaeaID != GetInstantMapHostID() )
				{
					...
					if ( !bMove )
					{
						CDebugSet::ToFileWithTime( "instancemap.txt", "GLLandMan::FrameMoveInstantMap failed to moveout character[%d %s] [%d~%d]", 
							pChar->m_dwCharID, pChar->m_szName, m_sMapID.wMainID, m_sMapID.wSubID );
					}
	...
	if( pLevelEtcFunc )
	{
		if( pLevelEtcFunc->m_bUseFunction[EMETCFUNC_LIMITTIME] && m_nLastSendRemainTime != -1.0f )
		{
			...
			if( nLimitMinute >= pLevelEtcFunc->m_sLimitTime.nLimitMinute )
			{
				...
				m_nLastSendRemainTime = -1.0f;
			}else 
			if( nLimitMinute % pLevelEtcFunc->m_sLimitTime.nTellTermMinute == 0 && m_nLastSendRemainTime != nLimitMinute )
			{
...
//				glGaeaServer.DropOutSummon ( pSummonfield->m_dwGUID,true );
				glGaeaServer.ReserveDropOutSummon ( pSummonfield->m_dwGUID );
			}
		}else{
			glGaeaServer.ReserveDropOutSummon ( pSummonfield->m_dwGUID );
		}
...
void GLLandMan::DoSkillAct ( const SSKILLACT &sACT )
{
	...
	if( sACT.IsATTACK()  )
	{
		...
		else if ( sACT.sID.emCrow==CROW_SUMMON && pREACTOR->GetCrow()==CROW_PC )
		{
			bREACT = false;
	...
	{
		PGLCHAR _pCHAR = GLGaeaServer::GetInstance().GetChar( sACT.sID.dwID );
		if ( _pCHAR )
		{
			if ( _pCHAR->m_bCurse && _pCHAR->m_fCurseDamage != 0.0f )
				_nDamage = int(nVAR_HP*_pCHAR->m_fCurseDamage); 
		}
	}
	else if ( pACTOR->GetCrow() == CROW_MOB )
	{
		PGLCROW _pCROW = GetCrow( sACT.sID.dwID );
		if ( _pCROW )
		{
			if ( _pCROW->m_bCurse && _pCROW->m_fCurseDamage != 0.0f )
				_nDamage = int(nVAR_HP*_pCROW->m_fCurseDamage); 
		}
	}
	else if ( pACTOR->GetCrow() == CROW_SUMMON )
	{
		PGLSUMMONFIELD _pSUMMON = GLGaeaServer::GetInstance().GetSummon( sACT.sID.dwID );
		if ( _pSUMMON )
		{
			if ( _pSUMMON->m_bCurse && _pSUMMON->m_fCurseDamage != 0.0f )
				_nDamage = int(nVAR_HP*_pSUMMON->m_fCurseDamage); 
		}
	}

	if ( _nDamage != 0 && sACT.sID_TAR != sACT.sID )
	{
		SSKILLACT sSKILLACTCURSE;
		sSKILLACTCURSE.sID = sACT.sID_TAR;
		sSKILLACTCURSE.sID_TAR = sACT.sID;
		sSKILLACTCURSE.fDELAY = 0.4f;
		sSKILLACTCURSE.emAPPLY = SKILL::EMAPPLY_PHY_LONG;
		sSKILLACTCURSE.nVAR_HP = _nDamage;
		sSKILLACTCURSE.dwDamageFlag = DAMAGE_TYPE_NONE;		

		if ( sSKILLACTCURSE.VALID() )
		{
			RegSkillAct ( sSKILLACTCURSE );
		}
	}
...
void GLLandMan::DoSkillActEx ( const SSKILLACTEX &sACTEX )
{
	...
	{
		DWORD dwSELSLOT(0);
		pREACTOR->RECEIVE_SKILLFACT ( sACTEX.idSKILL, sACTEX.wSKILL_LVL, dwSELSLOT, (WORD)sACTEX.sID.emCrow, sACTEX.sID.dwID );
		...
		if ( dwSELSLOT != SKILLFACT_SIZE )
		{
			...
			NetMsgBRD.wCasterCrow = (WORD)sACTEX.sID.emCrow;
			NetMsgBRD.dwCasterID = sACTEX.sID.dwID;
			...
			if ( sACTEX.sID_TAR.emCrow==CROW_PC )
			{
				...
				if ( pSkill && !( pChar->m_bVehicle && pSkill->m_sBASIC.emIMPACT_SIDE != SIDE_ENEMY) )
				{
					GLGaeaServer::GetInstance().SENDTOCLIENT ( pREACTOR->GETCLIENTID (), &NetMsgBRD );
					pREACTOR->SNDMSGAROUND ( (NET_MSG_GENERIC*) &NetMsgBRD );
				}
	...
	else if ( sACTEX.bPush )
	{
		/*push pull skill logic, Juver, 2017/06/04 */
		PushAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPUSH_PULL );
	}
	else if ( sACTEX.bPull )
	{
		/*push pull skill logic, Juver, 2017/06/04 */
		PullAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPUSH_PULL );
	}
	else if ( sACTEX.bDash )
	{
		/*dash skill logic, Juver, 2017/06/17 */
		DashAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPUSH_PULL, sACTEX.wActionAnim );
	}
	// 밀어내기
	else
	{
		...
		if ( pNavi )
		{
			...
			if ( bCoA )
			{
				...
				if ( bCoB )
				{
					...
					{
						pREACTOR->ReceivePushPull ( vCoB, GLCONST_CHAR::fPUSHPULL_VELO, false, false, false, AN_SUB_00_SIZE ); /*push pull skill logic, Juver, 2017/06/04 */
					}
				}
			}
		}
...
/*push pull skill logic, Juver, 2017/06/04 */
void GLLandMan::PushAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fDISTANCE )
{
	if ( _fDISTANCE == 0.0f )	return;
	if ( !GetNavi() )	return;

	float fDistance = _fDISTANCE;

	GLACTOR* pACTOR = GLGaeaServer::GetInstance().GetTarget ( this, _dwActorID );
	if ( !pACTOR )	return;

	GLACTOR* pREACTOR = GLGaeaServer::GetInstance().GetTarget ( this, _dwTargetID );
	if ( !pREACTOR )	return;

	D3DXVECTOR3 vREACTOR_POS = pREACTOR->GetPosition();
	D3DXVECTOR3 vDir = ( vREACTOR_POS - pACTOR->GetPosition() );

	vDir.y = 0.0f;
	D3DXVec3Normalize (  &vDir, &vDir );

	D3DXVECTOR3 vPosA = vREACTOR_POS;
	D3DXVECTOR3 vPosB = vREACTOR_POS + vDir * fDistance;
	...
	GetNavi()->IsCollision ( D3DXVECTOR3(vPosA.x,vPosA.y+5.0f,vPosA.z), D3DXVECTOR3(vPosA.x,vPosA.y-5.0f,vPosA.z), vCoA, dwCoA, bCoA );
	if ( bCoA )
	{
		GetNavi()->IsCollision ( D3DXVECTOR3(vPosB.x,vPosB.y+5.0f,vPosB.z), D3DXVECTOR3(vPosB.x,vPosB.y-5.0f,vPosB.z), vCoB, dwCoB, bCoB );
		if ( !bCoB )
		{
			float fdxLength = 0.5f;
			if ( fDistance > 40.0f )		fdxLength = 10.0f;
			else if ( fDistance > 15.0f )	fdxLength = 5.0f;
			else if ( fDistance > 5.0f )	fdxLength = 2.0f;

			float fNewPushPull = fDistance - fdxLength;
			while ( 0.0f < fNewPushPull )
			{
				vPosB = vREACTOR_POS + vDir * fNewPushPull;
				GetNavi()->IsCollision ( D3DXVECTOR3(vPosB.x,vPosB.y+5.0f,vPosB.z), D3DXVECTOR3(vPosB.x,vPosB.y-5.0f,vPosB.z), vCoB, dwCoB, bCoB );
		...
		if ( bCoB )
		{
			if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
				pREACTOR->ReceivePushPull ( vCoB, GLCONST_CHAR::fPUSHPULL_VELO, true, false, false, AN_SUB_00_SIZE ); /*push pull skill logic, Juver, 2017/06/04 */
		}
	}
}

/*push pull skill logic, Juver, 2017/06/04 */
void GLLandMan::PullAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED )
{
	if ( _fSPEED == 0.0f )	return;
	if ( !GetNavi() )	return;
	...
	D3DXVECTOR3 vREACTOR_POS = pREACTOR->GetPosition();
	D3DXVECTOR3 vDir = ( pACTOR->GetPosition() - vREACTOR_POS );
	...
	if ( bCoA )
	{
		...
		{
			float fdxLength = 0.5f;
			if ( _fSPEED > 5.0f ) fdxLength = _fSPEED * 0.1f;
		...
		{
			if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
				pREACTOR->ReceivePushPull ( vCoB, _fSPEED, true, true, false, AN_SUB_00_SIZE );
		}
	}
}

/*dash skill logic, Juver, 2017/06/17 */
void GLLandMan::DashAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED, const WORD _wActionAnim )
{
	if ( _fSPEED == 0.0f )	return;
	if ( !GetNavi() )	return;

	GLACTOR* pACTOR = GLGaeaServer::GetInstance().GetTarget ( this, _dwTargetID );
	if ( !pACTOR )	return;

	GLACTOR* pREACTOR = GLGaeaServer::GetInstance().GetTarget ( this, _dwActorID );
	if ( !pREACTOR )	return;
	...
	if ( bCoA )
	{
		...
		{
			if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
				pREACTOR->ReceivePushPull ( vCoB, _fSPEED, false, true, true, _wActionAnim );
		}
	}
}
...
void GLLandMan::ResetTyrannyTower()
{
	GLCROWNODE *pCurCrow = m_GlobCROWList.m_pHead;
