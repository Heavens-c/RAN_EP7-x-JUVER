...
void CSubMasterSet::LoadJoin()
{
	...
	SetCheck_Flags( bJoin, m_dwClubFlag, EMCLUB_SUBMASTER_JOIN );
}
...
void CSubMasterSet::LoadStrike()
{
	...
	SetCheck_Flags( bStrike, m_dwClubFlag, EMCLUB_SUBMASTER_KICK );
}
...
void CSubMasterSet::LoadNotice()
{
	...
	SetCheck_Flags( bNotice, m_dwClubFlag, EMCLUB_SUBMASTER_NOTICE );
}
...
void CSubMasterSet::LoadMark()
{
	...
	SetCheck_Flags( bMark, m_dwClubFlag, EMCLUB_SUBMASTER_MARK );
}
...
void CSubMasterSet::LoadCertify()
{
	...
	SetCheck_Flags( bCertify, m_dwClubFlag, EMCLUB_SUBMASTER_CD );
}
...
void CSubMasterSet::LoadClubDeathMatch()
{
	...
	SetCheck_Flags( bCDM, m_dwClubFlag, EMCLUB_SUBMASTER_CDM );
}
	...
	case SUBMASTER_JOIN_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				{
					bool bJoin = GetCheck_Flags( m_dwClubFlag, EMCLUB_SUBMASTER_JOIN );
					SetCheck_Flags( !bJoin, m_dwClubFlag, EMCLUB_SUBMASTER_JOIN );
					m_pJoinButton->SetFlip( !bJoin );
				}
			}
	...
	case SUBMASTER_STRIKE_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( dwMsg & UIMSG_LB_UP )
				{
					bool bStrike = GetCheck_Flags( m_dwClubFlag, EMCLUB_SUBMASTER_KICK );
					SetCheck_Flags( !bStrike, m_dwClubFlag, EMCLUB_SUBMASTER_KICK );
					m_pStrikeButton->SetFlip( !bStrike );
				}
			}
	...
	case SUBMASTER_NOTICE_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( dwMsg & UIMSG_LB_UP )
				{
					bool bNotice = GetCheck_Flags( m_dwClubFlag, EMCLUB_SUBMASTER_NOTICE );
					SetCheck_Flags( !bNotice, m_dwClubFlag, EMCLUB_SUBMASTER_NOTICE );
					m_pNoticeButton->SetFlip( !bNotice );
				}
			}
	...
	case SUBMASTER_MARK_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( dwMsg & UIMSG_LB_UP )
				{
					bool bMark = GetCheck_Flags( m_dwClubFlag, EMCLUB_SUBMASTER_MARK );
					SetCheck_Flags( !bMark, m_dwClubFlag, EMCLUB_SUBMASTER_MARK );
					m_pMarkButton->SetFlip( !bMark );
				}
			}
	...
	case SUBMASTER_CERTIFY_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( dwMsg & UIMSG_LB_UP )
				{	
					bool bCertify = GetCheck_Flags( m_dwClubFlag, EMCLUB_SUBMASTER_CD );
					SetCheck_Flags( !bCertify, m_dwClubFlag, EMCLUB_SUBMASTER_CD );
					m_pCertifyButton->SetFlip( !bCertify );
				}
			}
	...
	case SUBMASTER_CDM_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( dwMsg & UIMSG_LB_UP )
				{
					bool bCDM = GetCheck_Flags( m_dwClubFlag, EMCLUB_SUBMASTER_CDM );
					SetCheck_Flags( !bCDM, m_dwClubFlag, EMCLUB_SUBMASTER_CDM );
					m_pCDMButton->SetFlip( !bCDM );
				}
			}
