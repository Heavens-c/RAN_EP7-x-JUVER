...
void CDlgUserEdit::DataDefault()
{
	...
	SetWin_Text( this, IDC_USEREDIT_EDIT_SECKEY, "" );
...
void CDlgUserEdit::DataShow()
{
	...
	SetWin_Text( this, IDC_USEREDIT_EDIT_SECKEY, m_pData->szSecurityKey );
...
BOOL CDlgUserEdit::DataSave()
{
	...
	std::string strSecKey = GetWin_Text( this, IDC_USEREDIT_EDIT_SECKEY );
	...
	if ( strSecKey.size() < MIN_USER_PASS || strSecKey.size() > MAX_USER_PASS ){
		CDebugSet::MsgBox( GetSafeHwnd(), "Security Key can only have %d to %d characters", MIN_USER_PASS, MAX_USER_PASS );
		GotoDlgCtrl(GetDlgItem(IDC_USEREDIT_EDIT_SECKEY)); 
		return FALSE;
	}
	...
	if ( GMTOOL_GLOBAL::IsBadString( strSecKey.c_str() ) ){
		CDebugSet::MsgBox( GetSafeHwnd(), "Security Key contains invalid characters!" );
		GotoDlgCtrl(GetDlgItem(IDC_USEREDIT_EDIT_SECKEY)); 
		return FALSE;
	}
	...
	if ( nUserPoints < 0 ){
		CDebugSet::MsgBox( GetSafeHwnd(), "E Points invalid" );
		GotoDlgCtrl(GetDlgItem(IDC_USEREDIT_EDIT_USERPOINT)); 
		return FALSE;
	}

	if ( nUserPoints2 < 0 ){
		CDebugSet::MsgBox( GetSafeHwnd(), "GT Points invalid" );
	...
	StringCchCopy( m_pData->szSecurityKey, USR_PASS_LENGTH, strSecKey.c_str() );
