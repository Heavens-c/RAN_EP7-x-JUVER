...
//userpanel
void CClientManager::SetRandomString(DWORD dwClient, const TCHAR* szRandString )
{
	m_pClient[dwClient].SetRandomString(szRandString);
}

TCHAR* CClientManager::GetRandomString(DWORD dwClient)
{
	return m_pClient[dwClient].GetRandomString();
}
...
//userpanel
void CClientManager::SetBattlePoint( DWORD dwClient, USHORT uBattlePoint )
{
	m_pClient[dwClient].SetBattlePoint( uBattlePoint );
}

void CClientManager::SetUserPoint( DWORD dwClient, USHORT uUserPoint )
{
	m_pClient[dwClient].SetUserPoint( uUserPoint );
}

void CClientManager::SetUserEmail( DWORD dwClient, const TCHAR* strEmail )
{
	m_pClient[dwClient].SetUserEmail( strEmail );
}

int	CClientManager::GetUserPoint (DWORD dwClient )
{
	return m_pClient[dwClient].GetUserPoint();
}

int	CClientManager::GetBattlePoint (DWORD dwClient )
{
	return m_pClient[dwClient].GetBattlePoint();
}

TCHAR*	CClientManager::GetUserEmail (DWORD dwClient )
{
	return m_pClient[dwClient].GetUserEmail();
}
//userpanel end
// 중국 GameTime을 가지고 온다.
...
void CClientManager::IPSetClientMax( WORD _wMAX )
{
	...
	if ( m_wClientIPMax == 0 ){
		CConsoleMessage::GetInstance()->Write( _T( "IPLimit set to no limit"), m_wClientIPMax );
	}
	else{
...
BOOL CClientManager::IPAllowConnect( SOCKET _s )
{
	...
	::getpeername(_s, (sockaddr *) &sAddrIn, &nSize);
...
BOOL CClientManager::IPAllowConnect( std::string _strIP )
{
	...
	if ( IPExcluded( _strIP ) ){
		LockOff();
		return TRUE;
	}


	MAP_CLIENTIP_ITER iterip = m_mapClientIP.find( _strIP );
	if ( iterip != m_mapClientIP.end() ){
		SET_CLIENTID setClientID = (*iterip).second;
		if ( setClientID.size() >= m_wClientIPMax ){
...
void CClientManager::IPAddClient( std::string _strIP, DWORD _dwClientID )
{
	...
	if ( iterip != m_mapClientIP.end() ){
		SET_CLIENTID &setClientID = (*iterip).second;
		setClientID.insert( _dwClientID );
	}
	else{
		SET_CLIENTID setClientID;
		setClientID.insert( _dwClientID );
		m_mapClientIP.insert( std::make_pair( _strIP, setClientID ) );
	}
...
void CClientManager::IPRemove( std::string _strIP )
{
	...
	if ( iterip != m_mapClientIP.end() ){
		...
		CConsoleMessage::GetInstance()->Write(_T("IPLimit Remove IP:%s"), _strIP.c_str());
	}
...
DWORD CClientManager::IPGetTotalClient()
{
	...
	DWORD dwTOTALSIZE = 0;
	for( MAP_CLIENTIP_ITER iterip = m_mapClientIP.begin();
		iterip != m_mapClientIP.end();
		++ iterip ){

		SET_CLIENTID setClientID = (*iterip).second;
		dwTOTALSIZE += DWORD( setClientID.size());
	}
