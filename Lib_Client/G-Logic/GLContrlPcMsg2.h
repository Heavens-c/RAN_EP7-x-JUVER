...
namespace GLMSG
{
	...
	struct SNETPC_INVEN_INVENTORY_LOCK_ENABLE
	{
		NET_MSG_GENERIC		nmg;
		char				szPin1[CHAR_INVENTORY_LOCK_PASS_SIZE+1];
		char				szPin2[CHAR_INVENTORY_LOCK_PASS_SIZE+1];

		SNETPC_INVEN_INVENTORY_LOCK_ENABLE ()
		{
			memset (szPin1, 0, sizeof(char) * CHAR_INVENTORY_LOCK_PASS_SIZE+1);
			memset (szPin2, 0, sizeof(char) * CHAR_INVENTORY_LOCK_PASS_SIZE+1);

			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_ENABLE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*inventory lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_INVENTORY_LOCK_ENABLE_FB
	{
		NET_MSG_GENERIC		nmg;
		EMFB_INVENTORY_LOCK_ENABLE emFB;
		BOOL		bInventoryLockEnable;

		SNETPC_INVEN_INVENTORY_LOCK_ENABLE_FB ()
			: emFB(EMFB_INVENTORY_LOCK_ENABLE_FAILED)
			, bInventoryLockEnable(FALSE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_ENABLE_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*inventory lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_INVENTORY_LOCK_ENABLE_FROM_DB
	{
		NET_MSG_GENERIC				nmg;
		EMFB_INVENTORY_LOCK_ENABLE	emFB;
		char						szName[CHAR_SZNAME+1];
		char						szPin[CHAR_INVENTORY_LOCK_PASS_SIZE+1];
		
		enum { EMMSG_SIZE = sizeof(NET_MSG_GENERIC)+sizeof(DWORD)+CHAR_SZNAME+1+CHAR_INVENTORY_LOCK_PASS_SIZE+1};
		char				m_cBUFFER[NET_DATA_BUFSIZE-EMMSG_SIZE];

		SNETPC_INVEN_INVENTORY_LOCK_ENABLE_FROM_DB () :
			emFB(EMFB_INVENTORY_LOCK_ENABLE_FAILED)
		{
			nmg.dwSize = sizeof(*this);
			memset ( szName, 0, sizeof(char)*CHAR_SZNAME+1 );
			memset ( szPin, 0, sizeof(char)*CHAR_INVENTORY_LOCK_PASS_SIZE+1 );
			nmg.nType = NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_ENABLE_FROM_DB;
		}
	};

	/*inventory lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_INVENTORY_LOCK_INPUT
	{
		NET_MSG_GENERIC		nmg;
		char				szPin[CHAR_INVENTORY_LOCK_PASS_SIZE+1];

		SNETPC_INVEN_INVENTORY_LOCK_INPUT ()
		{
			memset (szPin, 0, sizeof(char) * CHAR_INVENTORY_LOCK_PASS_SIZE+1);

			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_INPUT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*inventory lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_INVENTORY_LOCK_INPUT_FB
	{
		NET_MSG_GENERIC				nmg;
		EMFB_INVENTORY_LOCK_INPUT	emFB;
		BOOL						bInventoryLockStatus;

		SNETPC_INVEN_INVENTORY_LOCK_INPUT_FB ()
			: emFB(EMFB_INVENTORY_LOCK_INPUT_FAILED)
			, bInventoryLockStatus(FALSE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_INPUT_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	
	/*inventory lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_INVENTORY_LOCK_INPUT_FROM_DB
	{
		NET_MSG_GENERIC				nmg;
		EMFB_INVENTORY_LOCK_INPUT	emFB;
		char						szName[CHAR_SZNAME+1];
		...
		SNETPC_INVEN_INVENTORY_LOCK_INPUT_FROM_DB () :
			emFB(EMFB_INVENTORY_LOCK_INPUT_FAILED)
		{
			nmg.dwSize = sizeof(*this);
			memset ( szName, 0, sizeof(char)*CHAR_SZNAME+1 );
			memset ( szPin, 0, sizeof(char)*CHAR_INVENTORY_LOCK_PASS_SIZE+1 );
			nmg.nType = NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_INPUT_FROM_DB;
		}
	};
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/*storage lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_STORAGE_LOCK_ENABLE
	{
		NET_MSG_GENERIC		nmg;
		char				szPin1[CHAR_STORAGE_LOCK_PASS_SIZE+1];
		char				szPin2[CHAR_STORAGE_LOCK_PASS_SIZE+1];

		SNETPC_INVEN_STORAGE_LOCK_ENABLE ()
		{
			memset (szPin1, 0, sizeof(char) * CHAR_STORAGE_LOCK_PASS_SIZE+1);
			memset (szPin2, 0, sizeof(char) * CHAR_STORAGE_LOCK_PASS_SIZE+1);

			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_STORAGE_LOCK_ENABLE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*storage lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_STORAGE_LOCK_ENABLE_FB
	{
		NET_MSG_GENERIC		nmg;
		EMFB_STORAGE_LOCK_ENABLE emFB;
		BOOL		bStorageLockEnable;

		SNETPC_INVEN_STORAGE_LOCK_ENABLE_FB ()
			: emFB(EMFB_STORAGE_LOCK_ENABLE_FAILED)
			, bStorageLockEnable(FALSE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_STORAGE_LOCK_ENABLE_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*storage lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_STORAGE_LOCK_ENABLE_FROM_DB
	{
		NET_MSG_GENERIC				nmg;
		EMFB_STORAGE_LOCK_ENABLE	emFB;
		char						szName[CHAR_SZNAME+1];
		char						szPin[CHAR_STORAGE_LOCK_PASS_SIZE+1];
		
		enum { EMMSG_SIZE = sizeof(NET_MSG_GENERIC)+sizeof(DWORD)+CHAR_SZNAME+1+CHAR_STORAGE_LOCK_PASS_SIZE+1};
		char				m_cBUFFER[NET_DATA_BUFSIZE-EMMSG_SIZE];

		SNETPC_INVEN_STORAGE_LOCK_ENABLE_FROM_DB () :
			emFB(EMFB_STORAGE_LOCK_ENABLE_FAILED)
		{
			nmg.dwSize = sizeof(*this);
			memset ( szName, 0, sizeof(char)*CHAR_SZNAME+1 );
			memset ( szPin, 0, sizeof(char)*CHAR_STORAGE_LOCK_PASS_SIZE+1 );
			nmg.nType = NET_MSG_GCTRL_INVEN_STORAGE_LOCK_ENABLE_FROM_DB;
		}
	};

	/*storage lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_STORAGE_LOCK_INPUT
	{
		NET_MSG_GENERIC		nmg;
		char				szPin[CHAR_STORAGE_LOCK_PASS_SIZE+1];

		SNETPC_INVEN_STORAGE_LOCK_INPUT ()
		{
			memset (szPin, 0, sizeof(char) * CHAR_STORAGE_LOCK_PASS_SIZE+1);

			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_STORAGE_LOCK_INPUT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*storage lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_STORAGE_LOCK_INPUT_FB
	{
		NET_MSG_GENERIC				nmg;
		EMFB_STORAGE_LOCK_INPUT	emFB;
		BOOL						bStorageLockStatus;

		SNETPC_INVEN_STORAGE_LOCK_INPUT_FB ()
			: emFB(EMFB_STORAGE_LOCK_INPUT_FAILED)
			, bStorageLockStatus(FALSE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_STORAGE_LOCK_INPUT_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	
	/*storage lock, DevArt22, 2020/02/11 */
	struct SNETPC_INVEN_STORAGE_LOCK_INPUT_FROM_DB
	{
		NET_MSG_GENERIC				nmg;
		EMFB_STORAGE_LOCK_INPUT	emFB;
		char						szName[CHAR_SZNAME+1];
		...
		SNETPC_INVEN_STORAGE_LOCK_INPUT_FROM_DB () :
			emFB(EMFB_STORAGE_LOCK_INPUT_FAILED)
		{
			nmg.dwSize = sizeof(*this);
			memset ( szName, 0, sizeof(char)*CHAR_SZNAME+1 );
			memset ( szPin, 0, sizeof(char)*CHAR_STORAGE_LOCK_PASS_SIZE+1 );
			nmg.nType = NET_MSG_GCTRL_INVEN_STORAGE_LOCK_INPUT_FROM_DB;
		}
	};
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
