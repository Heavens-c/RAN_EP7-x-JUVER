...
static void AddFileItem(HWND hwndFileList, PCTSTR pszFilePath)
{
	...
	if (hFindFile != INVALID_HANDLE_VALUE)
	{
		...
		if (dwFileSize >= 1000000000)
			_stprintf(szFileSize, _T("%lu.%lu GB"), dwFileSize / 1000000000, (dwFileSize % 1000000000) / 100000000);
		else if (dwFileSize >= 1000000)
			_stprintf(szFileSize, _T("%lu.%lu MB"), dwFileSize / 1000000, (dwFileSize % 1000000) / 100000);
		else if (dwFileSize >= 1000)
			_stprintf(szFileSize, _T("%lu.%lu KB"), dwFileSize / 1000, (dwFileSize % 1000) / 100);
		else
			_stprintf(szFileSize, _T("%lu B"), dwFileSize);
		FindClose(hFindFile);
	}
	else
	{
	...
	if (pFileItemInfo != NULL)
	{
		...
		_tcscpy(pFileItemInfo->szFileName, pszFilePath);
	}
