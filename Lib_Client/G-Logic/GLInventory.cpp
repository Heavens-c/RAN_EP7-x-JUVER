...
BOOL GLInventory::FindInsrtable ( const WORD wSX, const WORD wSY, WORD &wPosX, WORD &wPosY, bool bAllLine )
{
	...
	for ( int cx=0; cx<=nCellSX; cx++ )
	{
		...
		for ( WORD i=0; i<wSY; i++ )// Sid | 8-23-15 | Inventory Y to X Load
		for ( WORD j=0; j<wSX; j++ )
		{
...
DWORD GLInventory::CountItemTotal ()
{
	DWORD dwNum = 0;
...
BOOL GLInventory::SETITEM_BYBUFFER ( CByteStream &ByteStream )
{
	...
	for ( DWORD i=0; i<dwNum; i++ )
	{
		...
		else
		{
			...
			//
			CDebugSet::ErrorVersion( "GLInventory::SETITEM_BYBUFFER", dwVersion );
			return FALSE;
		}
	}
