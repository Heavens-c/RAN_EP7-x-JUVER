...
void CConsoleMessage::SetControl(HWND hControl, HWND hControlInfo, HWND hControlStats)
{
	m_pEditBox = hControl;
	m_pEditBoxInfo = hControlInfo;
	m_pEditBoxStats = hControlStats;
	...
	SendMessage( m_pEditBoxStats, EM_LIMITTEXT, (WPARAM) (32 * 1024), (LPARAM) 0 );	
}
...
void CConsoleMessage::WriteStats( const TCHAR* msg, ... )
{
	if (msg == NULL) return;
	...
	Write( LOG_CONSOLE, C_MSG_STATS, szBuf );
}

void CConsoleMessage::WriteStats( int nType, const TCHAR* msg, ... )
{
	if (msg == NULL) return;
...
void CConsoleMessage::UpdateEditControlStats( TCHAR* sbuf )
{
	if ( !sbuf ) return;
	if ( !m_pEditBoxStats ) return;
	...
	::SendMessageCallback( m_pEditBoxStats, EM_SETSEL,      (WPARAM) UINT_MAX-1,  (LPARAM) UINT_MAX, NULL, (ULONG_PTR) &dwRetCode );
	::SendMessageCallback( m_pEditBoxStats, EM_SCROLL,      (WPARAM) SB_PAGEDOWN, (LPARAM) 0,	    NULL, (ULONG_PTR) &dwRetCode );	
	::SendMessageCallback( m_pEditBoxStats, EM_SCROLLCARET, (WPARAM) 0,			 (LPARAM) 0,		NULL, (ULONG_PTR) &dwRetCode );	
	::SendMessageTimeout(  m_pEditBoxStats, EM_REPLACESEL,  (WPARAM) FALSE,		 (LPARAM) TempBuffer, SMTO_BLOCK, 15, &dwRetCode );
	::SendMessageCallback( m_pEditBoxStats, EM_LINESCROLL,  (WPARAM) 0,			 (LPARAM) 1,		NULL, (ULONG_PTR) &dwRetCode );
	::SendMessageCallback( m_pEditBoxStats, EM_SETSEL,	   (WPARAM) -1,		     (LPARAM) UINT_MAX, NULL, (ULONG_PTR) &dwRetCode );
}