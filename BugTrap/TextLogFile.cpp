...
BOOL CTextLogFile::LoadEntries(void)
{
	...
	TCHAR szMessage[128];
	_stprintf(szMessage, _T("CTextLogFile::LoadEntries(): %lu entries, %lu bytes, %lu milliseconds\r\n"), GetNumEntries(), GetNumBytes(), dwEndTime - dwStartTime);
...
BOOL CTextLogFile::SaveEntries(bool /*bCrash*/)
{
	...
	DWORD dwEndTime = GetTickCount();
	TCHAR szMessage[128];
	_stprintf(szMessage, _T("CTextLogFile::SaveEntries(): %lu entries, %lu bytes, %lu milliseconds\r\n"), GetNumEntries(), GetNumBytes(), dwEndTime - dwStartTime);
	OutputDebugString(szMessage);
#endif
	CloseHandle(hFile);
	return TRUE;
