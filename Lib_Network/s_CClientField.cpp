...
void CClientField::CloseClient( DWORD dwClient )
{
	...
	if (m_pClient[dwClient].IsOnline())
	{
		...
		{
			//iplimit
			IPRemoveClient( m_pClient[NET_RESERVED_SESSION].GetIP(), NET_RESERVED_SESSION );
		...
		if (dwClient >= NET_RESERVED_SLOT)
		{
			if (dwAgentSlot != dwClientSlot)
			{
				...
				{
					//iplimit
					IPRemoveClient( m_pClient[dwAgentSlot].GetIP(), dwAgentSlot );
				...
				{
					//iplimit
					IPRemoveClient( m_pClient[dwClientSlot].GetIP(), dwClientSlot );
