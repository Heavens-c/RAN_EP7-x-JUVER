...
private:
	...
	DWORD				m_dwTyrannyDamage[TYRANNY_SCHOOL_SIZE];
	float				m_fTyrannyDamageLast[TYRANNY_SCHOOL_SIZE];

public:
	/*pvp tyranny, Juver, 2017/08/25 */
	void	TyrannyResetTower();
	void	TyrannyDamageCheck();

	/*pvp capture the flag, Juver, 2018/02/06 */
	void	CaptureTheFlagSetHide( bool bHide );
...
public:
	...
	virtual DWORD GetBonusExp () const				{ return DWORD(m_pCrowData->m_dwBonusExp*GLCONST_CHAR::GETEXP_SCALE()); };
