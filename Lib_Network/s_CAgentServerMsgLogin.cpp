...
void CAgentServer::ChinaMsgLogin( MSG_LIST* pMsg )
{
	...
	//PCID
	//StringCchCopy (szPcID, PCID_LENGTH+1, pNml->szPCID );
...
void CAgentServer::ChinaMsgLoginBack( NET_LOGIN_FEEDBACK_DATA2* nlfd2 )
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK)
	{
			...
			(strcmp( m_pClientManager->GetUserID( dwClient ),   nlfd2->szUserid ) == 0))
		{
			...
			nlfd.uChaRemain = nlfd2->uChaRemain;
			nlfd.dwGameTime = nlfd2->nChinaGameTime; //itemmall
			nlfd.uUserPoints = nlfd2->uUserPoints;
			nlfd.uBattlePoints = nlfd2->uBattlePoints;
			::StringCchCopy(nlfd.szUserEmail, USR_EMAIL+1, (const char*) nlfd2->szUserEmail);
			...
			m_pClientManager->SetUserPoint ( dwClient , nlfd2->uUserPoints ); //itemmall
			m_pClientManager->SetBattlePoint ( dwClient , nlfd2->uBattlePoints );
			m_pClientManager->SetUserEmail ( dwClient , nlfd2->szUserEmail );
...
void CAgentServer::MsgLogInBack (NET_LOGIN_FEEDBACK_DATA2* nlfd2)
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK)
	{
			...
			nTestNum != 100*/ )
		{
			...
			m_pClientManager->SetChatBlockDate(dwClient, nlfd2->tChatBlockTime); // 채팅 블록 기간			
			//userpanel
			m_pClientManager->SetUserPoint ( dwClient , nlfd2->uUserPoints );
			m_pClientManager->SetBattlePoint ( dwClient , nlfd2->uBattlePoints );
			m_pClientManager->SetUserEmail ( dwClient , nlfd2->szUserEmail );
			m_pClientManager->SetChinaGameTime( dwClient, nlfd2->nChinaGameTime );
/*
...
//			m_pClientManager->SetThaiClass	  ( dwClient, nlfd2->nThaiCC_Class ); // 테스트용
			...
			// 극강부 남/여 생성 가능 숫자를 넘겨준다.
// 극강부 추가
#if	defined(_RELEASED) || defined(KRT_PARAM) || defined(KR_PARAM) || defined ( TW_PARAM ) || defined(HK_PARAM) 
			nlfd.nExtremeM = nlfd2->nExtremeM;
			nlfd.nExtremeW = nlfd2->nExtremeW;
#else
			nlfd.nExtremeM = nlfd2->nExtremeM = 0;
