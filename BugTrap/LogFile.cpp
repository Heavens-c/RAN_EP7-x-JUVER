 ...
 */
void CLogFile::GetTimeStatistics(const SYSTEMTIME* pSystemTime, PTSTR pszTimeStatistics)
{
	_stprintf(pszTimeStatistics, 
...
void CLogFile::FillEntryText(BUGTRAP_LOGLEVEL eLogLevel, const SYSTEMTIME* pSystemTime, PCTSTR pszEntry)
{
	...
	if (m_dwLogFlags & BTLF_SHOWTIMESTAMP)
	{
		...
		GetTimeStatistics(pSystemTime, szTimeStatistics);
...
PTSTR CLogFile::FormatBufferV(PCTSTR pszFormat, va_list argList)
{
	...
	for (;;)
	{
		...
		if (pchFormatBuffer == NULL)
			return NULL;
		int iResult = _vsntprintf(pchFormatBuffer, ((size_t)-1), pszFormat, argList);
		if (iResult < 0)
