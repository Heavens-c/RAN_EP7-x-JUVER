...
enum GLCONST
{
	...
	MAX_SELECTCHAR		= 3, // 캐릭터 선택 화면에서 보이는 최대 캐릭터의 수, 현재 4개
...
//	extern float		fDISPRICE;				//	아이템 재판매 가격.
	...
	//---------------------------------------------------------------------------------------[연마]
	//userpanel
	extern WORD			wGameTimeDiv;			//	렙업당 스탯 포인트 부여값.
	extern WORD			wGameTimeRes;			//	렙업당 스탯 포인트 부여값.
	...
	extern DWORD		dwGM_NORMAL_NAME_COLOR;	// gm color name - devart22 
	...
	extern float		fEXP_SCALE_BACKUP;			//	Note : 경험치 배율.
	extern float		fITEM_DROP_SCALE_BACKUP;	//	Note : 아이탬 드롭율.
	extern float		fMONEY_DROP_SCALE_BACKUP;	//	Note : 돈 드롭율.
	...
	extern GLCONST_CHARCLASS	cCONSTCLASS[GLCI_NUM_8CLASS];	//	캐릭터 클래스별 상수.
	...
	//
	extern char					szCharSkin[GLCI_NUM_8CLASS][MAX_PATH];	//	캐릭별 초기 형상.
