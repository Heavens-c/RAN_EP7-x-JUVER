...
void COuterInterface::ModalMsgProcess ( UIGUID nCallerID, DWORD dwMsg )
{
	...
	case OUTER_MODAL_NON_PK_CHANNEL:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				...
				if ( IsVisibleGroup ( SELECT_CHARACTER_USERPANEL ) )			HideGroup ( SELECT_CHARACTER_USERPANEL );
	...
	case OUTER_MODAL_CONNECTCLOSED:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				...
				if ( IsVisibleGroup ( CHARSELECT_PASS_PAGE ) )		HideGroup ( CHARSELECT_PASS_PAGE );
	...
	/*security key on charselect, DevArt22*/
	case OUTER_MODAL_CHARSELECT_PASS:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if ( IsVisibleGroup ( CHARSELECT_PASS_PAGE ) )			HideGroup ( CHARSELECT_PASS_PAGE );
