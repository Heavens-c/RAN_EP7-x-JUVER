...
CTradeControlPage::CTradeControlPage () :
	m_pMoney ( NULL ),
    m_pPlayerName ( NULL ),
	m_pLock( NULL ) /*trade lock, Juver, 2018/01/02 */
{
}
...
//	pButton->SetUseDynamic ( TRUE );
	...
	m_pLock = CreateTextButton ( "TRADE_LOCK_BUTTON", TRADE_LOCK_BUTTON, (char*)ID2GAMEWORD ( "TRADE_LOCK_BUTTON", 0 ) );
...
void CTradeControlPage::ApplyButtonUpdate ()
{
	if ( m_bMyPage )
	{
		...
		{
			/*trade lock, Juver, 2018/01/02 */
			BOOL bLock = GLTradeClient::GetInstance().GetMyTrade().GetLock();
	...
	else
	{
		...
		{
			/*trade lock, Juver, 2018/01/02 */
			BOOL bLock = GLTradeClient::GetInstance().GetTarTrade().GetLock();
...
//				}
	...
	case TRADE_LOCK_BUTTON:
		if ( CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
		{
			if ( m_bMyPage )
			{
				GLGaeaClient::GetInstance().GetCharacter()->ReqTradeLock();
			}
		}
		break;
	}
