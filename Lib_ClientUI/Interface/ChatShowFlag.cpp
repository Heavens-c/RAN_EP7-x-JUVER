...
CUIChatShowFlag::CUIChatShowFlag ()
	...
	, m_pRegionalButton(NULL)
	, m_pRegionalTextBox(NULL)
{
}
...
//		pBasicLineBox->ResetAlignFlagLine ();
	...
	/*regional chat, Juver, 2017/12/06 */
	m_pRegionalButton = CreateButton ( "CHAT_REGIONAL_CHECK", "CHAT_REGIONAL_CHECK_F", CHAT_REGIONAL_CHECK );
	m_pRegionalButton->SetFlip ( TRUE );
	m_pRegionalTextBox = CreateTextBox ( "CHAT_REGIONAL_CHECK_TEXT", ID2GAMEWORD("CHAT_ONOFF",6), CHAT_REGIONAL_TEXTBOX );
}
...
void CUIChatShowFlag::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	//	고치기전까지의 사용으로 임시적용
	{
		...
		m_pRegionalButton->SetVisibleSingle ( TRUE ); /*regional chat, Juver, 2017/12/06 */
	}
	...
	if ( m_pRegionalButton->IsFlip() )	AddMessageEx ( UIMSG_CHAT_REGIONAL );  /*regional chat, Juver, 2017/12/06 */
}
...
void CUIChatShowFlag::TranslateUIMessage ( UIGUID cID, DWORD dwMsg )
{
	...
	case CHAT_SYSTEM_TEXTBOX:
		...
		/*regional chat, Juver, 2017/12/06 */
	case CHAT_REGIONAL_CHECK:
	case CHAT_REGIONAL_TEXTBOX:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( UIMSG_LB_DOWN & dwMsg )
				{
					BOOL bFlip = !m_pRegionalButton->IsFlip();
					m_pRegionalButton->SetFlip ( bFlip );

					D3DCOLOR dwColor = NS_UITEXTCOLOR::DARKGRAY;
					if ( bFlip ) dwColor = NS_UITEXTCOLOR::WHITESMOKE;					

					m_pRegionalTextBox->SetUseTextColor ( 0, TRUE );
					m_pRegionalTextBox->SetTextColor ( 0, dwColor );
				}
			}
		}break;
	}
