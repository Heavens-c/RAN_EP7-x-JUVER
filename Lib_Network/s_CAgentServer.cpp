...
CAgentServer::CAgentServer( HWND hWnd, HWND hEditBox, HWND hEditBoxInfo, HWND hEditBoxStats )
	...
	if (pFieldInfo != NULL)
	{
		for (int nCh=0; nCh<MAX_CHANNEL_NUMBER; ++nCh)
		{
			for (int nSvrNum=0; nSvrNum<FIELDSERVER_MAX; ++nSvrNum)
			{
				...
				std::string strIP = pFieldInfo[nCh*MAX_CHANNEL_NUMBER+nSvrNum].szServerIP;
				if ( strIP.size() )
					CIPFilter::GetInstance()->AddIPKnown( strIP );
			}
		}
	}
		...
		if (pFieldInfo != NULL){
			for (int nCh=0; nCh<MAX_CHANNEL_NUMBER; ++nCh){
				for (int nSvrNum=0; nSvrNum<FIELDSERVER_MAX; ++nSvrNum){
					std::string strIP = pFieldInfo[nCh*MAX_CHANNEL_NUMBER+nSvrNum].szServerIP;
					if ( strIP.size() )
						m_pClientManager->IPAddExcluded( strIP );
				}
			}
		}
	}
	...
	if ((dwCurrentTime - m_dwHeartBeatTime) > SESSION_CHECK_TIME)
	{
		...
		if (m_bGameGuardInit == true && m_bGameGuardAuth ==true)
		{
			...
			if (m_bHeartBeat)
			{
				...
				for (dwN = NET_RESERVED_SLOT; dwN < m_nMaxClient; ++dwN)
				{
...
void CAgentServer::PrintDebugMsg( DWORD dwUdpateTime )
{
	...
	{		
		...
		CConsoleMessage::GetInstance()->WriteStats( _T("INSTANCE %u"), 
			GLAgentServer::GetInstance().GetInstantMapNum2() );
...
void CAgentServer::IPFilterCleanIP( std::string _strIP )
{
