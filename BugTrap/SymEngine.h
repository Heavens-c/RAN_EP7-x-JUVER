...
public:
	...
	struct CStackTraceEntry
	{
		...
		/// Function where ер?entry is located.
...
private:
	...
	void GetErrorInfo(CXmlWriter& rXmlWriter);
	/// Get register values.
	...
	void GetTimeStamp(PTSTR pszTimeStamp);
...
public:
	...
	BOOL WriteScreenShot(PCTSTR pszFileName);
	/// Writes dxdialog to file.
	BOOL WriteDxDialog(PCTSTR pszFileName);
	...
	/// Get OS information.
	static void GetOsString(PTSTR pszOSString);
	/// Get description of system memory.
	static void GetMemString(PTSTR pszMemString);
	/// Get process environment strings.
