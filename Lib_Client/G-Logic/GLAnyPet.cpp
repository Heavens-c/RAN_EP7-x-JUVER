...
HRESULT GLAnyPet::Create ( PSDROPPET pDropPet, NavigationMesh* pNavi, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( pTarChar )
	{
		if ( m_sActiveSkillID_A != NATIVEID_NULL () )
		{
			...
			FACTEFF::NewSkillFactEffect ( sTargetID, pTarChar->GetCharSkin (), m_sActiveSkillID_A, pTarChar->GetMatrix (), sTargetID.vPos );
		}

		if ( m_sActiveSkillID_B != NATIVEID_NULL () )
		{
			STARGETID sTargetID( CROW_PC, pTarChar->m_dwGaeaID, pTarChar->GetPosition () );
			FACTEFF::NewSkillFactEffect ( sTargetID, pTarChar->GetCharSkin (), m_sActiveSkillID_B, pTarChar->GetMatrix (), sTargetID.vPos );
		}
	}
...
/*dual pet skill, Juver, 2017/12/27 */
void GLAnyPet::MsgSkillChange_A ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_SKILLCHANGE_A_BRD* pNetMsg = ( GLMSG::SNETPET_REQ_SKILLCHANGE_A_BRD* ) nmg;

	// 이전 이펙트 제거
	PGLCHARCLIENT pTarChar = GLGaeaClient::GetInstance().GetChar ( pNetMsg->dwTarID );
	if ( pTarChar )
	{
		FACTEFF::DeleteSkillFactEffect ( STARGETID(CROW_PC, pTarChar->m_dwGaeaID, pTarChar->GetPosition () ), pTarChar->GetCharSkin (), m_sActiveSkillID_A );
	}
	...
	{
		PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( m_sActiveSkillID_A.wMainID, m_sActiveSkillID_A.wSubID );
		if ( !pSkill )								return;
...
/*dual pet skill, Juver, 2017/12/27 */
void GLAnyPet::MsgSkillChange_B ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_SKILLCHANGE_B_BRD* pNetMsg = ( GLMSG::SNETPET_REQ_SKILLCHANGE_B_BRD* ) nmg;

	// 이전 이펙트 제거
	PGLCHARCLIENT pTarChar = GLGaeaClient::GetInstance().GetChar ( pNetMsg->dwTarID );
	if ( pTarChar )
	{
		FACTEFF::DeleteSkillFactEffect ( STARGETID(CROW_PC, pTarChar->m_dwGaeaID, pTarChar->GetPosition () ), pTarChar->GetCharSkin (), m_sActiveSkillID_B );
	}
	...
	{
		PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( m_sActiveSkillID_B.wMainID, m_sActiveSkillID_B.wSubID );
		if ( !pSkill )								return;
...
DxEffSingleGroup* GLAnyPet::SK_EFF_SELFZONE ( const SANIMSTRIKE &sStrikeEff, const SKILL::EMEFFECTPOS emPOS, const char* const szZONE_EFF, STARGETID *pTarget )
{
	D3DXMATRIX matTrans;
...
void GLAnyPet::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_PET_STOP_BRD:						MsgStop ( nmg );			break;
		...
		/*dual pet skill, Juver, 2017/12/27 */
	case NET_MSG_PET_REQ_SKILLCHANGE_A_BRD:			MsgSkillChange_A ( nmg );	break;
	case NET_MSG_PET_REQ_SKILLCHANGE_B_BRD:			MsgSkillChange_B ( nmg );	break;
...
void GLAnyPet::GetSkinAABB( D3DXVECTOR3& vMin, D3DXVECTOR3& vMax )
{ 
	if( m_pSkinChar )
		m_pSkinChar->GetAABBBox( vMax, vMin );
}

float GLAnyPet::GetSkinScale()
{ 
	if( !m_pSkinChar )	return 1.0f;
	return m_pSkinChar->GetScale();
}

float GLAnyPet::GetSkinHeight()
{ 
