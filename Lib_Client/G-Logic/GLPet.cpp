...
SPETCARDINFO& SPETCARDINFO::operator= ( const SPETCARDINFO& rvalue )
{
	...
	m_sActiveSkillID_A	= rvalue.m_sActiveSkillID_A;
	m_sActiveSkillID_B	= rvalue.m_sActiveSkillID_B;
	m_bDualSkill		= rvalue.m_bDualSkill;
...
void GLPET::ASSIGN ( const GLPET& sPetData )
{
	...
	/*dual pet skill, Juver, 2017/12/27 */
	m_sActiveSkillID_A	= sPetData.m_sActiveSkillID_A;
	m_sActiveSkillID_B	= sPetData.m_sActiveSkillID_B;
	m_bDualSkill		= sPetData.m_bDualSkill;
...
BOOL GLPET::GETPUTONITEMS_BYBUF ( CByteStream &ByteStream ) const
{
	...
	DWORD dwSize = (DWORD) PET_ACCETYPE_SIZE;
	ByteStream << dwSize;
...
{ 
	...
	if ( i >= PET_ACCETYPE_SIZE ) return SITEMCUSTOM ();
...
{ 
	...
	if ( i >= PET_ACCETYPE_SIZE ) return;
	m_PutOnItems[i] = SITEMCUSTOM ( NATIVEID_NULL() );
}

EMPIECECHAR GLPET::GetPieceFromSlot ( PET_ACCESSORY_TYPE emType )
{
	...
	case PET_TYPE_N:	//not sure
		{
			if ( emType == PET_ACCETYPE_SIZE )	
				return PIECE_SIZE;
			if ( emType == PET_ACCETYPE_A )			
				return PIECE_HEAD;
			else								 
				return PIECE_LOBODY;
		}break;
	...
	for ( WORD i = 0; i < PET_ACCETYPE_SIZE; ++i )
	{
		...
		if ( pITEM->IsTIMELMT() )
		{
			...
			if ( cTIME_CUR > cTIME_LMT )
			{
				...
				NetMsg.accetype = (PET_ACCESSORY_TYPE)i;
				...
				NetMsgBrd.accetype = (PET_ACCESSORY_TYPE)i;
...
namespace COMMENT
{
	std::string PET_TYPE[PET_TYPE_SIZE] = 
	{ 
		"Type_A(Siberian Husky)",
		"Type_B(Turtle)", 
		"Type_C(White Baby Tiger)", 
		"Type_D(Rani Bear)", 
		"Type_E(Owl)", 
		"Type_F(Parrot)", 
		"Type_G(Rani Stone)", 
