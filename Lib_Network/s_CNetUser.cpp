...
#include "s_CIPFilter.h"	//add by fat dick
...
int CNetUser::addRcvMsg(void* pMsg, DWORD dwSize)
{
	...
	if (nTotalRcvSize < 0 )
	{
		CConsoleMessage::GetInstance()->Write(_T("ERROR:%d %s Buffer Negative"),  nTotalRcvSize, m_szIp);
        ResetRcvBuffer();		
		return NET_ERROR;
	}
	...
	{
		CConsoleMessage::GetInstance()->Write(_T("ERROR:%d %s Buffer Over"),  nTotalRcvSize, m_szIp);
        ResetRcvBuffer();		
		return NET_ERROR;
	}
	else
...
{
	char szTempChar[12];
	strcpy( szTempChar, m_pRcvManager->getGarbageMsg() );

	if( strcmp( szTempChar, m_szRecvGarbageMsg[0] ) != 0 &&
		strcmp( szTempChar, m_szRecvGarbageMsg[1] ) != 0 &&
//		strcmp( szTempChar, m_szRecvGarbageMsg[2] ) != 0 &&
		strlen( szTempChar ) != 0 )
	{
		strcpy( m_szRecvGarbageMsg[m_dwGarbageNum], szTempChar );
		m_dwGarbageNum++;
		...
		if( pNmg->nType < NET_MSG_LOBBY ) sprintf( szTempChar2, "LOGIN MESSAGE!! LOGIN_MSG NUM %d, MSG NUM %d", NET_MSG_LGIN, (DWORD)pNmg->nType );
		else if( pNmg->nType < NET_MSG_GCTRL ) sprintf( szTempChar2, "LOBBY MESSAGE!! LOBBY_MSG NUM %d, MSG NUM %d", NET_MSG_LOBBY, (DWORD)pNmg->nType );
		else sprintf( szTempChar2, "GTRL MESSAGE!! GTRL_MSG NUM %d, MSG NUM %d", NET_MSG_GCTRL, (DWORD)pNmg->nType );
		...
		CIPFilter::GetInstance()->AddIPBlock( m_szIp, true );
	...
	else
	{
		...
		if( bClient )
		{
			...
			else if( strlen( m_pRcvManager->getGarbageMsg() ) == 0 )
			{
				...
				CloseClient();
				CConsoleMessage::GetInstance()->WriteConsole(
				_T("ERROR:CNetUser::CloseClient client already closed IP %s"),m_szIp );
				CloseClient();
			}
			return msg;
		}else{
...
//userpanel
void CNetUser::SetBattlePoint( USHORT uBattlePoint )
{
	m_uBattlePoint = uBattlePoint;
}

void CNetUser::SetUserPoint( USHORT uUserPoint )
{
	m_uUserPoint = uUserPoint;
}

void CNetUser::SetUserEmail( const TCHAR* szEmail )
{
	 StringCchCopy( m_szEmail, USR_EMAIL+1 , szEmail );
}


int CNetUser::GetBattlePoint()
{
	return m_uBattlePoint;
}

int CNetUser::GetUserPoint()
{
	return m_uUserPoint;
}

TCHAR* CNetUser::GetUserEmail()
{
	return m_szEmail;
}
//userpanel end
...
//userpanel
void CNetUser::SetRandomString( const TCHAR* szRandString )
{
	StringCchCopy( m_szRandomString, USR_CAPTCHA +2 , szRandString );
}

TCHAR* CNetUser::GetRandomString()
{
	return m_szRandomString;
}
...
/*register page, Juver, 2017/11/18 */
bool CNetUser::IsRegister()
{
	return m_bREGISTER;
}

/*register page, Juver, 2017/11/18 */
bool CNetUser::IsRegisterWait()
{
	return m_bREGISTERWAIT;
}

/*register page, Juver, 2017/11/18 */
void CNetUser::SetRegister( bool bSet )
{
	m_bREGISTER = bSet;
}

/*register page, Juver, 2017/11/18 */
void CNetUser::SetRegisterWait( bool bSet )
{
	m_bREGISTERWAIT = bSet;
}