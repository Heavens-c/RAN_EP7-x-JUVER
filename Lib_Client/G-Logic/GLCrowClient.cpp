...
HRESULT GLCrowClient::CreateCrow ( GLLandManClient *pLandManClient, SDROP_CROW *pCrowDrop, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	m_wTyrannyOwner = pCrowDrop->m_wTyrannyOwner;

	/*pvp tyranny, Juver, 2017/08/24 */
	for ( int i=0; i<TYRANNY_SCHOOL_SIZE; ++i )
		m_fTyrannyDamage[i] = pCrowDrop->m_fTyrannyDamage[i];

	/*pvp capture the flag, Juver, 2018/02/06 */
	m_bCaptureTheFlagCrowHide = pCrowDrop->m_bCaptureTheFlagCrowHide;
	...
	{	/*pvp tyranny, Juver, 2017/08/24 */
		...
		for ( int i=0; i< TYRANNY_SCHOOL_SIZE; ++ i )
			DxEffGroupPlayer::GetInstance().DeletePassiveEffect( GLCONST_CHAR::strSchoolEffect[i].c_str(), STARGETID( m_pCrowData->m_emCrow, m_dwGlobID, m_vPos ) );

		if ( m_wTyrannyOwner < TYRANNY_SCHOOL_SIZE )
			DxEffGroupPlayer::GetInstance().NewEffGroup( GLCONST_CHAR::strSchoolEffect[m_wTyrannyOwner].c_str(), matEffect, &STARGETID( m_pCrowData->m_emCrow, m_dwGlobID, m_vPos ) );
	}
...
void GLCrowClient::GetSkinAABB( D3DXVECTOR3& vMin, D3DXVECTOR3& vMax )
{ 
	if( m_pSkinChar )
		m_pSkinChar->GetAABBBox( vMax, vMin );
}

float GLCrowClient::GetSkinScale()
{ 
	if( !m_pSkinChar )	return 1.0f;
	return m_pSkinChar->GetScale();
}

float GLCrowClient::GetSkinHeight()
{ 
...
DxEffSingleGroup* GLCrowClient::SK_EFF_SELFZONE ( const SANIMSTRIKE &sStrikeEff, const SKILL::EMEFFECTPOS emPOS, const char* const szZONE_EFF, STARGETID *pTarget )
{
...
/*skill activated effect, Juver, 2017/12/29 */
DxEffSingleGroup* GLCrowClient::SK_EFF_TARZONE ( const STARGETID &_sTarget, const SKILL::EMEFFECTPOS emPOS, const char* const szZONE_EFF )
{
	D3DXMATRIX matTrans;
...
void GLCrowClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_TYRANNY_CROW_OWNER:
		{
			GLMSG::SNETCROW_TYRANNY_OWNER *pNetMsg = (GLMSG::SNETCROW_TYRANNY_OWNER *) nmg;
			m_wTyrannyOwner = pNetMsg->wTyrannyOwner;
	...
	case NET_MSG_GCTRL_TYRANNY_CROW_DAMAGE:
		{
			GLMSG::SNETCROW_TYRANNY_DAMAGE *pNetMsg = (GLMSG::SNETCROW_TYRANNY_DAMAGE *) nmg;

			for ( int i=0; i< TYRANNY_SCHOOL_SIZE; ++ i )
				m_fTyrannyDamage[i] = pNetMsg->fTyrannyDamage[i];
		}break;

		/*pvp capture the flag, Juver, 2018/02/06 */
	case NET_MSG_GCTRL_CAPTURE_THE_FLAG_CROW_HIDE:
		{
			GLMSG::SNETCROW_CAPTURE_THE_FLAG_SET_HIDE *pNetMsg = (GLMSG::SNETCROW_CAPTURE_THE_FLAG_SET_HIDE *) nmg;
			m_bCaptureTheFlagCrowHide = pNetMsg->bHide;
		}break;
