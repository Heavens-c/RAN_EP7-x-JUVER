...
void CEditorSkinCharView::PostNcDestroy()
{
...
void CEditorSkinCharView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView)
{
	CEditorSkinCharApp *pApp = (CEditorSkinCharApp*) AfxGetApp();
	pApp->SetActive ( bActivate );
	CEditorSkinCharView::SetActive ( bActivate );

	__super::OnActivateView(bActivate, pActivateView, pDeactiveView);
}

void CEditorSkinCharView::OnSize(UINT nType, int cx, int cy)
{
...
void CEditorSkinCharView::SetActive ( BOOL bActive )
{
	DxInputDevice::GetInstance().OnActivate ( bActive );
}

BOOL CEditorSkinCharView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
...
void CEditorSkinCharView::OnInitialUpdate()
{
	CEditorSkinCharApp *pApp = (CEditorSkinCharApp *) AfxGetApp();
...
HRESULT CEditorSkinCharView::FrameMove3DEnvironment ()
{
	HRESULT hr=S_OK;
...
HRESULT CEditorSkinCharView::Render3DEnvironment ()
{
	HRESULT hr=S_OK;
...
HRESULT CEditorSkinCharView::OneTimeSceneInit()
{
	HRESULT hr = S_OK;
...
HRESULT CEditorSkinCharView::ConfirmDevice ( D3DCAPSQ* pCaps, DWORD dwBehavior, D3DFORMAT Format )
{
...
HRESULT CEditorSkinCharView::CreateObjects()
{
...
HRESULT CEditorSkinCharView::InitDeviceObjects()
{
	HRESULT hr = S_OK;
	...
	if ( m_pVehicle )	m_pVehicle->InitDeviceObjects( m_pd3dDevice );
	if ( m_pVehicleSkin )	m_pVehicleSkin->InitDeviceObjects( m_pd3dDevice );
...
HRESULT CEditorSkinCharView::RestoreDeviceObjects()
{
	HRESULT hr=S_OK;
	...
	if ( m_pd3dDevice )
	{
		...
		/*vehicle system, Juver, 2017/08/05 */
		if ( m_pVehicle )	m_pVehicle->RestoreDeviceObjects( m_pd3dDevice );
		if ( m_pVehicleSkin )	m_pVehicleSkin->RestoreDeviceObjects( m_pd3dDevice );
...
HRESULT CEditorSkinCharView::FrameMove()
{
	...
	if ( m_pSkinChar )	
	{
		...
		if ( m_pSkinChar->GETCURANIMNODE() && m_pSkinChar->ISENDANIM() )
		{
		}	
	}
	...
	/*vehicle system, Juver, 2017/08/05 */
	if ( m_pVehicleSkin )
		m_pVehicleSkin->FrameMove( m_fTime, m_fElapsedTime, TRUE, FALSE, FALSE );
	...
	if ( bOk && bCLICK_LEFT && ( dwML&DXKEY_UP ) )
	{
		...
		/*ABL system, Juver, 2017/05/30 */
		if ( m_pAttBoneRender )	pSkeleton = m_pAttBoneRender->GetSkeleton();
		if ( m_pAttBone )	pSkeleton = m_pAttBone->m_pSkeletonLink;
...
HRESULT CEditorSkinCharView::Render()
{
		...
		if ( m_pVehicle )
		{
			...
			if ( m_pVehicleSkin )
			{
				...
				if ( pBoneCur )
				{
					...
					m_pVehicleSkin->Render( m_pd3dDevice, matCombine3 );
				}

				
				if ( m_bRenderSkeleton )
				{
					DxSkeleton* pSkeleton = m_pVehicleSkin->GetSkeleton();
					if ( pSkeleton )
					{
						DWORD dwZFunc;
						m_pd3dDevice->GetRenderState ( D3DRS_ZFUNC,		&dwZFunc );
...
HRESULT CEditorSkinCharView::InvalidateDeviceObjects()
{
	...
	/*vehicle system, Juver, 2017/08/05 */
	if ( m_pVehicle )	m_pVehicle->InvalidateDeviceObjects();
	if ( m_pVehicleSkin )	m_pVehicleSkin->InvalidateDeviceObjects();
...
HRESULT CEditorSkinCharView::DeleteDeviceObjects()
{
#ifdef _DEBUG
	_CrtCheckMemory();
#endif //_DEBUG
	...
	/*vehicle system, Juver, 2017/08/05 */
	if ( m_pVehicle )	m_pVehicle->DeleteDeviceObjects();
	if ( m_pVehicleSkin )	m_pVehicleSkin->DeleteDeviceObjects();
...
HRESULT CEditorSkinCharView::FinalCleanup()
{
