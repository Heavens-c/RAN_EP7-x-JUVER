...
UIGUID COuterInterface::GetRenderStage ()
{
	...
	if ( pFocusControl ) 
	{
		...
		/*security key on charselect, DevArt22*/
		case CHARSELECT_PASS_PAGE:	return m_pCharSelectPassPage->GetPrevPageID ();
		default:					return cID;
		}		
	}
	return NO_ID;
...
void COuterInterface::ToSelectServerPage ( UIGUID cID )
{
	...
	if( IsVisibleGroup( SELECT_CHINA_AREA_PAGE ) || IsVisibleGroup( SELECT_CHARACTER_PAGE ) )			
	{
		HideGroup( SELECT_CHINA_AREA_PAGE );
		HideGroup( SELECT_CHARACTER_PAGE );
	}
	...
	HideGroup ( cID, bMakeMsg );
	HideGroup ( SELECT_CHARACTER_USERPANEL, bMakeMsg );
...
void 	COuterInterface::ToTOPUPPage( UIGUID cID )
{
	bool bMakeMsg = true;

	ShowGroupFocus ( TOPUP_PAGE, bMakeMsg );
	if ( cID == SELECT_CHARACTER_USERPANEL )
	{}
	HideGroup ( CHANGEPIN_PAGE, bMakeMsg );
	HideGroup ( GAMETIMECVT_PAGE, bMakeMsg );
	HideGroup ( PREMIUMPOINTCVT_PAGE, bMakeMsg );
	HideGroup ( CHANGEPASS_PAGE, bMakeMsg );
	HideGroup ( RESETPASS_PAGE, bMakeMsg );
	HideGroup ( RESETPASS_PAGE2, bMakeMsg );
	HideGroup ( RESETPIN_PAGE, bMakeMsg );
	HideGroup ( CHANGEEMAIL_PAGE, bMakeMsg );
	HideGroup ( CHARSELECT_PASS_PAGE, bMakeMsg );
	HideGroup ( COPYRIGHT, bMakeMsg );
	HideGroup ( CHINA_TITLE, bMakeMsg );
}
/*security key on charselect, DevArt22*/
void 	COuterInterface::ToCharSelectPassPage( UIGUID cID )
{
	bool bMakeMsg = true;

	m_pCharSelectPassPage->SetPrevPageID ( GetRenderStage() );

	ShowGroupFocus ( CHARSELECT_PASS_PAGE, bMakeMsg );
	if ( cID == SELECT_CHARACTER_USERPANEL )
	{}
	HideGroup ( CHANGEPIN_PAGE, bMakeMsg );
	HideGroup ( GAMETIMECVT_PAGE, bMakeMsg );
	...
	HideGroup ( COPYRIGHT, bMakeMsg );
	HideGroup ( CHINA_TITLE, bMakeMsg );
	HideGroup ( TOPUP_PAGE, bMakeMsg );
}
void 	COuterInterface::ToGTCVTPage( UIGUID cID )
{
	bool bMakeMsg = true;


	ShowGroupFocus ( GAMETIMECVT_PAGE, bMakeMsg );
...
void 	COuterInterface::ToPPCVTPage( UIGUID cID )
{
	bool bMakeMsg = true;


	ShowGroupFocus ( PREMIUMPOINTCVT_PAGE, bMakeMsg );
...
void 	COuterInterface::ToCPPage( UIGUID cID )
{
	bool bMakeMsg = true;

	ShowGroupFocus ( CHANGEPASS_PAGE, bMakeMsg );
	if ( cID == SELECT_CHARACTER_USERPANEL )
	{}
	HideGroup ( GAMETIMECVT_PAGE, bMakeMsg );
	HideGroup ( CHANGEPIN_PAGE, bMakeMsg );
...
void 	COuterInterface::ToCPNPage( UIGUID cID )
{
	bool bMakeMsg = true;

	ShowGroupFocus ( CHANGEPIN_PAGE, bMakeMsg );
	if ( cID == SELECT_CHARACTER_USERPANEL )
	{}
	HideGroup ( CHANGEPASS_PAGE, bMakeMsg );
	HideGroup ( GAMETIMECVT_PAGE, bMakeMsg );
...
void 	COuterInterface::ToRPNPage( UIGUID cID )
{
	bool bMakeMsg = true;

	ShowGroupFocus ( RESETPIN_PAGE, bMakeMsg );
	if ( cID == SELECT_CHARACTER_USERPANEL )
	{}
	HideGroup ( CHANGEPASS_PAGE, bMakeMsg );
	HideGroup ( GAMETIMECVT_PAGE, bMakeMsg );
...
void 	COuterInterface::ToRPSPage( UIGUID cID )
{
	bool bMakeMsg = true;

	ShowGroupFocus ( RESETPASS_PAGE, bMakeMsg );
	HideGroup ( cID, bMakeMsg );
	HideGroup ( CHANGEPASS_PAGE, bMakeMsg );
	HideGroup ( GAMETIMECVT_PAGE, bMakeMsg );
	HideGroup ( RESETPIN_PAGE, bMakeMsg );
...
void 	COuterInterface::ToRPSPage2( UIGUID cID )
{
	bool bMakeMsg = true;

	ShowGroupFocus ( RESETPASS_PAGE2, bMakeMsg );
	if ( cID == SELECT_CHARACTER_USERPANEL )
	{}
	HideGroup ( CHANGEPASS_PAGE, bMakeMsg );
	HideGroup ( GAMETIMECVT_PAGE, bMakeMsg );
...
void 	COuterInterface::ToCEPage( UIGUID cID )
{
	bool bMakeMsg = true;
	ShowGroupFocus ( CHANGEEMAIL_PAGE, bMakeMsg );
	if ( cID == SELECT_CHARACTER_USERPANEL )
	{}
	HideGroup ( CHANGEPIN_PAGE, bMakeMsg );
	HideGroup ( RESETPASS_PAGE, bMakeMsg );
...
void	COuterInterface::ToSelectCharacterPage ( UIGUID cID )
{
	...
	if ( m_pSelectServerPage ){
		...
		ShowGroupFocus ( SELECT_CHARACTER_USERPANEL, bMakeMsg );
	}
