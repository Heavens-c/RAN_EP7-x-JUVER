...
void COdbcManager::VerifyTimeValid( std::string strFunction, std::string strVariable, TIMESTAMP_STRUCT& time )
{
	TIMESTAMP_STRUCT tOrig = time;

	if ( !_HLIB::timestamp_correction( time ))
	{
		//CConsoleMessage::GetInstance()->WriteDatabase( _T("COdbcManager::VerifyTimeValid Error Function:%s Variable:%s Year:%d Month:%d Day:%d Hour:%d Minute:%d Second:%d "), 
		//	strFunction.c_str(), strVariable.c_str(), tOrig.year, tOrig.month, tOrig.day, tOrig.hour, tOrig.minute, tOrig.second );

		CDebugSet::ToFileWithTime( "_VerifyTimeValid.txt", "Time Error Function:%s Variable:%s Year:%d Month:%d Day:%d Hour:%d Minute:%d Second:%d ", 
			strFunction.c_str(), strVariable.c_str(), tOrig.year, tOrig.month, tOrig.day, tOrig.hour, tOrig.minute, tOrig.second );
	}
}