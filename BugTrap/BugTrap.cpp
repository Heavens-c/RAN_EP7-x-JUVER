...
{
	if (pszAppName)
		_tcscpy(g_szAppName, pszAppName);
	else
		*g_szAppName = _T('\0');
}
...
{
	if (pszAppVersion)
		_tcscpy(g_szAppVersion, pszAppVersion);
	else
		*g_szAppVersion = _T('\0');
}
...
{
	if (pszSupportURL)
		_tcscpy(g_szSupportURL, pszSupportURL);
	else
		*g_szSupportURL = _T('\0');
}
...
{
	if (pszSupportEMail)
		_tcscpy(g_szSupportEMail, pszSupportEMail);
	else
...
extern "C" BUGTRAP_API void APIENTRY BT_SetNotificationEMail(PCTSTR pszNotificationEMail)
{
	if (pszNotificationEMail)
		_tcscpy(g_szNotificationEMail, pszNotificationEMail);
	else
...
{
	if (pszSupportHost)
		_tcscpy(g_szSupportHost, pszSupportHost);
	else
...
//				return ERROR_INCORRECT_SIZE;
			{
				...
				_tcscpy(pLogFileEntry->szLogFileName, pLogLink->GetLogFileName());
			}
			break;
		case BTLT_REGEXPORT:
...
//				return ERROR_INCORRECT_SIZE;
			{
				...
				_tcscpy(pRegExportEntry->szLogFileName, pRegLink->GetLogFileName());
				_tcscpy(pRegExportEntry->szRegKey, pRegLink->GetRegKey());
			}
			break;
		default:
			_ASSERT(FALSE);
...
extern "C" BUGTRAP_API void APIENTRY BT_SetReportFilePath(PCTSTR pszReportFilePath)
{
	if (pszReportFilePath && *pszReportFilePath)
		_tcscpy(g_szReportFilePath, pszReportFilePath);
	else
		SetDefaultReportPath();
}
...
{
	if (pszMailProfile)
		_tcscpy(g_szMailProfile, pszMailProfile);
	else
		*g_szMailProfile = _T('\0');
	if (pszMailPassword)
		_tcscpy(g_szMailPassword, pszMailPassword);
	else
...
extern "C" BUGTRAP_API void APIENTRY BT_SetUserMessageFromCode(DWORD dwErrorCode)
{
	...
	_stprintf(szErrorCode, _T("0x%08lX - "), dwErrorCode);
	strTemp << szErrorCode;
	if (pszMessageBuffer)
	{
...
extern "C" BUGTRAP_API BOOL APIENTRY BT_ReadVersionInfo(HMODULE hModule)
{
		...
		VerQueryValue(pVersionInfo, _T("\\VarFileInfo\\Translation"), (void**)&pTranslation, &uTranslationLength))
	{
		...
		if (iItemPos >= 0)
		{
			...
			_stprintf(szSubBlock, _T("\\StringFileInfo\\%04x%04x\\AppName"), pTranslation[iItemPos].wLanguage, pTranslation[iItemPos].wCodePage);
			if (VerQueryValue(pVersionInfo, szSubBlock, (void**)&pszValue, &uValueLength))
			{
				bResult = TRUE;
				BT_SetAppName(pszValue);
				_stprintf(szSubBlock,  _T("\\StringFileInfo\\%04x%04x\\AppVersion"), pTranslation[iItemPos].wLanguage, pTranslation[iItemPos].wCodePage);
				if (VerQueryValue(pVersionInfo, szSubBlock, (void**)&pszValue, &uValueLength))
...
static DWORD WriteRegData(REGEXPORT_DATA& rRegData, DWORD dwValueType, PBYTE pbValueData, DWORD dwValueSize, DWORD dwPosition)
{
	...
	{
		_stprintf(rRegData.szAuxiliaryBuffer, _T("hex(%02x):"), dwValueType);
	...
	for (;;)
	{
		...
		{
			if (dwValueSize > 1)
				_stprintf(rRegData.szAuxiliaryBuffer, _T("%02x,"), *pbValueData);
			else
				_stprintf(rRegData.szAuxiliaryBuffer, _T("%02x"), *pbValueData);
	...
	for (DWORD dwRegIndex = 0; ; ++dwRegIndex)
	{
		...
		if (lResult == ERROR_SUCCESS)
		{
			...
			case REG_SZ:
				...
				break;
			case REG_DWORD:
				_stprintf(rRegData.szAuxiliaryBuffer, _T("dword:%08x"), *(PDWORD)rRegData.pbValueBuffer);
...
static void ExportRegKeys(REGEXPORT_DATA& rRegData, HKEY hKey, DWORD dwRegPathLength)
{
	...
	for (DWORD dwRegIndex = 0; ; ++dwRegIndex)
	{
		...
		if (lResult == ERROR_SUCCESS)
		{
			_tcscpy(pszRegPathEnd, rRegData.szName);
			HKEY hNestedKey;
...
static void WriteRegError(REGEXPORT_DATA& rRegData, PCTSTR pszRegKey, LONG lResult)
{
	...
	WriteRegData(rRegData, _T("]\r\n; Error code: "));
	_stprintf(rRegData.szAuxiliaryBuffer, _T("0x%08lX"), lResult);
	WriteRegData(rRegData, rRegData.szAuxiliaryBuffer);
	PTSTR pszMessageBuffer = NULL;
			...
			CreateParentFolder(szRegFileName))
		{
			...
			if (RegData.pbValueBuffer)
			{
				...
				if (RegData.hFile != INVALID_HANDLE_VALUE)
				{
					...
					_tcscpy(RegData.szRegPathBuffer, pszRegPath);
					HKEY hKey;
