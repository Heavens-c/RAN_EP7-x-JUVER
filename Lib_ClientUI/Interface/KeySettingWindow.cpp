...
void CKeySettingWindow::CreateSubControl()
{
	...
	// 퀵슬롯 
	for(i = 0; i < QUICK_SLOT_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_QUICKSLOT%d", i);
	...
	// 스킬 슬롯
	for (i = 0; i < QUICK_SKILL_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_QUICKSKILL%d", i);
		pTextBox = CreateStaticControl ( strTemp, m_pFont, dwFontColor, nAlign );
	...
	// 메뉴 슬롯
	for (i = 0; i < MENU_SHOTCUT_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_MENUSHOTCUT%d", i);
		pTextBox = CreateStaticControl (strTemp, m_pFont, dwFontColor, nAlign );
	...
	// EditBox배경 생성 ( 퀵슬롯 )
	for(i = 0; i < QUICK_SLOT_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_QUICKEDIT_BACK%d", i);
		pBasicLineBox = new CBasicLineBox;
	...
	// EditBox배경 생성 ( 스킬슬롯)
	for(i = 0; i < QUICK_SKILL_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_SKILLEDIT_BACK%d", i);
		pBasicLineBox = new CBasicLineBox;
		pBasicLineBox->CreateSub ( this, "BASIC_LINE_BOX_EDIT", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		pBasicLineBox->CreateBaseBoxEditBox ( strTemp );
		RegisterControl ( pBasicLineBox );        
	...
	// EditBox배경 생성 ( 메뉴숏컷 )
	for(i = 0; i < MENU_SHOTCUT_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_SHOTCUTEDIT_BACK%d", i);
		pBasicLineBox = new CBasicLineBox;
		pBasicLineBox->CreateSub ( this, "BASIC_LINE_BOX_EDIT", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		pBasicLineBox->CreateBaseBoxEditBox ( strTemp );
		RegisterControl ( pBasicLineBox );        
    ...
    // EditBox 생성 ( 퀵슬롯 )
	for(i = 0; i < QUICK_SLOT_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_QUICKEDIT%d", i);		
	...
	// EditBox 생성 ( 스킬슬롯 )
	for(i = 0; i < QUICK_SKILL_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_SKILLEDIT%d", i);		
		pEditBox = new CShotCutBox;
	...
	// EditBox배경 생성 ( 메뉴숏컷 )
	for(i = 0; i < MENU_SHOTCUT_NUM; ++i){
		sprintf(strTemp,"KEY_SETTING_SHOTCUTEDIT%d", i);		
		pEditBox = new CShotCutBox;
