...
void GLStringTable::CLEAR( EMST_TYPE emType )
{
	switch ( emType )
	{
	case ITEM:	m_mapItemStringTable.clear();	break;
	case SKILL:	m_mapSkillStringTable.clear();	break;
	case CROW:	m_mapCrowStringTable.clear();	break;
...
bool GLStringTable::SAVEFILE( EMST_TYPE emType )
{
	...
	CRijndael oRijndael;
	oRijndael.Initialize( CRijndael::VERSION, CRijndael::sm_Version[CRijndael::VERSION-1].c_str(), CRijndael::DEFAULT_KEY_LENGTH );
