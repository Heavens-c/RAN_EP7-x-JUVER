...
HRESULT GLItemMan::LoadFile ( const char* szFile, BOOL bServer, bool bPastLoad )
{
	...
	if ( dwFILEVER>=ENCODE_VER )
		SFile.SetEncodeType ( EMBYTECRYPT_ITEM );
...
//#ifdef DAUMTESTPARAM	// 한국 테섭에서만 2차 랜덤 옵션 파일 사용 ( 준혁 )
	SRANDOM_GEN* GLItemMan::LoadRandomGenData ( const char* szName )
	{
		...
		{
			CDebugSet::ErrorFile( "GLItemMan::LoadRandomGenData", szName );
		...
		for( INT i=0; i<2; ++i )
		{
			for( DWORD j=0; j<sGEN.dwSET_NUM && j<SRANDOM_GEN::MAX_SET; ++j )
			{
				...
				_snprintf( szOPTION, 64, "%s_OPTION_%u", strObj[i], j+1 );
				...
				{
					_snprintf( szOPT_SE, 64, "%s_OPT_SE_%u_%u", strObj[i], j+1, k+1 );
...
HRESULT GLItemMan::LoadRandomOptCsvFile ( CWnd* pWnd )
{
	...
	while( StrFile.GetNextLine( strLine ) )
	{
		...
		StringCchCopy( sITEM.sRandomOpt.szNAME, sizeof(char)*SRANDOM_DATA::NAME_LEN, strName.c_str() );
	}
