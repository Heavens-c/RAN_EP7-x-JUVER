...
void CBasicChatLeftBar::CreateSubControl ()
{
	...
	CUIControl* pRegionalChat = new CUIControl;
	pRegionalChat->CreateSub ( this, "CHAT_REGIONAL_STATE", UI_FLAG_BOTTOM, CHAT_REGIONAL_STATE );	
	RegisterControl ( pRegionalChat );
	m_pCHATSTATE[6] = pRegionalChat;
	...
	case CHAT_ALLIANCE_STATE:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( UIMSG_LB_DOWN & dwMsg )
				{
					AddMessageEx ( UIMSG_MOUSEIN_REGIONAL_CHAT );
				}
			}
		}
		break;
