...
#include "../Lib_ClientUI/Interface/PetskinMixImage.h"
...
HRESULT GLCharacter::SelectiveformBoxOpen(WORD wPosX, WORD wPosY)
{
	...
	{
		CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEWORD("SELFORMBOX_MSG", 0) );
		return E_FAIL;
	}

	if ( !pItem->sBox.VALID() )
	{
		CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEWORD("SELFORMBOX_MSG", 1));
		return E_FAIL;
	}
	...
	ITEM::VEC_BOX vecBoxItems;
	vecBoxItems.reserve(ITEM::SBOX::ITEM_SIZE);

	for ( int i=0; i<ITEM::SBOX::ITEM_SIZE; ++i )
	{
		ITEM::SBOX_ITEM &sBoxItems = pItem->sBox.sITEMS[i];
		vecBoxItems.push_back(sBoxItems);
	}

	if ( !vecBoxItems.empty() )
	{
		CInnerInterface::GetInstance().OpenSelectiveformBox( vecBoxItems );
	}
	else
		CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEWORD("SELFORMBOX_MSG", 1));
}
