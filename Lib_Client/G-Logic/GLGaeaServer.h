...
public:
	...
	//bool					m_bEmptyMsg;
	//void					SetEmptyMsg ( bool bEmptyMsg ) { m_bEmptyMsg = bEmptyMsg; }
	int						m_nRandomNumber;

	//dmk14 | 11-4-16 | pk ranking
	PLAYER_RANKING_STATUS_MAP m_mapPlayerRanking;
	bool					m_bPlayerReqRank;
	float					m_fPlayerReqTimer;
...
private:
	float m_fAutoPotionDelay;

public:
	float GetAutoPotionDelay()				{ return m_fAutoPotionDelay; }
	void SetAutoPotionDelay(float fDelay)	{ m_fAutoPotionDelay = fDelay; }
...
public:
	...
	BOOL		ReqActiveVehicleConfirm ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_ACTIVE_VEHICLE_CONFIRM* pNetMsg );
	...
	BOOL		ReqZoomOutCheatDetected ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_ZOOMOUT_CHEAT_DETECTED* pNetMsg );

	void		GerberSecDetSpdHack ( DWORD dwClientID, DWORD dwGaeaID );
	//void		GerberHackShieldHeartbeat ( DWORD dwClientID, DWORD dwGaeaID );
	...
	void		SetDisTrade( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_DISTRADE_REQ* pNetMsg );

	//	클럽데스매치 랭킹 요청
	BOOL	ReqClubDeathMatchRanking ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_CLUB_DEATHMATCH_RANKING_REQ* pNetMsg );
	
	//dmk14 | 11-4-16 | pk ranking
	BOOL	ReqPlayerRanking ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_MSG_REQ_PLAYERRANKING* pNetMsg );

	BOOL MsgReqHoldArray ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_HOLD_ARRAY* nmg );
	BOOL MsgReqHoldArrayToInven ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_HOLDARRAY_TO_INVEN* nmg );
public:
	...
	BOOL RequestPassenger ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_PASSENGER *pNetMsg );
	BOOL RequestPassengerTarAns ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_PASSENGER_TAR_ANS *pNetMsg );
	BOOL RequestPassengerCancel ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_PASSENGER_CANCEL *pNetMsg );
	...
	BOOL RequestItemShopBuyItem ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_ITEMSHOP_ITEM_BUY *pNetMsg ); //itemmall
	...
	BOOL TyrannyMoveMap ( DWORD dwCharID, SNATIVEID sMapID, D3DXVECTOR3 vPos, BOOL bCheckCurMap = TRUE );
	...
	BOOL SchoolWarsMoveMap ( DWORD dwCharID, SNATIVEID sMapID, D3DXVECTOR3 vPos, BOOL bCheckCurMap = TRUE );

	/*pvp capture the flag, Juver, 2018/01/29 */
	void CaptureTheFlagPlayerTeam( GLMSG::SNETPC_CAPTURE_THE_FLAG_A2F_PLAYER_TEAM *pNetMsg );
	BOOL CaptureTheFlagMoveMap ( DWORD dwCharID, SNATIVEID sMapID, D3DXVECTOR3 vPos, BOOL bCheckCurMap = TRUE );
	...
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 CharView
	BOOL ReqCharView(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_CHAR_VIEW_AGT* pNetMsg);
	/////////////////////////////////////////////////////////////////////////////
...
public:
	...
	BOOL MsgDropCharFromAgent( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_DROPCHAR_TOFIELD* pNetMsgFld );
...
public:
	void	PkRankAdd( DWORD dwKilled, DWORD dwKiller );
...
inline PGLCHAR GLGaeaServer::GetChar(std::string strName)
{
	GLCHAR_MAP_ITER pos = m_PCNameMap.find(strName);
	if (pos == m_PCNameMap.end())		return NULL;

	PGLCHAR pGLChar = (*pos).second;
