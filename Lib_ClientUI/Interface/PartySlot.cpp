...
void CPartySlot::CreateSubControl ()
{
	...
	{	//	클래스 표시
		CString strClassKeyword[GLCI_NUM_8CLASS] = 
		{
			"PARTY_SLOT_CLASS_FIGHTER",
			"PARTY_SLOT_CLASS_ARMS",
			"PARTY_SLOT_CLASS_ARCHER",
			"PARTY_SLOT_CLASS_SPIRIT",
			"PARTY_SLOT_CLASS_EXTREME",
			"PARTY_SLOT_CLASS_EXTREME",
			"PARTY_SLOT_CLASS_FIGHTER",
			"PARTY_SLOT_CLASS_ARMS",
			"PARTY_SLOT_CLASS_ARCHER",
			"PARTY_SLOT_CLASS_SPIRIT",
			"PARTY_SLOT_CLASS_SCIENTIST",
			"PARTY_SLOT_CLASS_SCIENTIST",
			"PARTY_SLOT_CLASS_ASSASSIN",
			"PARTY_SLOT_CLASS_ASSASSIN",
			"PARTY_SLOT_CLASS_TRICKER",
			"PARTY_SLOT_CLASS_TRICKER",
		};
...
{
	if ( nClassType < GLCI_BRAWLER_M && GLCI_TRICKER_W < nClassType )
	{
		GASSERT ( 0 && "CPartySlot::SetClass incorrect class index" );
		return ;
	}
