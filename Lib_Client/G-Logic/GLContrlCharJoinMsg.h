...
namespace GLMSG
{
	...
	struct SNETLOBBY_CHARJOIN
	{
		...
		DWORD				dwNumActivityProg;				
		DWORD				dwNumActivityDone;
	...
	struct SNETPC_LAND_INFO
	{
		...
		float				fCommission;
		bool				bPVPTyranny;		/*pvp tyranny, Juver, 2017/08/25 */
		bool				bPVPSchoolWars;		/*school wars, Juver, 2018/01/19 */
		bool				bPVPCaptureTheFlag;	/*pvp capture the flag, Juver, 2018/01/31 */
		...
		SNETPC_LAND_INFO () 
			...
			, dwGuidClubMarkVer(0)
			, fCommission(0.0f)
			, bPVPTyranny(false)		/*pvp tyranny, Juver, 2017/08/25 */
			, bPVPSchoolWars(false)		/*school wars, Juver, 2018/01/19 */
			, bPVPCaptureTheFlag(false)	/*pvp capture the flag, Juver, 2018/01/31 */
		{
	...
	struct SNETLOBBY_ITEMFOOD
	{
		NET_MSG_GENERIC		nmg;
		SFITEMFACT	sFITEMFACT[FITEMFACT_SIZE];

		SNETLOBBY_ITEMFOOD ()
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_LOBBY_CHAR_ITEMFOOD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};	

	/*activity system, Juver, 2017/10/30 */
	struct SNETLOBBY_ACTIVITY_PROG
	{
		NET_MSG_GENERIC		nmg;

		DWORD					dwNum;
		SACTIVITY_CHAR_DATA		sActivity[EMGLMSG_ACTIVITY_MAX];

		SNETLOBBY_ACTIVITY_PROG ()
			: dwNum(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_LOBBY_CHAR_ACTIVITY_PROG;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}

		BOOL ADD ( const SACTIVITY_CHAR_DATA &sActivity_Data )
		{
			if ( dwNum == EMGLMSG_ACTIVITY_MAX )	return FALSE;

			sActivity[dwNum] = sActivity_Data;
			dwNum++;

			nmg.dwSize = sizeof(*this) - sizeof(SACTIVITY_CHAR_DATA) * ( EMGLMSG_ACTIVITY_MAX - dwNum );
			return TRUE;
		}
	};

	/*activity system, Juver, 2017/10/30 */
	struct SNETLOBBY_ACTIVITY_DONE
	{
		NET_MSG_GENERIC		nmg;
		...
		SNETLOBBY_ACTIVITY_DONE ()
			: dwNum(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_LOBBY_CHAR_ACTIVITY_DONE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	...
	//fastrevive fix
	struct SNETPC_REQ_DEATHTIMER_AG
	{
		NET_MSG_GENERIC		nmg;
		SNETPC_REQ_DEATHTIMER_AG ()
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_DEATHTIMER_AG;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_DEATHTIMER_FB_AG
	{
		NET_MSG_GENERIC		nmg;
		SNETPC_REQ_DEATHTIMER_FB_AG ()
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_DEATHTIMER_FB_AG;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
