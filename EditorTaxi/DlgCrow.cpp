...
void CDlgCrow::ReadData()
{
	...
	for ( int MID = 0; MID < GLCrowDataMan::MAX_CROW_MID; MID++ )
	{
		...
		{        	
			...
			if ( pCROW && pCROW->m_emCrow == CROW_NPC )
			{	
...
void CDlgCrow::OnLbnSelchangeListSelect()
{
	...
	if ( pCROW )
	{
		...
		SetWin_Text( this, IDC_EDIT_SEL_NAME, _HLIB::cstringformat("[%03d/%03d] %s",
			m_SELECTED.wMainID, m_SELECTED.wSubID, pCROW->GetName() ).GetString());
	}
	else
	{
		SelectionReset();
