...
struct SGMTOOL_USER_NEW
{
	...
	TCHAR		szUserEmail[USR_EMAIL_LENGTH];
	TCHAR		szSecurityKey[USR_PASS_LENGTH];
	WORD		wUserType;
	WORD		wChaRemain;
	DWORD		dwUserPoint;
	DWORD		dwUserPoint2;
		...
		, dwUserPoint2(0)
	{
		...
		memset( szSecurityKey, 0, sizeof(char)*USR_PASS_LENGTH );
	};
};
...
struct SGMTOOL_INVENTORY_LOCK_DATA_SIMPLE
{
	DWORD	dwNum;
	DWORD	dwChaNum; 
	TCHAR	szChaName[CHR_ID_LENGTH+1];
	TCHAR	szEPin[CHAR_INVENTORY_LOCK_PASS_SIZE+1];
	__time64_t	tDate;

	SGMTOOL_INVENTORY_LOCK_DATA_SIMPLE ()
		: dwNum(0)
		, dwChaNum(0)
		, tDate(0)
	{
		memset( szChaName, 0, sizeof(char)*CHR_ID_LENGTH+1 );
		memset( szEPin, 0, sizeof(char)*CHAR_INVENTORY_LOCK_PASS_SIZE+1 );
	};
};

typedef std::map<DWORD, SGMTOOL_INVENTORY_LOCK_DATA_SIMPLE>	SGMTOOL_INVENTORY_LOCK_DATA_SIMPLE_MAP;
typedef SGMTOOL_INVENTORY_LOCK_DATA_SIMPLE_MAP::iterator	SGMTOOL_INVENTORY_LOCK_DATA_SIMPLE_MAP_ITER;

struct SGMTOOL_STORAGE_LOCK_DATA_SIMPLE
{
	DWORD	dwNum;
	DWORD	dwChaNum; 
	TCHAR	szChaName[CHR_ID_LENGTH+1];
	TCHAR	szEPin[CHAR_STORAGE_LOCK_PASS_SIZE+1];
	__time64_t	tDate;

	SGMTOOL_STORAGE_LOCK_DATA_SIMPLE ()
		: dwNum(0)
		, dwChaNum(0)
		, tDate(0)
	{
		memset( szChaName, 0, sizeof(char)*CHR_ID_LENGTH+1 );
		memset( szEPin, 0, sizeof(char)*CHAR_STORAGE_LOCK_PASS_SIZE+1 );
	};
};

typedef std::map<DWORD, SGMTOOL_STORAGE_LOCK_DATA_SIMPLE>	SGMTOOL_STORAGE_LOCK_DATA_SIMPLE_MAP;
typedef SGMTOOL_STORAGE_LOCK_DATA_SIMPLE_MAP::iterator		SGMTOOL_STORAGE_LOCK_DATA_SIMPLE_MAP_ITER;
