...
IMPLEMENT_DYNAMIC(CVehicleEditTab, CPropertySheet)

CVehicleEditTab::CVehicleEditTab( GLVEHICLE* pData, CRect& rect, LOGFONT logfont, CWnd* pParentWnd /*= NULL*/ )
	...
	m_PageColor ( logfont, pData ), /*bike color , Juver, 2017/11/13 */
	...
	m_pData( NULL )
{
	...
	m_PageColor.SetSheetTab ( this ); /*bike color , Juver, 2017/11/13 */

	AddPage ( &m_PageBasic );
	AddPage ( &m_PageInven ); 
	AddPage ( &m_PageColor ); /*bike color , Juver, 2017/11/13 */

	m_pFont = new CFont();
	m_pFont->CreateFontIndirect(&logfont);
}

CVehicleEditTab::~CVehicleEditTab()
{
	SAFE_DELETE( m_pFont );
}
...
BOOL CVehicleEditTab::OnInitDialog() 
{
	HWND hTabWnd;
	CRect rectTabCtrl;
...
BEGIN_MESSAGE_MAP(CVehicleEditTab, CPropertySheet)
	//{{AFX_MSG_MAP(CVehicleEditTab)
	ON_WM_CTLCOLOR_REFLECT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

HBRUSH CVehicleEditTab::CtlColor(CDC* pDc, UINT uCtlColor) 
{
...
BOOL CVehicleEditTab::PreTranslateMessage(MSG* pMsg)
{
	switch ( pMsg->message )
	{
	case WM_KEYDOWN:
...
void CVehicleEditTab::ChangeDialogFont(CWnd* pWnd, CFont* pFont, int nFlag)
{
	CRect windowRect;
...
void CVehicleEditTab::InitPages()
{
