...
void CNetClient::MsgRandomNum(NET_MSG_GENERIC* nmg)
{
	...
	//userpanel
	StringCchCopy( m_szRandomString, USR_CAPTCHA+1, pMsg->szRandomString );
	bRandomStringRcv = TRUE;
}
...
int CNetClient::SndGameJoin(int nChaNum)
{
 ...
 /*item link, Juver, 2017/08/01 */
int CNetClient::SndChatNormal (const char* szName, const char* szMsg, SITEMLINK* pItemLink )
{
	NET_CHAT nc;	
	...
	if ( pItemLink )
		nc.sItemLink = *pItemLink;

	return Send((char*) &nc);
}

/*item link, Juver, 2017/08/01 */
int CNetClient::SndChatParty ( const char* szMsg, SITEMLINK* pItemLink ) 
{
	NET_CHAT nc;
	nc.nmg.nType	= NET_MSG_CHAT;
...
/*item link, Juver, 2017/08/01 */
int CNetClient::SndChatPrivate( const char* szName, const char* szMsg, SITEMLINK* pItemLink ) 
{
	NET_CHAT nc;
	nc.nmg.nType	= NET_MSG_CHAT;
...
/*item link, Juver, 2017/08/01 */
int CNetClient::SndChatGuild( const char* szMsg, SITEMLINK* pItemLink ) 
{
	NET_CHAT nc;
	nc.nmg.nType	= NET_MSG_CHAT;
...
/*item link, Juver, 2017/08/01 */
int CNetClient::SndChatAlliance( const char* szMsg, SITEMLINK* pItemLink ) 
{
	NET_CHAT nc;
	nc.nmg.nType	= NET_MSG_CHAT;
...
int CNetClient::SndChatRegional( const char* szMsg, SITEMLINK* pItemLink ) 
{
	NET_CHAT nc;
	nc.nmg.nType	= NET_MSG_CHAT;
	nc.emType		= CHAT_TYPE_REGIONAL;
	::StringCchCopy(nc.szChatMsg, CHAT_MSG_SIZE+1, szMsg);
