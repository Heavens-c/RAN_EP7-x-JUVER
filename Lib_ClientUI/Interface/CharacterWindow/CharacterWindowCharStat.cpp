...
void CCharacterWindowCharStat::CreateSubControl ()
{
	...
	m_pButtonStatReset = CreateButtonBlue ( "CHARACTER_WINDOW_STAT_INIT_BUTTON", "CHARACTER_WINDOW_STAT_BUTTON_TEXT_POS2", pFont, nAlignCenterBoth, CHARACTER_STAT_RESET, ID2GAMEWORD("CHARACTER_STAT_INIT", 0) );
	m_pButtonStatApply = CreateButtonBlue ( "CHARACTER_WINDOW_STAT_APPLY_BUTTON", "CHARACTER_WINDOW_STAT_BUTTON_TEXT_POS2", pFont, nAlignCenterBoth, CHARACTER_STAT_APPLY, ID2GAMEWORD("CHARACTER_STAT_APPLY", 0) );
...
void CCharacterWindowCharStat::SetData( const GLCHARLOGIC& sCharData )
{
	...
	strCombine.Format ( "%d", sCharData.m_sMP.wNow );
	if ( m_pMPText ) m_pMPText->SetOneLineText ( strCombine, NS_UITEXTCOLOR::WHITE );

	strCombine.Format ( "%d", sCharData.m_sSP.wNow );
...
void CCharacterWindowCharStat::SetUpData( const GLCHARLOGIC& sCharData )
{
	...
	int nHP = m_sCharLogic.m_sHP.wMax - sCharData.m_sHP.wMax;
	int nMP = m_sCharLogic.m_sMP.wMax - sCharData.m_sMP.wMax;
	int nSP = m_sCharLogic.m_sSP.wMax - sCharData.m_sSP.wMax;
...
void CCharacterWindowCharStat::InitData()
{
	...
	if ( pChar )
	{
		...
		if ( pItem && pItem->sBasicOp.emItemType == ITEM_SUIT )
		{
			...
			case SUIT_EARRING:	
				{
					if ( pChar->GetCharLogic().VALID_SLOT_ITEM( SLOT_REARRING ) )	emSlot = SLOT_LEARRING;
					else	emSlot = SLOT_REARRING;
				}
				break;

			case SUIT_ACCESSORY:
				emSlot = SLOT_ACCESSORY;	
				break;
