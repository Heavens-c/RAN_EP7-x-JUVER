...
void CCharacterWindow::SetName ( const GLCHARLOGIC& sCharData )
{
	if ( m_pNameText )
	{
		...
		if ( RANPARAM::bFeatureActivity && strlen( sCharData.m_szBadge ) )
		{
...
void CCharacterWindow::SetContributionPoint ( const GLCHARLOGIC& sCharData )
{
	...
	if ( m_pContributionPoint ) m_pContributionPoint->SetOneLineText ( strCombine, NS_UITEXTCOLOR::WHITE );
}

/*activity point, Juver, 2017/08/23 */
void CCharacterWindow::SetActivitySystemPoint( const GLCHARLOGIC& sCharData )
{
	CString strCombine;
	strCombine.Format ( "%d", sCharData.m_dwActivityPoint );
	if ( m_pExtraActivityPoint ) m_pExtraActivityPoint->SetOneLineText ( strCombine, NS_UITEXTCOLOR::WHITE );
}

/*activity system, Juver, 2017/11/05 */
void CCharacterWindow::LoadBadge( const GLCHARLOGIC& sCharData )
{
...
void CCharacterWindow::LoadPKInfo( const GLCHARLOGIC& sCharData )
{
	CString strCombine;
	strCombine.Format ( "%d", sCharData.m_dwPKScore );
	if ( m_pPKScore ) m_pPKScore->SetOneLineText ( strCombine, NS_UITEXTCOLOR::GREENYELLOW );

	strCombine.Format ( "%d", sCharData.m_dwPKDeath );
	if ( m_pPKDeath ) m_pPKDeath->SetOneLineText ( strCombine, NS_UITEXTCOLOR::ORNAGERED );
}