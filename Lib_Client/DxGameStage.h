...
struct DxCharJoinData
{
	...
	bool CheckReceive ()
	{
		...
		/*itemfood system, Juver, 2017/05/25 */
		if ( !m_bReceiveItemFood )	return false; 

		/*activity system, Juver, 2017/10/30 */
		if ( m_dwNumActivityProg!=m_dwReceiveActivityProg )	return false;
		if ( m_dwNumActivityDone!=m_dwReceiveActivityDone )	return false;
