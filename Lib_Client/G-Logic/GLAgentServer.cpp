	...
	m_dwFIELDSVR_NUM(0)
{
	...
	m_vecTopKillBr.clear();
	m_vecTopKillSw.clear();
	m_vecTopKillAr.clear();
	m_vecTopKillSh.clear();
	m_vecTopKillEx.clear();
	m_vecTopKillSc.clear();
	m_vecTopKillAs.clear();
...
HRESULT GLAgentServer::Create ( DWORD dwMaxClient, DxMsgServer *pMsgServer, DxConsoleMsg *pConsoleMsg, GLDBMan *pDBMan, char* szMapList, F_SERVER_INFO* pFieldInfo )
{
	...
	if ( !szMapList )
	{
		hr = m_sMapList.LoadMapsListFile ( "mapslist.mst", pFieldInfo, FIELDSERVER_MAX );
		if ( FAILED(hr) )	return E_FAIL;
	}
	else
	{
		hr = m_sMapList.LoadMapsListFile ( szMapList, pFieldInfo, FIELDSERVER_MAX );
		if ( FAILED(hr) )	return E_FAIL;
	}
	...
	for ( WORD i=0; i<GLCONST_CHAR::wSCHOOLNUM; ++i )
	{
		...
		if ( !pLandMan )
		{
			...
			TCHAR szTemp[128] = {0};
			_snprintf( szTemp, 128, "Charactor Start Map Error.\n"
									"'mapslist.mst' [%u,%u] MapID ( ID:[M,S] ) not found.",
									nidSTARTMAP.wMainID,
									nidSTARTMAP.wSubID );
	...
	/*pvp tyranny, Juver, 2017/08/24 */
	GLPVPTyrannyAgent::GetInstance().SetMapState();

	/*school wars, Juver, 2018/01/19 */
	GLPVPSchoolWarsAgent::GetInstance().SetMapState();
	...
	// LG-7 GlobalRanking
	m_vecTopKill.clear();
	m_pDBMan->GetTopKill( m_vecTopKill);
	
	/*re-coded LG-7 Global Ranking by DevArt22*/
	m_vecTopKillBr.clear();
	m_pDBMan->GetTopKillBr( m_vecTopKillBr);
	
	/*re-coded LG-7 Global Ranking by DevArt22*/
	m_vecTopKillSw.clear();
	m_pDBMan->GetTopKillSw( m_vecTopKillSw);
	
	/*re-coded LG-7 Global Ranking by DevArt22*/
	m_vecTopKillAr.clear();
	m_pDBMan->GetTopKillAr( m_vecTopKillAr);
	
	/*re-coded LG-7 Global Ranking by DevArt22*/
	m_vecTopKillSh.clear();
	m_pDBMan->GetTopKillSh( m_vecTopKillSh);

	/*re-coded LG-7 Global Ranking by DevArt22*/
	m_vecTopKillEx.clear();
	m_pDBMan->GetTopKillEx( m_vecTopKillEx);

	/*re-coded LG-7 Global Ranking by DevArt22*/
	m_vecTopKillSc.clear();
	m_pDBMan->GetTopKillSc( m_vecTopKillSc);

	/*re-coded LG-7 Global Ranking by DevArt22*/
	m_vecTopKillAs.clear();
	m_pDBMan->GetTopKillAs( m_vecTopKillAs);
	
	m_vecTopRich.clear();
	m_pDBMan->GetTopRich( m_vecTopRich );
	
	m_vecTopGuild.clear();
	//m_pDBMan->GetTopGuild( m_vecTopGuild );
	/////////////////////////////////////////////////////////////////////////////
...
//	if( !m_pInstantMapLandMan[iMapNum] ) return E_FAIL;
	...
	BOOL bInsert = m_sMapList.InsertMapList( smap_node );
	...
	if ( bInsert )
	{
		...
		*pSrcLandMan = *pDestLandMan;
		...
		CDebugSet::ToFileWithTime( "instancemap.txt", "[INFO AGENT]InstanceMap insert to mapslist[%d~%d]", 
...
BOOL GLAgentServer::ClearReservedDropOutPC ()
{
	EnterCriticalSection(&m_CSPCLock);
	{
		...
		for ( ; iter!=iter_end; ++iter )
		{
			...
			/*pvp tyranny, Juver, 2017/08/24 */
			GLPVPTyrannyAgent::GetInstance().RequestPlayerCancelRegister( pChar->m_dwCharID );

			/*school wars, Juver, 2018/01/19 */
			GLPVPSchoolWarsAgent::GetInstance().RequestPlayerCancelRegister( pChar->m_dwCharID );

			/*pvp capture the flag, Juver, 2018/01/28 */
			GLPVPCaptureTheFlagAgent::GetInstance().RequestPlayerCancelRegister( pChar->m_dwCharID );
										...
										const DWORD* pMBR_A, const DWORD* pMBR_B, const D3DXVECTOR3 vPos)
{
	...
	if ( sOPT_A.bSCHOOL )
	{
		...
		if ( szMAP_NAME )
		{
			...
			GLMSG::SNET_EMCONFRONT_START_PARTY NetMsgStartParty;
			StringCchCopy ( NetMsgStartParty.szMapName, CHAR_SZNAME, szMAP_NAME );	
			StringCchCopy ( NetMsgStartParty.szGuild1, CHAR_SZNAME, GLCONST_CHAR::GETSCHOOLNAME(wSCHOOL_A) );	
			StringCchCopy ( NetMsgStartParty.szGuild2, CHAR_SZNAME, GLCONST_CHAR::GETSCHOOLNAME(wSCHOOL_B) );	
			SENDTOALLCLIENT ( &NetMsgStartParty );
		}
...
HRESULT GLAgentServer::StartClubBattle( DWORD dwClubID, GLCLUBBATTLE sClubBattle )
{
	...
	if ( sClubBattle.m_bAlliance )
	{
		...
		GLMSG::SNET_ALLIANCE_BATTLE_BEGIN NetMsg;
		StringCchCopy ( NetMsg.szParty1, CHAR_SZNAME, pClubP->m_szName );	
		StringCchCopy ( NetMsg.szParty2, CHAR_SZNAME, pClubS->m_szName );	
	...
	{	
		...
		GLMSG::SNET_CLUB_BATTLE_BEGIN NetMsg;
		StringCchCopy ( NetMsg.szGuild1, CHAR_SZNAME, pClubP->m_szName );	
		StringCchCopy ( NetMsg.szGuild2, CHAR_SZNAME, pClubS->m_szName );	
		GLAgentServer::GetInstance().SENDTOALLCLIENT ( &NetMsg );
	}	
