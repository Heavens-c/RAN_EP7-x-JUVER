...
BOOL CreateFolder(PCTSTR pszFolder)
{
	...
	_tcsncpy(szFolderPath, pszFolder, pszOldSegment - pszFolder);
	while (*pszOldSegment)
	{
		PCTSTR pszSegment = PathFindNextComponent(pszOldSegment);
		_tcsncat(szFolderPath, pszOldSegment, pszSegment - pszOldSegment);
...
{
	TCHAR szFolderPath[MAX_PATH];
	_tcscpy(szFolderPath, pszFileName);
 ...
 * @return true if operation was completed successfully.
 */
BOOL CreateTempFolder(PTSTR pszTempPath)
{
	...
	for (;;)
	{
		_stprintf(pszTempPath, _T("%s\\TEMP%lu"), szSysTempPath, dwTicks);
...
DWORD GetCanonicalAppName(PTSTR pszAppName, DWORD dwBufferSize, BOOL bAllowSpaces)
{
	...
	else
	{
		...
		_tcscpy(pszAppName, pszFileName);
...
BOOL GetCompleteLogFileName(PTSTR pszCompleteLogFileName, PCTSTR pszLogFileName, PCTSTR pszDefFileExtension)
{
	...
	{
		_tcscpy(pszCompleteLogFileName, pszLogFileName);
		return TRUE;
	}
