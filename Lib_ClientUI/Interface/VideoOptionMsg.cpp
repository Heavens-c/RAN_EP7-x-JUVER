...
void CBasicVideoOption::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	case HWOPTION_VIDEO_DEFAULT_COMBO_OPEN:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( dwMsg & UIMSG_LB_DOWN )
				{
					...
					m_pComboBoxTextureFilterRollOver->SetVisibleSingle ( FALSE );
	...
	case HWOPTION_VIDEO_TEXTUREFILTER_COMBO_OPEN:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( dwMsg & UIMSG_LB_DOWN )
				{
					...
					m_pComboBoxCharDetailRollOver->SetVisibleSingle ( FALSE );					
					m_pComboBoxSightRollOver->SetVisibleSingle ( FALSE );
					m_pComboBoxTextureFilterRollOver->SetVisibleSingle ( TRUE );
					m_pComboBoxTextureFilterRollOver->SetScrollPercent ( 0.0f );

					m_RollOverID = HWOPTION_VIDEO_TEXTUREFILTER_COMBO_ROLLOVER;
					m_bFirstLBUP = TRUE;
				}
			}
		}
	...
	case HWOPTION_VIDEO_TEXTUREFILTER_COMBO_ROLLOVER:
		{
			if ( dwMsg & UI_MSG_COMBOBOX_ROLLOVER_SCROLL )
			{				
				DWORD dwMsg = GetMessageEx ();
			...
			if ( CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{		
				int nIndex = m_pComboBoxTextureFilterRollOver->GetSelectIndex ();
				if ( nIndex < 0 ) return ;
				m_dwTextureFilter = (DWORD)nIndex;
