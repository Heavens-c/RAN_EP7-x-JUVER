...
BOOL CDlgUserNew::OnInitDialog()
{
	...
	SetWin_Num_int( this, IDC_USERNEW_EDIT_USERPOINT2, 0 );
...
void CDlgUserNew::OnBnClickedOk()
{
	...
	int nUserPoints2 = GetWin_Num_int( this, IDC_USERNEW_EDIT_USERPOINT2 );
	...
	if ( nUserPoints < 0 ){
		CDebugSet::MsgBoxAfx( "E Points invalid" );
		GotoDlgCtrl(GetDlgItem(IDC_USERNEW_EDIT_USERPOINT)); 
		return;
	}

	if ( nUserPoints2 < 0 ){
		CDebugSet::MsgBoxAfx( "GT Points invalid" );
		GotoDlgCtrl(GetDlgItem(IDC_USERNEW_EDIT_USERPOINT2)); 
		return;
	}
	...
	sDATA.dwUserPoint2 = (WORD)nUserPoints2;
