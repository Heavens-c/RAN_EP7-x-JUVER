...
GLGaeaServer::GLGaeaServer(void) :
	...
	m_bPlayerReqRank(false),
	m_fPlayerReqTimer(0.0f),
	...
	m_nRandomNumber(0), //userpanel
	m_dwInstantMapNum(0),
	m_fAutoPotionDelay(0.5f)
{
...
PGLPETFIELD GLGaeaServer::CreatePET ( PGLPET pPetData, DWORD dwOwner, DWORD dwPetID, bool bValid )
{
	...
	if ( pPet->IsValid () )
	{
		...
		if ( pPet->m_sActiveSkillID_A != NATIVEID_NULL() )
			pPet->ChangePetSKill_A( pPet->m_sActiveSkillID_A );

		/*enable pet skill at pet creation, Juver, 2017/12/13 */
		/*dual pet skill, Juver, 2017/12/27 */
		if ( pPet->m_sActiveSkillID_B != NATIVEID_NULL() )
			pPet->ChangePetSKill_B( pPet->m_sActiveSkillID_B );
...
HRESULT GLGaeaServer::FrameMove ( float fTime, float fElapsedTime )
{
	...
	//dmk14 | 11-4-16 | pk ranking update timer
	if( m_bPlayerReqRank )
	{
		m_fPlayerReqTimer += fElapsedTime;
		if( m_fPlayerReqTimer >= 1800.0f )	{ m_bPlayerReqRank = false; m_fPlayerReqTimer = 0.0f; }
	}
...
BOOL GLGaeaServer::ReqZoomOutCheatDetected( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_ZOOMOUT_CHEAT_DETECTED* pNetMsg ){
	PGLCHAR pMyChar = GetChar( dwGaeaID );

	if( !pMyChar ) return FALSE;

	if( pNetMsg->fZoomOutValue > 200.0f ){

		TCHAR szMsg[MAX_PATH] = {0};
		StringCchPrintf( szMsg, MAX_PATH, _T("%f"), pNetMsg->fZoomOutValue);
		...
		DropMsg.dwCHARID  = pMyChar->m_dwCharID;
		GLGaeaServer::GetInstance().SENDTOAGENT(dwClientID, &DropMsg);
...
BOOL GLGaeaServer::ReqActiveVehicleConfirm ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_ACTIVE_VEHICLE_CONFIRM* pNetMsg ){
	PGLCHAR pOwner = GetChar ( dwGaeaID );
	if ( !pOwner ) return FALSE;
	if( pNetMsg->bActive )
		pOwner->m_bHoverConfirm = TRUE;
	else 
		pOwner->m_bHoverConfirm = FALSE;
...
void GLGaeaServer::GerberSecDetSpdHack ( DWORD dwClientID, DWORD dwGaeaID )
{
	PGLCHAR pChar = GetChar ( dwGaeaID );
	if ( !pChar ) return;

	CDebugSet::ToFileWithTime( "_speedhack_detect.txt", "[%d]%s Speedhack Detected", pChar->m_dwCharID, pChar->m_szName );

	CString strTextGen = "";
	GLMSG::SNET_SERVER_GENERALCHAT NetMsgGen;
	strTextGen.Format( "%s has been banned due to speedhack attempt.", pChar->m_szName );
	...
	GLITEMLMT::GetInstance().ReqAction(pChar->m_dwCharID,EMLOGACT_CHEATER_BAN,ID_CHAR, 1,0,0,0,0);

	GLMSG::SNETPC_DROPCHAR_TOAGENT DropMsg;
	DropMsg.dwCHARID  = pChar->m_dwCharID;
	GLGaeaServer::GetInstance().SENDTOAGENT(dwClientID, &DropMsg);
	return;
}
...
void GLGaeaServer::SetDisTrade ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_DISTRADE_REQ* pNetMsg )
{
	PGLCHAR pChar = GetChar ( dwGaeaID );
	if ( !pChar ) return;

	pChar->m_bDisTrade = pNetMsg->bDisTrade;
}
...
PGLSUMMONFIELD GLGaeaServer::CreateSummon ( SUMMON_DATA_SKILL sdata, DWORD dwOwnerGaeaId )
{
	...
	PGLSUMMON psummon_base_data = GLCONST_SUMMON::GetSummonData ( psummon_crow_data->m_emSummonType );
	...
	for( int i=0; i<SKILL_SUMMON_MAX_CLIENT_NUM; ++i )
	{
		...
		if ( psummon_crow_data->m_emSummonType == psummon_active->m_emTYPE )	nSummonActiveType ++;
		if ( sdata.sidSkill == psummon_active->m_Summon.sidSkill )				nSummonActiveSkill ++;

		if ( nSummonActiveType >= GLCONST_SUMMON::GetMaxSummon( psummon_crow_data->m_emSummonType ) )		bMaxedType = TRUE;
	...
	if ( bMaxedType )
	{
		...
		for( int i=0; i<SKILL_SUMMON_MAX_CLIENT_NUM; ++i )
		{
			...
			if ( psummon_active->m_emTYPE != psummon_crow_data->m_emSummonType )	continue;
	...
	GLSUMMON NewSummon(*psummon_base_data);
...
BOOL GLGaeaServer::DropSummon ( PGLSUMMONFIELD pSummon, SNATIVEID sMapID )
{
	...
	// 글로벌에 등록
	//summon array overflow control
	if ( pSummon->m_dwGUID>=m_dwMaxClient )	return FALSE;
...
//dmk14 | 11-4-16 | pk ranking
BOOL GLGaeaServer::ReqPlayerRanking ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_MSG_REQ_PLAYERRANKING* pNetMsg )
{
	if ( m_bEmulator )	return FALSE;

	PGLCHAR pChar = GetChar ( dwGaeaID );
	if ( !pChar ) return FALSE;	
	if ( !pChar->m_pLandMan ) return FALSE;
	if ( pChar->m_pLandMan->GetMapID().dwID != pNetMsg->dwMapID ) return FALSE;
//	if( IsTowerWarsStarted() || IsClubDMStarted() || IsClubBattleStarted () || IsSchoolWarsStarted() || IsRoyalRumbleStarted() )		return FALSE;

	if( !m_bPlayerReqRank )
	{
		m_mapPlayerRanking.clear();
		if( m_pDBMan ) m_pDBMan->GetPlayerRanking( m_mapPlayerRanking );

		PLAYER_RANKING_STATUS_VEC m_vecPRankNew;
		m_vecPRankNew.reserve( m_mapPlayerRanking.size() );

		PLAYER_RANKING_STATUS_MAP_ITER	pos = m_mapPlayerRanking.begin();
		PLAYER_RANKING_STATUS_MAP_ITER	end = m_mapPlayerRanking.end();

		for ( ; pos != end; pos++ )
		{
			const PLAYER_RANKING&	sRankInfo = pos->second;					
			m_vecPRankNew.push_back( sRankInfo );
		}
		
		std::sort( m_vecPRankNew.begin(), m_vecPRankNew.end() );	

		int nSize = m_vecPRankNew.size();

		for ( int i = 0; i < nSize; ++i )
		{
			m_vecPRankNew[i].wCharRanking = i+1;
			m_vecPRankNew[i].nIndex = i;
		}

		for ( int i = nSize-1; i > 0; --i )
		{
			if ( m_vecPRankNew[i] == m_vecPRankNew[i-1] )
			{
				m_vecPRankNew[i-1].wCharRanking = m_vecPRankNew[i].wCharRanking;			
			}
		}	

		GLMSG::SNET_MSG_REQ_PLAYERRANKING_UPDATE	NetMsg;
		GLMSG::SNET_MSG_REQ_PLAYERRANKING_UPDATE_EX NetMsgMy;

		bool bMyRank = false;
		bool bTop = false;
		for ( int i = 0; i < m_vecPRankNew.size(); ++i )
		{
			PLAYER_RANKING_STATUS_MAP_ITER iter = m_mapPlayerRanking.find( m_vecPRankNew[i].dwCharID );
			if ( iter == m_mapPlayerRanking.end() )	continue;

			if( pChar->m_dwCharID == m_vecPRankNew[i].dwCharID && !bMyRank ) 
			{
				NetMsgMy.sMyPKRank.wCharRanking = m_vecPRankNew[i].wCharRanking;
				NetMsgMy.sMyPKRank.wKillNum = m_vecPRankNew[i].wKillNum;
				NetMsgMy.sMyPKRank.wDeathNum = m_vecPRankNew[i].wDeathNum;
				NetMsgMy.sMyPKRank.dwClass = m_vecPRankNew[i].dwClass;
				NetMsgMy.sMyPKRank.wSchool = m_vecPRankNew[i].wSchool;
				StringCchCopy ( NetMsgMy.sMyPKRank.szCharName, CHAR_NAME+1, m_vecPRankNew[i].szCharName );	

				bMyRank = true;
			}

			PLAYER_RANKING&	sRankInfo = iter->second;
			
			if ( m_vecPRankNew[i].nIndex != sRankInfo.nIndex || 
				m_vecPRankNew[i].wCharRanking != sRankInfo.wCharRanking )
			{
				sRankInfo.wCharRanking = m_vecPRankNew[i].wCharRanking;
				sRankInfo.nIndex = m_vecPRankNew[i].nIndex;
				sRankInfo.dwCharID = m_vecPRankNew[i].dwCharID;
				sRankInfo.dwClass = m_vecPRankNew[i].dwClass;
				sRankInfo.wKillNum = m_vecPRankNew[i].wKillNum;
				sRankInfo.wDeathNum = m_vecPRankNew[i].wDeathNum;
				sRankInfo.wSchool = m_vecPRankNew[i].wSchool;
				StringCchCopy ( sRankInfo.szCharName, CHAR_NAME+1, m_vecPRankNew[i].szCharName );

				if ( m_vecPRankNew[i].nIndex <= RANKING_PKNUM ) 
				{
					PLAYER_RANKING sSwPRank = sRankInfo;
					NetMsg.ADDCHAR( sSwPRank );
					//if ( m_pConsoleMsg )	m_pConsoleMsg->Write("ADDCHAR Rank: %d, Name: %s(%d), Win(%d), Lose(%d) KD:%1.1f", sSwPRank.wCharRanking, sSwPRank.szCharName, sSwPRank.dwCharID,sSwPRank.wKillNum,sSwPRank.wDeathNum,float(sSwPRank.wKillNum/max(1,sSwPRank.wDeathNum)) );
				}			

				if( NetMsg.wRankNum == RANKING_PKNUM )	bTop = true;
			}		

			//stop when all the data is fucking complete to lessen the process
			if( bMyRank && bTop )	break;
		}
		...
		m_bPlayerReqRank = true;
		if ( m_pConsoleMsg )	m_pConsoleMsg->Write("PK Ranking updated!");
	}
	else
	{
		PLAYER_RANKING_STATUS_MAP_ITER pos = m_mapPlayerRanking.find( pChar->m_dwCharID );

		if( pos != m_mapPlayerRanking.end() ) 
		{
			GLMSG::SNET_MSG_REQ_PLAYERRANKING_UPDATE_EX NetMsgMy;

			NetMsgMy.sMyPKRank.wCharRanking = pos->second.wCharRanking;
			NetMsgMy.sMyPKRank.wKillNum = pos->second.wKillNum;
			NetMsgMy.sMyPKRank.wDeathNum = pos->second.wDeathNum;
			NetMsgMy.sMyPKRank.dwClass = pos->second.dwClass;
			NetMsgMy.sMyPKRank.wSchool = pos->second.wSchool;
			StringCchCopy ( NetMsgMy.sMyPKRank.szCharName, CHAR_NAME+1, pos->second.szCharName );	
		...
		PLAYER_RANKING_STATUS_MAP_ITER pos_begin = m_mapPlayerRanking.begin();
		PLAYER_RANKING_STATUS_MAP_ITER pos_end = m_mapPlayerRanking.end();

		for ( ; pos_begin != pos_end; pos_begin++ )
		{
			if( NetMsg.wRankNum == RANKING_PKNUM )	break;

			PLAYER_RANKING& sSwPRankInfo = pos_begin->second;
			int nIndex = sSwPRankInfo.nIndex;

			if ( nIndex >= 0 && nIndex < RANKING_PKNUM )
			{
				PLAYER_RANKING sSwPRank = sSwPRankInfo;
				NetMsg.ADDCHAR( sSwPRank );
			}
		}	
...
void GLGaeaServer::PkRankAdd( DWORD dwKilled, DWORD dwKiller )
{
	PGLCHAR pCharKilled = GetCharID( dwKilled );
	PGLCHAR pCharKiller = GetCharID( dwKiller );

	if ( !pCharKilled )	return;
	if ( !pCharKiller )	return;

	//add pk history
	SPK_HISTORY sPKHistory;
	sPKHistory.dwKilled = dwKilled;
	sPKHistory.dwKiller = dwKiller;
	sPKHistory.cClassKilled = (BYTE)CharClassToIndex( pCharKilled->m_emClass );
	sPKHistory.cClassKiller = (BYTE)CharClassToIndex( pCharKiller->m_emClass );
	sPKHistory.cSchoolKilled = (BYTE)pCharKilled->m_wSchool;
	sPKHistory.cSchoolKiller = (BYTE)pCharKiller->m_wSchool;
	StringCchCopy ( sPKHistory.szCharKilled, CHAR_SZNAME+1, pCharKilled->m_szName );
	StringCchCopy ( sPKHistory.szCharKiller, CHAR_SZNAME+1, pCharKiller->m_szName );

	//update client side history
	GLMSG::SNET_PK_RANK_HISTORY_UPDATE	NetMsgBroadCast;
	NetMsgBroadCast.sPKHistory = sPKHistory;
	SENDTOCLIENT_ONMAP( pCharKiller->m_sMapID.dwID, &NetMsgBroadCast );
	//SENDTOALLCLIENT(&NetMsgBroadCast);
}