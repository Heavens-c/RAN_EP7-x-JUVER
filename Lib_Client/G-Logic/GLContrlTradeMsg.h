...
namespace GLMSG
{
	...
	struct SNET_TRADE_LOCK
	{
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_TRADE_LOCK;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*trade lock, Juver, 2018/01/02 */
	struct SNET_TRADE_LOCK_TAR
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwGaeaID;
		EMTRADE_LOCK_FB		emFB;

		SNET_TRADE_LOCK_TAR () :
		dwGaeaID(0),
			emFB(EMTRADE_LOCK_OK)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_TRADE_LOCK_TAR;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
