...
{
	_tcscpy(m_szErrorCode, rErrorInfo.m_szErrorCode);
	if (rErrorInfo.m_pszMessageBuffer)
	{
		...
		if (m_pszMessageBuffer)
			_tcscpy(m_pszMessageBuffer, rErrorInfo.m_pszMessageBuffer);
	}
	else
...
BOOL CSymEngine::WriteDxDialog(PCTSTR pszFileName)
{
	STARTUPINFO si;
	...
	wsprintf(szCmdLine, "dxdiag.exe /t %s", pszFileName );
...
void CSymEngine::GetSysErrorInfo(CSysErrorInfo& rErrorInfo)
{
	if (m_dwLastError)
	{
		_stprintf(rErrorInfo.m_szErrorCode, _T("0x%08lX"), m_dwLastError);
...
BOOL CSymEngine::GetErrorInfo(CErrorInfo& rErrorInfo)
{
	...
	_ultot(GetCurrentProcessId(), rErrorInfo.m_szProcessID, 10);
	DWORD dwException;
	switch (m_eExceptionType)
	{
...
#define CONVERT_REGISTER_VALUE_TO_STRING(reg, digits) \
	_stprintf(rRegVals.m_sz##reg, _T("0x%0") _T(#digits) _T("lX"), m_pExceptionPointers->ContextRecord->reg);
	...
	if (g_bWinNT)
	{
		...
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, szCentralProcessorPath, 0l, KEY_READ, &hKey) == ERROR_SUCCESS)
		{
			...
			if (RegQueryValueEx(hKey, _T("~MHz"), NULL, &dwValueType, (PBYTE)&dwValue, &dwValueSize) == ERROR_SUCCESS && dwValueType == REG_DWORD)
				_ultot(dwValue, rCpuInfo.m_szCpuSpeed, 10);
			else
...
void CSymEngine::GetCpuString(CStrStream& rStream)
{
	...
	TCHAR szTempBuf[16];
	_ultot(CpusInfo.m_dwNumCpus, szTempBuf, 10);
	rStream << szTempBuf;
	...
	for (DWORD dwCpuNum = 0; dwCpuNum < CpusInfo.m_dwNumCpus; ++dwCpuNum)
	{
		...
		{
			rStream << _T("\r\n");
			_ultot(dwCpuNum + 1, szTempBuf, 10);
			rStream << szTempBuf;
			rStream << _T(". ");
 ...
 */
void CSymEngine::GetOsString(PTSTR pszOSString)
{
	COsInfo OsInfo;
	GetOsInfo(OsInfo);
#ifdef _MANAGED
	_stprintf(pszOSString, dwOSStringSize,
	            _T("OS Version:    %s %s\r\n")
	            _T("Build Number:  %s\r\n")
				_T("CLR Version:   %s"),
...
void CSymEngine::GetMemInfo(CMemInfo& rMemInfo)
{
	...
	_ultot(ms.dwTotalPhys, rMemInfo.m_szTotalPhys, 10);
	_ultot(ms.dwAvailPhys, rMemInfo.m_szAvailPhys, 10);
	_ultot(ms.dwTotalPageFile, rMemInfo.m_szTotalPageFile, 10);
	_ultot(ms.dwAvailPageFile, rMemInfo.m_szAvailPageFile, 10);
}
 ...
 */
void CSymEngine::GetMemString(PTSTR pszMemString)
{
	MEMORYSTATUS ms;
	GlobalMemoryStatus(&ms);
	_stprintf(pszMemString,
	...
	if (pchEnvironment)
	{
		...
		while (*pchEnvironmentPair)
		{
			...
			if (pchEquation)
			{
				...
				TCHAR szVariableName[MAX_PATH] = {0};
				_tcsncpy(szVariableName, pchEnvironmentPair, pchEquation - pchEnvironmentPair);
...
void CSymEngine::GetAssemblyList(CXmlWriter& rXmlWriter)
{
	...
	_ultot(dwAppDomainID, szAppDomainID, countof(szAppDomainID), 10);
...
void CSymEngine::GetModuleList(CUTF8EncStream& rEncStream, CEnumProcess* pEnumProcess, CEnumProcess::CProcessEntry& rProcEntry)
{
	...
	CHAR szTempBuf[64];
	_ultoa(rProcEntry.m_dwProcessID, szTempBuf, 10);
	...
	if (pEnumProcess->GetModuleFirst(rProcEntry.m_dwProcessID, ModuleEntry))
	{
		...
		do
		{
			...
			sprintf(szTempBuf, "%08lX", (DWORD)ModuleEntry.m_pLoadBase);
			rEncStream.WriteAscii(szTempBuf);
			rEncStream.WriteAscii(szSizeMsg);
			sprintf(szTempBuf, "%08lX", (DWORD)ModuleEntry.m_dwModuleSize);
			rEncStream.WriteAscii(szTempBuf);
			rEncStream.WriteAscii(szNewLine);
		}
...
void CSymEngine::GetModuleList(CXmlWriter& rXmlWriter, CEnumProcess* pEnumProcess, CEnumProcess::CProcessEntry& rProcEntry)
{
	...
	rXmlWriter.WriteStartElement(_T("process")); // <process>
	 ...
	 TCHAR szTempBuf[64];
	 _ultot(rProcEntry.m_dwProcessID, szTempBuf, 10);
	 ...
	 while (bContinue)
	 {
		 rXmlWriter.WriteStartElement(_T("module")); // <module>
		  ...
		  _stprintf(szTempBuf, _T("0x%08lX"), (DWORD)ModuleEntry.m_pLoadBase);
		  rXmlWriter.WriteElementString(_T("base"), szTempBuf); // <base>...</base>
		  _stprintf(szTempBuf, _T("0x%08lX"), (DWORD)ModuleEntry.m_dwModuleSize);
...
void CSymEngine::GetWin32StackTrace(CUTF8EncStream& rEncStream, DWORD dwThreadID, HANDLE hThread, PCSTR pszThreadStatus)
{
	...
	CHAR szThreadID[32];
	_ultoa(dwThreadID, szThreadID, 10);
...
void CSymEngine::GetWin32StackTrace(CXmlWriter& rXmlWriter, DWORD dwThreadID, HANDLE hThread, PCTSTR pszThreadStatus)
{
	rXmlWriter.WriteStartElement(_T("thread")); // <thread>
	 ...
	 TCHAR szThreadID[32];
	 _ultot(dwThreadID, szThreadID, 10);
...
void CSymEngine::GetNetStackTrace(CXmlWriter& rXmlWriter)
{
	...
	NetThunks::GetThreadInfo(dwThreadID, szThreadName, countof(szThreadName));
	 ...
	 TCHAR szThreadID[32];
	 _ultot(dwThreadID, szThreadID, countof(szThreadID), 10);
	 rXmlWriter.WriteElementString(_T("id"), szThreadID); // <id>...</id>
	 rXmlWriter.WriteElementString(_T("status"), pszThreadStatus); // <status>...</status>
	 rXmlWriter.WriteStartElement(_T("stack")); // <stack>
 ...
 */
void CSymEngine::GetTimeStamp(PTSTR pszTimeStamp)
{
	FILETIME ftime;
	...
	_ui64tot(uint.QuadPart, pszTimeStamp, 10);
}
 ...
 */
void CSymEngine::GetErrorInfo(CXmlWriter& rXmlWriter)
{
#ifdef _MANAGED
	if (IsNetException())
	{
...
void CSymEngine::GetCpusInfo(CXmlWriter& rXmlWriter)
{
	...
	rXmlWriter.WriteStartElement(_T("cpus")); // <cpus>
	 TCHAR szNumCpus[16];
	 _ultot(CpusInfo.m_dwNumCpus, szNumCpus, 10);
...
BOOL CSymEngine::WriteReportFiles(PCTSTR pszFolderName, CEnumProcess* pEnumProcess)
{
	...
	_stprintf(szLogFileName, _T("errorlog.%s"), pszLogExtension);
	...
	TCHAR szDxDialogFileName[MAX_PATH];
	TCHAR szDxDialogFullName[MAX_PATH];
	_stprintf(szDxDialogFileName, _T("DxDialog.TXT"));
	PathCombine(szDxDialogFullName, pszFolderName, szDxDialogFileName);
	if ( !WriteDxDialog( szDxDialogFullName ) )
		return FALSE;
...
void CSymEngine::GetReportFileName(PCTSTR pszExtension, PTSTR pszFileName, DWORD dwBufferSize) const
{
	...
	_stprintf(pszFileName + dwFileNameLen, 
...
#else
		_tcscpy(rEntry.m_szFunctionName, pSymbol->Name);
#endif
		if (dwDisplacement64)
		{
			_ui64tot(dwDisplacement64, rEntry.m_szFunctionOffset, 10);
			_stprintf(rEntry.m_szFunctionInfo, _T("%s()+%s byte(s)"), rEntry.m_szFunctionName, rEntry.m_szFunctionOffset);
		}
		else
		{
			*rEntry.m_szFunctionOffset = _T('\0');
			_stprintf(rEntry.m_szFunctionInfo, _T("%s()"), rEntry.m_szFunctionName);
		}
	}
	else
	{
...
#else
		_tcscpy(rEntry.m_szSourceFile, il.FileName);
#endif
		_ultot(il.LineNumber, rEntry.m_szLineNumber, 10);
		if (dwDisplacement32)
		{
			_ultot(dwDisplacement32, rEntry.m_szLineOffset, 10);
			_stprintf(rEntry.m_szLineInfo, _T("line %s+%s byte(s)"), rEntry.m_szLineNumber, rEntry.m_szLineOffset);
		}
		else
		{
			*rEntry.m_szLineOffset = _T('\0');
			_stprintf(rEntry.m_szLineInfo, _T("line %s"), rEntry.m_szLineNumber);
		}
	}
	else
	{
