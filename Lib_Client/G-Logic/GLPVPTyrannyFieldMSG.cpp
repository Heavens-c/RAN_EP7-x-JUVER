...
HRESULT GLPVPTyrannyField::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	if ( !nmg ){
		return E_FAIL;
	}
	...
	case NET_MSG_GCTRL_TYRANNY_A2FC_STATE_BATTLE:
		{
			...
			if( pnetmsg->wPlayerNum[TYRANNY_SCHOOL_SG] != (WORD)m_mapPlayerDataSG.size() ||
				pnetmsg->wPlayerNum[TYRANNY_SCHOOL_MP] != (WORD)m_mapPlayerDataMP.size() ||
				 pnetmsg->wPlayerNum[TYRANNY_SCHOOL_PHX] != (WORD)m_mapPlayerDataPHX.size() )
			{
				CDebugSet::ToFileWithTime( "_pvptyranny.txt", "GLPVPTyrannyField Receive Player size error Agent:%d~%d~%d Field:%d~%d~%d", 
			...
			CDebugSet::ToFileWithTime( "_pvptyranny.txt", "GLPVPTyrannyField Receive Player Data Num:%d~%d~%d", (WORD)m_mapPlayerDataSG.size(), (WORD)m_mapPlayerDataMP.size(), (WORD)m_mapPlayerDataPHX.size() );
		}break;
	...
	case NET_MSG_GCTRL_TYRANNY_A2F_PLAYER_DATA:
		{
			GLMSG::SNETPC_TYRANNY_A2F_PLAYER_DATA* pnetmsg = ( GLMSG::SNETPC_TYRANNY_A2F_PLAYER_DATA* ) nmg;
	...
	case NET_MSG_GCTRL_TYRANNY_A2F_TOWER_CAPTURE:
		{
			GLMSG::SNETPC_TYRANNY_A2F_TOWER_CAPTURE* pnetmsg = ( GLMSG::SNETPC_TYRANNY_A2F_TOWER_CAPTURE* ) nmg;

			if ( pnetmsg->wTowerIndex < TYRANNY_TOWER_SIZE &&
				pnetmsg->wTowerSchool < TYRANNY_SCHOOL_SIZE )
			{
				m_sTowerData[pnetmsg->wTowerIndex].wOwner = pnetmsg->wTowerSchool;
			}

			GLMSG::SNETPC_TYRANNY_F2C_TOWER_CAPTURE netmsgclient;
			netmsgclient.wTowerIndex = pnetmsg->wTowerIndex;
			netmsgclient.wTowerSchool = pnetmsg->wTowerSchool;
			netmsgclient.wTowerScore = pnetmsg->wTowerScore;
			GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP( m_sMap.dwID, &netmsgclient );
		}break;

	case NET_MSG_GCTRL_TYRANNY_A2F_TOWER_WINNER:
		{
			GLMSG::SNETPC_TYRANNY_A2F_TOWER_WINNER* pnetmsg = ( GLMSG::SNETPC_TYRANNY_A2F_TOWER_WINNER* ) nmg;

			GLMSG::SNETPC_TYRANNY_F2C_TOWER_WINNER netmsgclient;
	...
	case NET_MSG_GCTRL_TYRANNY_C2F_REVIVE_REQ:
		{
			GLMSG::SNETPC_TYRANNY_C2F_REVIVE_REQ* pnetmsg = ( GLMSG::SNETPC_TYRANNY_C2F_REVIVE_REQ* ) nmg;
