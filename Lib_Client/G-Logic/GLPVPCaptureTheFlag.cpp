...
bool GLPVPCaptureTheFlag::Load( std::string strFile )
{
	...
	{
		CDebugSet::ToLogFile ( "ERROR : GLPVPCaptureTheFlag::Load(), File Open %s", strFile.c_str() );
		return false;
	}
	...
	//base gate
	for( DWORD i=0; i<CAPTURE_THE_FLAG_TEAM_SIZE; ++i ){
		cFILE.getflag( "EVENT", "EVENT_GATE", i, CAPTURE_THE_FLAG_TEAM_SIZE, m_wTeamGate[i] );
	}
	...
	cFILE.getflag( "EVENT", "CONTRI_REQ", 0, 1, m_llContributionReq );
	cFILE.getflag( "EVENT", "MAX_SCORE", 0, 1, m_wMaxScore );
	cFILE.getflag( "EVENT", "TEAM_BALANCE", 0, 1, m_wBalancer );
	...
	//score logic
	cFILE.getflag( "EVENT", "VAR_FLAG", 0, 1, m_fFlagVar );
	...
	cFILE.getflag( "EVENT", "FLAG_CROW_A", 0, 2, m_sFlagCrow[CAPTURE_THE_FLAG_TEAM_A].wMainID );
	cFILE.getflag( "EVENT", "FLAG_CROW_A", 1, 2, m_sFlagCrow[CAPTURE_THE_FLAG_TEAM_A].wSubID );
	
	cFILE.getflag( "EVENT", "FLAG_CROW_B", 0, 2, m_sFlagCrow[CAPTURE_THE_FLAG_TEAM_B].wMainID );
	cFILE.getflag( "EVENT", "FLAG_CROW_B", 1, 2, m_sFlagCrow[CAPTURE_THE_FLAG_TEAM_B].wSubID );
	...
	if ( m_wPlayerLimit == 0 )
	{
		CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid PlayerLimit:%u limit must be atleast 1", m_wPlayerLimit );
		m_wPlayerLimit = 1;
	}

	if ( m_wLevelReq == 0 )
	{
		CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid LevelReq:%u req must be atleast 1", m_wLevelReq );
		m_wLevelReq = 1;
	}

	if ( m_wMaxScore == 0 )
	{
		CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid MaxScore:%u limit must be atleast 1", m_wMaxScore );
		m_wMaxScore = 1;
	}

	if ( m_wRegisterTime < 300 )
	{
		CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid BattleTime time must be more than 300 sec:%u", m_wRegisterTime );
		m_wRegisterTime = 300;
	}

	if ( m_wBattleTime < 300 )
	{
		CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid BattleTime time must be more than 300 sec:%u", m_wBattleTime );
		m_wBattleTime = 300;
	}

	if ( m_wRewardTime < 180 )
	{
		CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid RewardTime time must be more than 300 sec:%u", m_wRewardTime );
		m_wRewardTime = 180;
	}
	...
	for( DWORD i=0; i<dwnum; ++i )
	{
		...
		{
			CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid startday:%u index:%d", sched.wStartDay, i );
			continue;
		}

		if ( sched.wStartHour >= 24 )
		{
			CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid StartHour:%u index:%d", sched.wStartHour, i );
			continue;
		}
		...
		{
			CDebugSet::MsgBoxAfx( "GLPVPCaptureTheFlag::Load invalid StartMinute:%u index:%d", sched.wStartMinute, i );
			continue;
		}
	...
	{
		CDebugSet::ToFileWithTime( "_pvpteambattle.txt", "GLPVPCaptureTheFlag::Load no schedules loaded" );
	}

	return true;
}

void GLPVPCaptureTheFlag::FindScheduleFirst()
{
	if ( m_vecSchedule.empty() )	return;

	m_sScheduleNext.dwIndex = CAPTURE_THE_FLAG_SCHEDULE_NONE;
	...
	else
	{
		...
		if ( m_sScheduleNext.dwIndex == CAPTURE_THE_FLAG_SCHEDULE_NONE )
		{
			CDebugSet::ToFileWithTime( "_pvpcapturetheflag.txt", "FindScheduleFirst failed! no events will start" );
		}
		else
		{
			CDebugSet::ToFileWithTime( "_pvpcapturetheflag.txt", "FindScheduleFirst Index:%u Start(D:%u/H:%u/M:%u) BattleStart(H:%u/M:%u) BattleEnd(H:%u/M:%u)", m_sScheduleNext.dwIndex, 
...
bool GLPVPCaptureTheFlag::FindScheduleNext()
{
	if ( m_sScheduleNext.dwIndex == CAPTURE_THE_FLAG_SCHEDULE_NONE )	return false;
	...
	CDebugSet::ToFileWithTime( "_pvpcapturetheflag.txt", "FindScheduleNext Index:%u Start(D:%u/H:%u/M:%u) BattleStart(H:%u/M:%u) BattleEnd(H:%u/M:%u)", m_sScheduleNext.dwIndex, 
		m_sScheduleNext.sStartSched.wStartDay, m_sScheduleNext.sStartSched.wStartHour, m_sScheduleNext.sStartSched.wStartMinute,
		m_sScheduleNext.wBattleStartHour, m_sScheduleNext.wBattleStartMinute, m_sScheduleNext.wBattleEndHour, m_sScheduleNext.wBattleEndMinute );
