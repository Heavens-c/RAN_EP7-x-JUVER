	...
	case NET_MSG_GCTRL_CLUB_MEMBER_DEL_2CLT:
		{
			...
			{
				m_sCLUB.SetMemberCDMFlag(pNetMsg->dwClub, FALSE);
	...
	case NET_MSG_GCTRL_CLUB_SUBMASTER_FB:
		{
			...
			case EMCLUBSUBMASTER_FB_CDM_MEM:
				{
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMCLUBSUBMASTER_FB_CDM_MEM"), GLCONST_CHAR::dwCLUB_DEATHMATCH_MEM );
				}break;

			case EMCLUBSUBMASTER_FB_CDM:
				{
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMCLUBSUBMASTER_FB_CDM") );
				}break;
			};
		}
		break;
