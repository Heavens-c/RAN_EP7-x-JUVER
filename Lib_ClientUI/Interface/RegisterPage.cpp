...
void CRegisterPage::CreateSubControl ()
{
	...
	pStaticText = new CBasicTextBox;
	pStaticText->CreateSub ( this, "RAN_REGISTER_PAGE_STATIC_INFO_SECKEY" );
	pStaticText->SetFont ( pFont9 );
	pStaticText->SetTextAlign ( TEXT_ALIGN_LEFT );	
	pStaticText->AddText( ID2GAMEINTEXT ( "RAN_REGISTER_INFO_SECURITY_KEY" ), NS_UITEXTCOLOR::GOLD );
	RegisterControl ( pStaticText );
...
void CRegisterPage::DoRegister()
{
	...
	if ( strcmp( strsa.c_str(), strpass.c_str() ) == 0 )
	{
		m_pTextStatus->SetText( ID2GAMEINTEXT( "RAN_REGISTER_FB_SA_UNIQUE" ), NS_UITEXTCOLOR::RED );
		return;
	}
