...
IMPLEMENT_DYNAMIC(CLockerTab, CPropertySheet)

CLockerTab::CLockerTab( SCHARDATA2* pData, CRect& rect, LOGFONT logfont, CWnd* pParentWnd /*= NULL*/ )
	 : CPropertySheet("Sheet", pParentWnd),
	m_bNoTabs(FALSE),
	m_pWndParent(NULL),
	m_iDistanceFromTop(0),
	
	m_PageMain ( logfont, pData ),
	m_PageItem1 ( logfont, pData ),
	m_PageItem2 ( logfont, pData ),
	m_PageItem3 ( logfont, pData ),
	m_PageItem4 ( logfont, pData ),
	m_PageItemP ( logfont, pData ),
	...
	m_pData( NULL )
{
	...
	m_PageItem1.SetSheetTab ( this );
	m_PageItem2.SetSheetTab ( this );
	m_PageItem3.SetSheetTab ( this );
	m_PageItem4.SetSheetTab ( this );
	m_PageItemP.SetSheetTab ( this );
...
BEGIN_MESSAGE_MAP(CLockerTab, CPropertySheet)
	//{{AFX_MSG_MAP(CLockerTab)
	ON_WM_CTLCOLOR_REFLECT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

HBRUSH CLockerTab::CtlColor(CDC* pDc, UINT uCtlColor) 
{
...
BOOL CLockerTab::PreTranslateMessage(MSG* pMsg)
{
	switch ( pMsg->message )
	{
	case WM_KEYDOWN:
...
void CLockerTab::ChangeDialogFont(CWnd* pWnd, CFont* pFont, int nFlag)
{
	CRect windowRect;
