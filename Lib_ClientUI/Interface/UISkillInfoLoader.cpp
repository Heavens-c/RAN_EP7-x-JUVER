...
namespace NS_SKILLINFO
{
	...
	void LOAD ( GLSKILL* const pSkill, SCHARSKILL* const pCharSkill, BOOL bNextLevel )
	{
		...
		//	기본 정보
		{
			...
			strText.Format("%s:%s",ID2GAMEWORD("SKILL_BASIC_INFO", 5), COMMENT::SKILL_ACTION_TYPE[pSkill->m_sBASIC.emACTION].c_str());
			AddTextNoSplit(strText,NS_UITEXTCOLOR::PALEGREEN);
		...
		{		
			...
			for( int nSpec=0; nSpec<SKILL::MAX_SPEC; ++nSpec )
			{
				...
				{
					if ( sAPPLY.sSpecs[nSpec].emSPEC == EMSPECA_PROHIBIT_POTION ) bShowDuration = false;
				}
			}

			if ( 0 < sDATA_LVL.fLIFE && bShowDuration )
			{
