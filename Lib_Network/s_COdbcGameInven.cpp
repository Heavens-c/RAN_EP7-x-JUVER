...
{
    if (wInvenLine < 1 || wInvenLine > 30 || nChaNum <= 0)
    {
        return DB_ERROR;
    }
...
{    
	...
	while(true)
	{
		...
		{	
			...
			VerifyTimeValid( "COdbcManager::ReadUserInven", "sStorageDate2", sStorageDate2 );
			VerifyTimeValid( "COdbcManager::ReadUserInven", "sStorageDate3", sStorageDate3 );
			VerifyTimeValid( "COdbcManager::ReadUserInven", "sStorageDate4", sStorageDate4 );
