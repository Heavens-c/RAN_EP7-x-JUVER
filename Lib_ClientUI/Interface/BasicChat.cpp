...
void CBasicChat::AddChat ( const CString& strName, const CString& strMsg, const WORD& wType, SITEMLINK* pItemLink )
{
	if( m_pBasicChatRightBody )
	{
		/*item link, Juver, 2017/07/31 */
		m_pBasicChatRightBody->AddChat ( strName, strMsg, wType, pItemLink ); 
	}
}

/*item link, Juver, 2017/07/31 */
void CBasicChat::AddStringToChatEx ( CString strTemp, WORD wType, SITEMLINK* pItemLink ) 
{
	if( m_pBasicChatRightBody )
	{
		/*item link, Juver, 2017/07/31 */
		m_pBasicChatRightBody->AddStringToChatEx ( strTemp, wType, pItemLink ); 
	}
}

/*item link, Juver, 2017/07/31 */
void CBasicChat::AddStringToSystemMessage ( CString strTemp, D3DCOLOR dwColor, SITEMLINK* pItemLink ) 
{
	if( m_pBasicChatRightBody )
	{
		 /*item link, Juver, 2017/07/31 */
		m_pBasicChatRightBody->AddStringToSystemMessage ( strTemp, dwColor, pItemLink );
	}
}

/*item link, Juver, 2017/07/31 */
void CBasicChat::AddItemLink( SITEMCUSTOM* pItemCustom )
{
	if( m_pBasicChatRightBody && pItemCustom )
		m_pBasicChatRightBody->AddItemLink( pItemCustom );
}
...
void CBasicChat::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	case CHAT_REGIONAL:	m_pBasicChatLeftBar->DO_CHAT_STATE( 6 ); break; /*regional chat, Juver, 2017/12/06 */
	}
	...
	case CBasicChatRightBody::CHAT_CHANNEL_REGIONAL:	/*regional chat, Juver, 2017/12/06 */
	...
	case CHAT_SHOW_FLAG:
		{
			...
			if ( UIMSG_CHAT_REGIONAL & dwMsg ) m_pBasicChatRightBody->AddChatFlagState ( CHAT_REGIONAL ); /*regional chat, Juver, 2017/12/06 */
	...
	case LEFT_BAR:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )		//	컨트롤 내부에 마우스가 위치하고,
			{
				...
				if ( UIMSG_MOUSEIN_REGIONAL_CHAT & dwMsg )
				{
					BEGIN_REGIONAL_CHAT();
					return ;
				}
...
/*regional chat, Juver, 2017/12/06 */
void	CBasicChat::BEGIN_REGIONAL_CHAT ( const CString& strMessage )
{
	m_pBasicChatRightBody->SetIgnoreState ( true );
	m_pBasicChatRightBody->BEGIN_REGIONAL_CHAT ( strMessage );
	m_pBasicChatRightBody->SetIgnoreState ( false );
}
