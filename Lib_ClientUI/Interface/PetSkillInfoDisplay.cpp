...
CPetSkillInfoDisplay::CPetSkillInfoDisplay () 
	: m_pSkillImage ( NULL )
	, m_strSkillName("")
	, m_fUpdateTime(0.0f)
{
...
void  CPetSkillInfoDisplay::CreateSubControl ()
{
...
void CPetSkillInfoDisplay::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );
	
	m_fUpdateTime += fElapsedTime;
	if ( m_fUpdateTime >= PET_SKILL_INFO_DISPLAY_UPDATE_TIME )
	{
		m_fUpdateTime = 0.0f;

		SNATIVEID sNativeID = GLGaeaClient::GetInstance().GetPetClient()->m_sActiveSkillID_A;
