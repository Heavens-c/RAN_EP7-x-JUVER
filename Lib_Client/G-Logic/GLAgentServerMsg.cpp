	...
	case CHAT_TYPE_REGIONAL: 
		{
			PGLCHARAG pChar = GetChar ( dwGaeaID );
			if ( !pChar )							return FALSE;
			if ( pChar->m_wSchool<0 )				return FALSE;
			if ( pChar->IsCHATBLOCK() )
			{
				GLMSG::SNETPC_CHAT_BLOCK NetChatBlock;
				StringCchCopy ( NetChatBlock.szName, USR_ID_LENGTH+1, pChar->m_szUserName );
			...
			for ( ; pCharNode; pCharNode=pCharNode->pNext )
			{
				if ( pChar->m_wSchool == pCharNode->Data->m_wSchool )
					SENDTOCLIENT ( pCharNode->Data->m_dwClientID, &NetChatFB );
			}
		}break;
...
HRESULT GLAgentServer::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 CharView
	case NET_MSG_GCTRL_REQ_CHAR_VIEW:					MsgReqCharView(nmg, dwClientID, dwGaeaID);		break;
	case NET_MSG_GCTRL_REQ_CHAR_VIEW_FLD:				MsgReqCharViewFLD(nmg);							break;
	/////////////////////////////////////////////////////////////////////////////
...
BOOL GLAgentServer::MsgReqGlobalRanking(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	...
	// Top GUild
	{
		...
		VEC_TOP_GUILD vecTopGuild = m_vecTopGuild;

		if (!vecTopGuild.empty())
		{
			sort(vecTopGuild.begin(), vecTopGuild.end(), SortByHighestWin);

			{
				...
				for (size_t j = 0; j < 10; j++)
				{
					...
					GLClubMan& cClubMan	= GetClubMan();
					GLCLUB* pCLUB		= cClubMan.GetClub(vecTopGuild[j].wGuNum);
					if (pCLUB)
					{
						CLUB_ALLIANCE_ITER iter = pCLUB->m_setAlliance.begin();
						CLUB_ALLIANCE_ITER iter_end = pCLUB->m_setAlliance.end();

						for ( ; iter != iter_end; ++iter )
						{
							//GLCLUBALLIANCE sClubAlliance = *iter;
							NetMsgFB.sTopGuild_[NetMsgFB.wSize].wAlliance++;
						}

						//NetMsgFB.sTopGuild_[NetMsgFB.wSize].wAlliance = (WORD)pCLUB->m_dwAlliance;

						for (size_t k = 0; k < pCLUB->m_mapMembers.size(); k++)
						{
							PGLCHARAG pMembers = GetCharID(pCLUB->m_mapMembers[k].dwID);
							if (pMembers)
								NetMsgFB.sTopGuild_[NetMsgFB.wSize].wOnline++;
						}
					}
...
/////////////////////////////////////////////////////////////////////////////
// LG-7 CharView
BOOL GLAgentServer::MsgReqCharView(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	GLMSG::SNETPC_REQ_CHAR_VIEW* pNetMsg = (GLMSG::SNETPC_REQ_CHAR_VIEW*)nmg;

	PGLCHARAG pCharAG = GetChar(dwGaeaID);
	if (!pCharAG) return FALSE;

	GLMSG::SNETPC_REQ_CHAR_VIEW_FB NetMsgFB;

	if (!pCharAG->m_bCharView)
	{
		NetMsgFB.emFB	= EM_CHAR_VIEW_FB_DELAY;
		NetMsgFB.fDelay	= pCharAG->m_fCharView;
		SENDTOCLIENT(pCharAG->m_dwClientID, &NetMsgFB);

		return FALSE;
	}

	pCharAG->m_bCharView = FALSE;

	PGLCHARAG pCharTar = NULL;
	
	pCharTar = GetCharID(pNetMsg->dwCharID);
	...
	GLMSG::SNETPC_REQ_CHAR_VIEW_AGT NetMsg;
	NetMsg.dwGaeaID	= pCharAG->m_dwGaeaID;
	SENDTOFIELD(pCharTar->m_dwClientID, &NetMsg);

	return TRUE;
}

BOOL GLAgentServer::MsgReqCharViewFLD(NET_MSG_GENERIC* nmg)
{
	GLMSG::SNETPC_REQ_CHAR_VIEW_FLD* pNetMsg = (GLMSG::SNETPC_REQ_CHAR_VIEW_FLD*)nmg;
	...
	case EM_CHAR_VIEW_FB_OK:
	{
		...
		NetMsgFB.sCharView	= pNetMsg->sCharView;
		SENDTOCLIENT(pCharAG->m_dwClientID, &NetMsgFB);
