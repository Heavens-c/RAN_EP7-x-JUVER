...
CServer::~CServer()
{
	...
	CIPFilter::GetInstance()->ReleaseInstance();
}

// Create CFG file loader and open cfg file
int CServer::StartCfg()
{	
	//use application filename as cfg filename

	TCHAR szPath[MAX_PATH] = {0};
	GetModuleFileName(::AfxGetInstanceHandle(), szPath, MAX_PATH);
	std::string strAppPath = szPath;

	if ( strAppPath.empty() )
	{
		CConsoleMessage::GetInstance()->Write(_T("ERROR:CFG file load error, failed to get application name"));
		return NET_ERROR;
	}

	std::string::size_type slashpos = strAppPath.find_last_of( _T("\\/") );
	if ( slashpos != std::string::npos )
		strAppPath.erase( 0, slashpos + 1 );
	std::string::size_type dotpos = strAppPath.rfind( _T('.') );
	if ( dotpos != std::string::npos )
		strAppPath.erase( dotpos, strAppPath.size() - dotpos );

	std::string strCfgFileName = strAppPath + _T(".cfg");

	int	nRetCode;

	nRetCode = CCfg::GetInstance()->Load( strCfgFileName.c_str() );
	if (nRetCode != 0)
	{
		CConsoleMessage::GetInstance()->Write(_T("ERROR:CFG file load error, cant find %s"), strCfgFileName.c_str() );
		m_bIsRunning = false;
		return NET_ERROR;
	}	
	...
	m_nServerType		= CCfg::GetInstance()->GetServerType();
