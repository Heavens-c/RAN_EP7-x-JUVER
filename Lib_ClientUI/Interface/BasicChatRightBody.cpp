...
const CString CBasicChatRightBody::REGIONAL_SYMBOL( _T("^") ); /*regional chat, Juver, 2017/12/06 */
...
void CBasicChatRightBody::CreateSubControl ( CD3DFontPar*	pFont )
{
	...
	m_pChatRegionalButton = CreateTextButton ( "CHAT_CHANNEL_REGIONAL", CHAT_CHANNEL_REGIONAL, const_cast<char*>(ID2GAMEWORD("CHAT_CHANNEL_BUTTON",7)) );
	...
	{	/*regional chat, Juver, 2017/12/06 */
		m_pTextBox_REGIONAL = CreateTextBoxEx ( pFont, CHAT_TEXTBOX_REGIONAL );
		nTotalLine = m_pTextBox_REGIONAL->GetTotalLine ();
		nVisibleLine = m_pTextBox_REGIONAL->GetVisibleLine ();

		m_pScrollBar_REGIONAL = CreateScrollBarEx ( nTotalLine, nVisibleLine );	
	}
...
//	if ( bFirstControl )
	{
		...
		{	
			...
			else if ( m_pTextBox_REGIONAL->IsVisible () )	UPDATE_CHAT_PAGE_TEXTBOX ( m_pTextBox_REGIONAL, m_pScrollBar_REGIONAL ); /*regional chat, Juver, 2017/12/06 */
		}
	}
		...
		SITEMLINK* pItemLink ) 
{
	...
	/*item link, Juver, 2017/07/31 */
	pTextBox->AddText ( strTemp, pItemLink, dwColor );
...
void CBasicChatRightBody::ChangeLastChat ()
{
	...
	if ( bKEYDOWN )
	{
		...
		else
		{
			...
			/*item link, Juver, 2017/07/31 */
			CString strChat = (*iter).strChat;
			m_pEditBox->SetEditString ( strChat );

			/*item link, Juver, 2017/07/31 */
			m_sItemLink = (*iter).sItemLink;
		}
	}
}
...
void	CBasicChatRightBody::UPDATE_CHATTYPE_STATE ( const int nCHATTYPE_BACK )
{
	...
	if ( nCHATTYPE_BACK != CHAT_NO && nCHATTYPE_BACK != m_nCHATTYPE )
	{
		...
		case CHAT_REGIONAL:
			{
				DWORD dwColor = NS_UITEXTCOLOR::DARKTURQUOISE;
				if ( RANPARAM::bFeatureRegionalChatColor )
					dwColor = D3DCOLOR_ARGB( 0xFF, RANPARAM::nFeatureRegionalChatColorR, RANPARAM::nFeatureRegionalChatColorG, RANPARAM::nFeatureRegionalChatColorB );

				m_pEditBox->SetTextColor ( dwColor );

				/*item link, Juver, 2017/07/31 */
				AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_REGIONAL_MODE"), dwColor, NULL );
			}break;
...
void	CBasicChatRightBody::ChangeChatType ()
{
	...
	if ( UIKeyCheck::GetInstance()->Check ( DIK_TAB, DXKEY_DOWN ) )
	{
		...
		case CHAT_ALLIANCE:	BEGIN_REGIONAL_CHAT();	break;
		case CHAT_REGIONAL:	BEGIN_NORMAL_CHAT();	break; /*regional chat, Juver, 2017/12/06 */
		}			
	}	
}
...
{	
	...
	if( nEditLength )
	{
		...
		else if( FIRST_CHAR == REGIONAL_SYMBOL )	m_nCHATTYPE = CHAT_REGIONAL;		/*regional chat, Juver, 2017/12/06 */
	}
}
...
//	m_pSystemMessageDisplay->SetVisibleSingle ( TRUE );
	...
	/*regional chat, Juver, 2017/12/06 */
	m_pTextBox_REGIONAL->SetVisibleSingle ( FALSE );
	m_pScrollBar_REGIONAL->SetVisibleSingle ( FALSE );
	...
	m_pChatRegionalButton->SetFlip ( FALSE ); /*regional chat, Juver, 2017/12/06 */
...
//			m_pSystemMessageDisplay->SetVisibleSingle ( FALSE );
	...
	case CHAT_CHANNEL_REGIONAL:
		{
			m_pChatRegionalButton->SetFlip ( TRUE );

			if ( !m_bMINIMUM_SIZE_TEXT ) m_pTextBox_REGIONAL->SetVisibleSingle ( TRUE );
			if ( !m_bMINIMUM_SIZE_SCROLL )
			{
				int nTotal = m_pTextBox_REGIONAL->GetTotalLine();
				int nVisible = m_pTextBox_REGIONAL->GetVisibleLine();

				if( nTotal > nVisible ) m_pScrollBar_REGIONAL->SetVisibleSingle ( TRUE );
			}
		}break;
	}
}
...
CBasicTextButton*	 CBasicChatRightBody::CreateTextButton ( char* szButton, UIGUID ControlID, char* szText )
{
 ...
 /*item link, Juver, 2017/07/31 */
void CBasicChatRightBody::AddChat ( const CString& strName, const CString& strMsg, const WORD& wType, SITEMLINK* pItemLink )
{
	CString strCombine;
	strCombine.Format ( "[%s]:%s", strName, strMsg );

	/*item link, Juver, 2017/07/31 */
	AddStringToChatEx ( strCombine, wType, pItemLink ); 
...
void CBasicChatRightBody::AddChatMacro(CString strMsg)
{
	...
	else if( ChatType == REGIONAL_SYMBOL )	wType = CHAT_REGIONAL;	/*regional chat, Juver, 2017/12/06 */
...
void CBasicChatRightBody::SetVisibleChatButton( BOOL bVisible )
{
	...
	m_pChatRegionalButton->SetVisibleSingle( bVisible );			/*regional chat, Juver, 2017/12/06 */
}

/*item link, Juver, 2017/07/31 */
void CBasicChatRightBody::EDIT_CLEAR()
{
	m_pEditBox->ClearEdit ();
...
/*item link, Juver, 2017/07/31 */
void CBasicChatRightBody::CheckItemLink( CString strText )
{
	CString strFind = strText.GetString();
	int n = strFind.Find ( m_sItemLink.GetFilter() );
	bool bFind = (n >= 0);
	m_sItemLink.bUSE = bFind;
}

/*item link, Juver, 2017/07/31 */
void CBasicChatRightBody::CheckItemLink( CString strText, SITEMLINK sItemLink )
{
	if ( sItemLink.bUSE )
	{
		CString strFind = strText.GetString();
		...
		if ( bFind )
		{
			...
			sItemCustom.Assign( sItemLink );
			CInnerInterface::GetInstance().SET_INFO_ITEM_LINK( sItemCustom );
		}
	}
}

/*item link, Juver, 2017/07/31 */
void CBasicChatRightBody::CheckPreviewItemLink( CString strText, SITEMLINK sItemLink )
{
	if ( sItemLink.bUSE )
	{
		CString strFind = strText.GetString();
...
void CBasicChatRightBody::CheckBoxContentsItemLink( CString strText, SITEMLINK sItemLink )
{
	if ( sItemLink.bUSE )
	{
		CString strFind = strText.GetString();
		...
		if ( bFind )
		{
			CInnerInterface::GetInstance().ShowBoxContents( sItemLink.sNativeID );
		}
	}
}