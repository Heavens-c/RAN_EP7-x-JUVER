	...
	bool bBooster )
{
	if (nVehicleNum <= 0 )
	{
		Print(_T("ERROR:COdbcManager::SetVehicleBooster"));
		return DB_ERROR;
	}

	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_UpdateVehicleBooster(%d,%d,%d,?)}", nVehicleNum, nCharNum, bBooster );
							...
							int nCharNum)
{
	...
	while (true)
	{
		...
		{			
			...
			::SQLGetData(pConn->hStmt, 6, SQL_C_LONG, &nBooster, 0, &cbBooster); /*vehicle booster system, Juver, 2017/08/10 */
			...
			pVehicle->m_bBooster	= (nBooster == 1)? true:false; /*vehicle booster system, Juver, 2017/08/10 */
			nRowCount++;
		}
		else
		{
...
/*bike color , Juver, 2017/11/13 */
int COdbcManager::GetVehicleColor( GLVEHICLE* pVehicle, int nVehicleNum, int nCharNum )
{
	if (pVehicle == NULL)
	{
		Print(_T("ERROR:COdbcManager::GetVehicleInven pVehicle==NULL"));
	...
	nReturn = m_pGameDB->ReadImageVehicle("VehicleInfo.VehicleColor", nVehicleNum, nCharNum,  ByteStream);
	...
	{
		pVehicle->SETCOLORS_BYBUF( ByteStream );
		return DB_OK;
	}
}

/*bike color , Juver, 2017/11/13 */
int COdbcManager::SetVehicleColor( int nVehicle, int nCharNum, CByteStream &ByteStream )
{
	if (nVehicle <= 0)
	{
