...
HRESULT GLPetField::MsgChangeActiveSkill_A ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_SKILLCHANGE_A* pNetMsg = ( GLMSG::SNETPET_REQ_SKILLCHANGE_A* ) nmg;

	if ( !CheckSkill ( pNetMsg->sSkillID ) && pNetMsg->sSkillID != NATIVEID_NULL() ) return E_FAIL;

	/* pet skill bug Juver 2017/06/27 */
	if ( !IsValid() && pNetMsg->bFromUI )
	{
		std::string strPlayerName = "";
		if( m_pOwner )	strPlayerName = m_pOwner->m_szName;
		GLGaeaServer::GetInstance().GetConsoleMsg()->Write( LOG_TEXT_CONSOLE, "MsgChangeActiveSkill_A Attempting to change skill when pet is not active! Name:[%s] Skill:[%u~%u]", 
	...
	if ( m_sActiveSkillID_A != NATIVEID_NULL() )
	{
		if( m_sActiveSkillID_A.wMainID != 26 )
		{
			CDebugSet::ToLogFile( "ERR : GLPetField::MsgChangeActiveSkill_01, MID = %d, SID = %d", m_sActiveSkillID_A.wMainID, m_sActiveSkillID_A.wSubID );
		}

		// DB에 스킬 변경 저장
		/*dual pet skill, Juver, 2017/12/29 */
		PETSKILL sPetSkill( m_sActiveSkillID_A, 0 );
		CSetPetSkill_A* pDbAction = new CSetPetSkill_A ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, false );
	...
	if ( pNetMsg->sSkillID == NATIVEID_NULL() )
	{
		...
		// 스킬제거
		GLMSG::SNETPET_REMOVE_SKILLFACT_A NetMsgRemoveSkill;
		GLGaeaServer::GetInstance().SENDTOCLIENT( m_pOwner->m_dwClientID,&NetMsgRemoveSkill );

		// 주변에 알림
		GLMSG::SNETPET_REQ_SKILLCHANGE_A_BRD NetMsgBRD;
		NetMsgBRD.dwGUID   = m_dwGUID;
		...
		if( m_sActiveSkillID_A.wMainID != 26 )
		{
			CDebugSet::ToLogFile( "ERR : GLPetField::MsgChangeActiveSkill_02, MID = %d, SID = %d", m_sActiveSkillID_A.wMainID, m_sActiveSkillID_A.wSubID );
		}
		...
		m_sActiveSkillID_A = pNetMsg->sSkillID;
	...
	PETSKILL_MAP_ITER iter = m_ExpSkills.find ( m_sActiveSkillID_A.dwID );
	...
	// 보조스킬 팩터에 추가
	m_pOwner->m_sPETSKILLFACT_A.emTYPE = pSkill->m_sAPPLY.emBASIC_TYPE;
	m_pOwner->m_sPETSKILLFACT_A.fMVAR = sSKILL_DATA.fBASIC_VAR;
	m_pOwner->m_sPETSKILLFACT_A.fAGE = sSKILL_DATA.fLIFE;
	m_pOwner->m_sPETSKILLFACT_A.sNATIVEID = m_sActiveSkillID_A;

	// 클라이언트에 팩터 추가 알림
	GLMSG::SNETPET_ADD_SKILLFACT_A NetMsgAddSkill;
	NetMsgAddSkill.sNATIVEID = m_sActiveSkillID_A;
	...
	if( m_sActiveSkillID_A.wMainID != 26 )
	{
		CDebugSet::ToLogFile( "ERR : GLPetField::MsgChangeActiveSkill_03, MID = %d, SID = %d",  m_sActiveSkillID_A.wMainID, m_sActiveSkillID_A.wSubID );
	}

	// DB에 스킬 변경 저장
	CSetPetSkill_A* pDbAction = new CSetPetSkill_A ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, true );
	GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
	if ( pDBMan ) pDBMan->AddJob ( pDbAction );
...
HRESULT GLPetField::MsgChangeActiveSkill_B ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_SKILLCHANGE_B* pNetMsg = ( GLMSG::SNETPET_REQ_SKILLCHANGE_B* ) nmg;
	...
	if ( !IsValid() && pNetMsg->bFromUI )
	{
		std::string strPlayerName = "";
		if( m_pOwner )	strPlayerName = m_pOwner->m_szName;
		GLGaeaServer::GetInstance().GetConsoleMsg()->Write( LOG_TEXT_CONSOLE, "MsgChangeActiveSkill_B Attempting to change skill when pet is not active! Name:[%s] Skill:[%u~%u]", 
			strPlayerName.c_str(), pNetMsg->sSkillID.wMainID, pNetMsg->sSkillID.wSubID );
	...
	if ( m_sActiveSkillID_B != NATIVEID_NULL() )
	{
		if( m_sActiveSkillID_B.wMainID != 26 )
		{
			CDebugSet::ToLogFile( "ERR : GLPetField::MsgChangeActiveSkill_01, MID = %d, SID = %d", m_sActiveSkillID_B.wMainID, m_sActiveSkillID_B.wSubID );
		}

		// DB에 스킬 변경 저장
		/*dual pet skill, Juver, 2017/12/29 */
		PETSKILL sPetSkill( m_sActiveSkillID_B, 0 );
		CSetPetSkill_B* pDbAction = new CSetPetSkill_B ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, false );
		GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
		if ( pDBMan ) pDBMan->AddJob ( pDbAction );
	}
	...
	if ( pNetMsg->sSkillID == NATIVEID_NULL() )
	{
		...
		// 스킬제거
		GLMSG::SNETPET_REMOVE_SKILLFACT_B NetMsgRemoveSkill;
		GLGaeaServer::GetInstance().SENDTOCLIENT( m_pOwner->m_dwClientID,&NetMsgRemoveSkill );

		// 주변에 알림
		GLMSG::SNETPET_REQ_SKILLCHANGE_B_BRD NetMsgBRD;
		NetMsgBRD.dwGUID   = m_dwGUID;
		NetMsgBRD.dwTarID  = m_pOwner->m_dwGaeaID;
		NetMsgBRD.sSkillID = pNetMsg->sSkillID;
		m_pOwner->SendMsgViewAround ( ( NET_MSG_GENERIC* ) &NetMsgBRD );
		...
		if( m_sActiveSkillID_B.wMainID != 26 )
		{
			CDebugSet::ToLogFile( "ERR : GLPetField::MsgChangeActiveSkill_02, MID = %d, SID = %d", m_sActiveSkillID_B.wMainID, m_sActiveSkillID_B.wSubID );
		}
		...
		m_sActiveSkillID_B = pNetMsg->sSkillID;
	...
	if ( !m_bDualSkill )
	{
		CDebugSet::ToFileWithTime( "_petcheck.txt", "[%u]%s MsgChangeActiveSkill_B attempt to change pet skill on non dual skill", 
	...
	PETSKILL_MAP_ITER iter = m_ExpSkills.find ( m_sActiveSkillID_B.dwID );
	if ( iter == m_ExpSkills.end() ) return E_FAIL;
	PETSKILL sPetSkill = (*iter).second;
	SKILL::CDATA_LVL &sSKILL_DATA = pSkill->m_sAPPLY.sDATA_LVL[sPetSkill.wLevel];
	...
	// 보조스킬 팩터에 추가
	m_pOwner->m_sPETSKILLFACT_B.emTYPE = pSkill->m_sAPPLY.emBASIC_TYPE;
	m_pOwner->m_sPETSKILLFACT_B.fMVAR = sSKILL_DATA.fBASIC_VAR;
	m_pOwner->m_sPETSKILLFACT_B.fAGE = sSKILL_DATA.fLIFE;
	m_pOwner->m_sPETSKILLFACT_B.sNATIVEID = m_sActiveSkillID_B;

	// 클라이언트에 팩터 추가 알림
	GLMSG::SNETPET_ADD_SKILLFACT_B NetMsgAddSkill;
	NetMsgAddSkill.sNATIVEID = m_sActiveSkillID_B;
	NetMsgAddSkill.emTYPE	 = pSkill->m_sAPPLY.emBASIC_TYPE;
	NetMsgAddSkill.fMVAR	 = sSKILL_DATA.fBASIC_VAR;
	NetMsgAddSkill.fAge		 = sSKILL_DATA.fLIFE;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID,&NetMsgAddSkill);
	...
	if( m_sActiveSkillID_B.wMainID != 26 )
	{
		CDebugSet::ToLogFile( "ERR : GLPetField::MsgChangeActiveSkill_03, MID = %d, SID = %d", m_sActiveSkillID_B.wMainID, m_sActiveSkillID_B.wSubID );
	}

	// DB에 스킬 변경 저장
	CSetPetSkill_B* pDbAction = new CSetPetSkill_B ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, true );
	GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
	if ( pDBMan ) pDBMan->AddJob ( pDbAction );
...
void GLPetField::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_PET_REQ_SKILLCHANGE_A:
		MsgChangeActiveSkill_A ( nmg );
		break;

		/*dual pet skill, Juver, 2017/12/27 */
	case NET_MSG_PET_REQ_SKILLCHANGE_B:
		MsgChangeActiveSkill_B ( nmg );
		break;
...
void GLPetField::ChangePetSKill_A( SNATIVEID sidSkill )
{
	if ( sidSkill == NATIVEID_NULL() )	return; 
	
	PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( sidSkill );
	if ( !pSkill )	return;

	PETSKILL_MAP_ITER iter = m_ExpSkills.find ( sidSkill.dwID );
	if ( iter == m_ExpSkills.end() ) return;

	m_sActiveSkillID_A = sidSkill;
...
/*dual pet skill, Juver, 2017/12/27 */
/*enable pet skill at pet creation, Juver, 2017/12/13 */
void GLPetField::ChangePetSKill_B( SNATIVEID sidSkill )
{
	if ( sidSkill == NATIVEID_NULL() )	return; 
