...
*/
	TCHAR szTemp[128] = {0};
    _snprintf( szTemp, 128, "{call sp_lottery_check('%s','%s','%s',%d,?)}", 
...
*/
	TCHAR szTemp[128] = {0};
    _snprintf( szTemp, 128, "{call sp_LogPungPung('%s','%s',%d,%d)}", 
...
*/
	TCHAR szTemp[128] = {0};
    _snprintf( szTemp, 128, "{call sp_PungPungCheck('%s',%d,?)}", 
...
int COdbcManager::LogUserAttend( int nUserNum, std::vector<USER_ATTEND_INFO> &vecUserAttend )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "Exec sp_UserAttendLog %d", nUserNum );
	...
	while (true)
	{
		...
		{	
			...
			VerifyTimeValid( "COdbcManager::LogUserAttend", "sAttendTime", sAttendTime );
...
int COdbcManager::InsertUserAttend( int nUserNum, int nCount, int nAttendReward )
{
	...
	TCHAR szTemp[128] = {0};
    _snprintf( szTemp, 128, "{call sp_InsertAttendLog( %d,%d,%d,?)}",
...
{
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_InsertItem('%s','%s',%d,%d,?)}",
		strPurKey.GetString(),
		strUserID.GetString(),
		nItemMID,
		nItemSID );
