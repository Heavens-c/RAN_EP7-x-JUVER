...
// could no longer update this list to newer cpu
// use simple naming
void CSystemInfo::GetProcessorName(char* szPname)
{
	...
	case PROCESSOR_ARCHITECTURE_AMD64:
		{
			StringCchCopy(szPname, MAX_PROCESSOR_NAME, _T("x64 Processor"));
		}break;
	case PROCESSOR_ARCHITECTURE_ARM:
		{
			StringCchCopy(szPname, MAX_PROCESSOR_NAME, _T("ARM Processor"));
		}break;
	case PROCESSOR_ARCHITECTURE_IA64:
		{
			StringCchCopy(szPname, MAX_PROCESSOR_NAME, _T("Itanium x64 Processor"));
		}break;
	case PROCESSOR_ARCHITECTURE_INTEL:
		{
			StringCchCopy(szPname, MAX_PROCESSOR_NAME, _T("x86 Processor"));
		}break;
	default:
		{
			StringCchCopy(szPname, MAX_PROCESSOR_NAME, _T("MultiCore Processor"));
		}break;
	};
...
void CSystemInfo::GetOSName(char* szName)
{
	...
	INT nWinVer;
	TCHAR szWinVer[ 50 ], szMajorMinorBuild[ 50 ], szServicePack[ 128 ];
	if( GetWinVer( szWinVer, &nWinVer, szMajorMinorBuild, szServicePack ) )
	{
		sprintf(szName, _T("%s %s (%s)"), szWinVer, szServicePack, szMajorMinorBuild);
	}
}
