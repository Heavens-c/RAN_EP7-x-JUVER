...
#include "GLPassenger/GLPassengerClient.h"
...
public:
	...
	EMACTIONTYPE GetACTION()	{ return m_Action; }
	void SetACTION( EMACTIONTYPE Action );
...
public:
	...
	HRESULT ReqRebuildResult( SITEMSEALDATA pData1, SITEMSEALDATA pData2 );								// ���� ��û
...
public:
	SAUTO_SKILL		m_pAutoSkill;
	void			ReqAutoSkill(SAUTO_SKILL* pAutoSkill);
	BOOL			AUTOSKILL_CHECK();
	BOOL			AutoSkillReaction(const bool bcontinue, BOOL& bMove, D3DXVECTOR3& vMoveTo);
	SNATIVEID		GetSKILL();
	SACTIONTAR		FindMOB();

	SAUTO_POTION	m_pAutoPotion;
	HRESULT			ReqAutoPotion(SAUTO_POTION* pAutoPotion);

	SAUTO_FOLLOW	m_pAutoFollow;
	HRESULT			ReqAutoFollow(CString strName);
	void			StartAutoFOLLOW(SAUTO_FOLLOW pAutoFollow);
	HRESULT			StopAutoFOLLOW();
	void			UpdateAutoFOLLOW(SAUTO_FOLLOW pAutoFollow);
	BOOL			AutoFollowReaction(const bool bcontinue, BOOL& bMove, D3DXVECTOR3& vMoveTo);
...
public:
		...
		/*get process command, Juver, 2017/06/08 */
	void GetPlayerProcess( DWORD _dwGMCHARID );
	
public:
	GLPassengerClient& GetPassengerClient ()	{ return GLPassengerClient::GetInstance(); }
...
public:
	...
	// LG-7 CharView
public:
	HRESULT ReqCharView(DWORD dwCharID);
	/////////////////////////////////////////////////////////////////////////////
};
