...
//			strTemp.freeze( false );	// Note : std::strstream의 freeze. 안 하면 Leak 발생.
		...
		{		
			...
			/*inventory lock, DevArt22, 2020/02/11 */
			::SQLGetData(pConn->hStmt,60, SQL_C_LONG, &nChaInventoryLockEnable, 0, &cbChaInventoryLockEnable );	
			::SQLGetData(pConn->hStmt,61, SQL_C_LONG, &nChaInventoryLockStatus, 0, &cbChaInventoryLockStatus );	

			/*storage lock, DevArt22, 2020/02/11 */
			::SQLGetData(pConn->hStmt,62, SQL_C_LONG, &nChaStorageLockEnable, 0, &cbChaStorageLockEnable );	
			::SQLGetData(pConn->hStmt,63, SQL_C_LONG, &nChaStorageLockStatus, 0, &cbChaStorageLockStatus );	

			::SQLGetData(pConn->hStmt,64, SQL_C_LONG, &nChaCWKill, 0, &cbChaCWKill );	
			::SQLGetData(pConn->hStmt,65, SQL_C_LONG, &nChaCWDeath, 0, &cbChaCWDeath );	
