...
#else
			strcpy(rRecipDesc.lpszName, pszName);
#endif
...
#else
			strcpy(rRecipDesc.lpszAddress, pszAddress);
#endif
	...
	else
	{
			...
			Resolve(strName, lpTempRecip))
		{
			...
			if (lpTempRecip->lpszName)
			{
				...
				if (rRecipDesc.lpszName)
					strcpy(rRecipDesc.lpszName, lpTempRecip->lpszName);
			}
			if (lpTempRecip->lpszAddress)
			{
				...
				if (rRecipDesc.lpszAddress)
					strcpy(rRecipDesc.lpszAddress, lpTempRecip->lpszAddress);
			}
		...
		else
		{
			...
			{
#ifdef _UNICODE
				WideCharToMultiByte(CP_ACP, 0, pszName, -1, rRecipDesc.lpszName, nSize, NULL, NULL);
#else
				strcpy(rRecipDesc.lpszName, pszName);
#endif
			}
...
#else
					strcpy(file.lpszPathName,pszFileName);
#endif
				}
...
#else
						strcpy(file.lpszFileName,pszTitle);
#endif
					}
				}
				else
