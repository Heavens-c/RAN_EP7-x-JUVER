...
void CAgentServer::MsgCharSelectPass( MSG_LIST* pMsg )
{

	CHINA_NET_CHARSELECT_PASS_DATA* pNml = NULL;

	pNml = reinterpret_cast<CHINA_NET_CHARSELECT_PASS_DATA *> (pMsg->Buffer);

	if (sizeof(CHINA_NET_CHARSELECT_PASS_DATA) != pNml->nmg.dwSize)
	{
		CConsoleMessage::GetInstance()->Write(
			
			_T("ERROR:CHINA_NET_CHARSELECT_PASS_DATA Wrong Message Size") );
		return;
	}
	// 복호화
	m_Tea.decrypt( pNml->szPass, USR_PASS_LENGTH );
	...
	m_pClientManager->SetAccountPass( pMsg->dwClient, true );
	StringCchCopy( szUserid, USR_ID_LENGTH+1, m_pClientManager->GetUserID(pMsg->dwClient) );
	StringCchCopy( szPass, USR_PASS_LENGTH, pNml->szPass );
	
	CAgentUserCharSelectPass* pAction = new CAgentUserCharSelectPass(
		szUserid, 
		szPass,
		dwClient);
	COdbcManager::GetInstance()->AddUserJob( (CDbAction*) pAction );
...
/*security key on charselect, DevArt22*/
void CAgentServer::MsgCharSelectPassBack (NET_CHARSELECT_PASS_FEEDBACK_DATA2* nlfd2)
{
	NET_CHARSELECT_PASS_FEEDBACK_DATA2		nlfd;

	nlfd.nmg.nType = CHINA_NET_MSG_CHARSELECT_PASS_FB;
	
	DWORD	dwClient   = (DWORD) nlfd2->nClient;
	DWORD	dwSndBytes = 0;
	int		nUserNum   = 0;		

	if ( nlfd2->nResult == EM_CHARSELECT_PASS_FB_SUB_OK )
	{
		nlfd.nResult   = nlfd2->nResult;
		if ( m_pClientManager->IsOnline( dwClient ) == true )
		{
	...
	}else if ( nlfd2->nResult == EM_CHARSELECT_PASS_FB_SUB_FAIL )
	{
		nlfd.nResult   = nlfd2->nResult;
		if ( m_pClientManager->IsOnline( dwClient ) == true )
		{
