...
void CLoginServer::SessionMsgProcess(MSG_LIST* pMsg)
{
	...
	case NET_MSG_IPFILTER_BLOCK_ADD_SERVERS:
		{
			NET_IPFILTER_BLOCK_ADD_SERVERS* netMsg = reinterpret_cast < NET_IPFILTER_BLOCK_ADD_SERVERS* > (nmg);
			CIPFilter::GetInstance()->AddIPBlock( std::string( netMsg->szIP ));
		}break;

	case NET_MSG_IPFILTER_BLOCK_DEL_SERVERS:
		{
			NET_IPFILTER_BLOCK_DEL_SERVERS* netMsg = reinterpret_cast < NET_IPFILTER_BLOCK_DEL_SERVERS* > (nmg);
	...
	case NET_MSG_IPFILTER_KNOWN_DEL_SERVERS:
		{
			NET_IPFILTER_KNOWN_DEL_SERVERS* netMsg = reinterpret_cast < NET_IPFILTER_KNOWN_DEL_SERVERS* > (nmg);
