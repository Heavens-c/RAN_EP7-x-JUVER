...
int CGMToolOdbcBase::UserFind( const char* szSQL, SGMTOOL_USER_SIMPLE_MAP &mapResult )
{
	...
	while (true)
	{
		...
		{			
			...
			::SQLGetData(pConn->hStmt,  11, SQL_C_LONG,	&nUserPoint2,         0, &cbUserPoint2);
			...
			sDATA.dwUserPoint2 = (DWORD)nUserPoint2;
...
int CGMToolOdbcBase::UserFind_UserNumber( int nUSERNUM, SGMTOOL_USER_SIMPLE_MAP &mapResult, int nLIMITNUM )
{
	...
	if ( nLIMITNUM == 0 ){
		_snprintf( szTemp, 1024, "SELECT UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserNum = '%d'", nUSERNUM );
	}
	else{
		_snprintf( szTemp, 1024, "SELECT TOP %d UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserNum = '%d'", nLIMITNUM, nUSERNUM );
	}
...
int CGMToolOdbcBase::UserFind_UserName( std::string strUSERID, SGMTOOL_USER_SIMPLE_MAP &mapResult, int nLIMITNUM )
{
	...
	if ( nLIMITNUM == 0 ){
		_snprintf( szTemp, 1024, "SELECT UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserID LIKE '%s%s%s' "
			"ORDER BY UserNum", strSign.c_str(), strUSERID.c_str(), strSign.c_str() );
	}
	else{
		_snprintf( szTemp, 1024, "SELECT TOP %d UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserID LIKE '%s%s%s' "
...
int CGMToolOdbcBase::UserFind_UserEmail( std::string strUSEREMAIL, SGMTOOL_USER_SIMPLE_MAP &mapResult, int nLIMITNUM )
{
	...
	if ( nLIMITNUM == 0 ){
		_snprintf( szTemp, 1024, "SELECT UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserEmail LIKE '%s%s%s' "
			"ORDER BY UserNum", strSign.c_str(), strUSEREMAIL.c_str(), strSign.c_str() );
	}
	else{
		_snprintf( szTemp, 1024, "SELECT TOP %d UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserEmail LIKE '%s%s%s' "
...
int CGMToolOdbcBase::UserFind_UserGM( SGMTOOL_USER_SIMPLE_MAP &mapResult, int nLIMITNUM )
{
	...
	TCHAR szTemp[1024] = {0};
	if ( nLIMITNUM == 0 ){
		_snprintf( szTemp, 1024, "SELECT UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserType >= '%d' "
			"ORDER BY UserNum", USER_GM3 );
	}
	else{
		_snprintf( szTemp, 1024, "SELECT TOP %d UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserType >= '%d' "
...
int CGMToolOdbcBase::UserFind_Online( SGMTOOL_USER_SIMPLE_MAP &mapResult, int nLIMITNUM )
{
	...
	TCHAR szTemp[1024] = {0};
	if ( nLIMITNUM == 0 ){
		_snprintf( szTemp, 1024, "SELECT UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserLoginState >= '%d' "
			"ORDER BY UserNum", 1 );
	}
	else{
		_snprintf( szTemp, 1024, "SELECT TOP %d UserNum, UserID, UserEmail, UserType, UserLoginState, UserAvailable, UserBlock, UserBlockDate, ChatBlockDate, UserPoint, UserPoint2 FROM UserInfo WITH (NOLOCK) WHERE UserLoginState >= '%d' "
...
int CGMToolOdbcBase::UserNew( SGMTOOL_USER_NEW* pData )
{
	...
	_snprintf( szTemp, 1024, 
		"INSERT Into UserInfo ( UserName, UserID, UserPass, UserPass2, UserEmail, UserSA, UserType, ChaRemain, ChaTestRemain, UserPoint, UserPoint2, UserAvailable, CreateDate )"
		" Values ( '%s', '%s', '%s', '%s', '%s', %u, %u, %u, %u, %u, %u, '%s' )"
		, pData->szUserID, pData->szUserID, pData->szUserPass, pData->szUserPass2, pData->szUserEmail, pData->szSecurityKey
		, pData->wUserType, pData->wChaRemain, pData->wChaRemain, pData->dwUserPoint, pData->dwUserPoint2, 1, 
...
int CGMToolOdbcBase::UserDataGet( int nUSERNUM, SGMTOOL_DATA_USER* pDATA )
{
	...
	TCHAR szTemp[1024] = {0};
	_snprintf( szTemp, 1024, "SELECT UserNum, UserID, UserPass, UserPass2, UserEmail, UserSA, UserType, UserLoginState, UserAvailable, ChaRemain, UserPoint, UserPoint2, UserBlock, "
	...
	SQLCHAR    szUserEmail[USR_EMAIL_LENGTH+1] = {0}; SQLINTEGER cbUserEmail = SQL_NTS;
	SQLCHAR    szSecurityKey[USR_PASS_LENGTH+1] = {0}; SQLINTEGER cbUserSA = SQL_NTS;
	SQLINTEGER nUserType = 0, cbUserType=SQL_NTS;
	SQLINTEGER nUserLoginState = 0, cbUserLoginState=SQL_NTS;
	SQLINTEGER nUserAvailable = 0, cbUserAvailable=SQL_NTS;
	...
	else
	{
		while(true)
		{
			...
			{	
				...
				::SQLGetData(pConn->hStmt,  6, SQL_C_CHAR,  szSecurityKey, USR_PASS_LENGTH, &cbUserSA);
				::SQLGetData(pConn->hStmt,  7, SQL_C_LONG,	&nUserType,          0, &cbUserType);
				::SQLGetData(pConn->hStmt,  8, SQL_C_LONG,	&nUserLoginState,    0, &cbUserLoginState);
				::SQLGetData(pConn->hStmt,  9, SQL_C_LONG,	&nUserAvailable,     0, &cbUserAvailable);
				::SQLGetData(pConn->hStmt,  10, SQL_C_LONG,	&nChaRemain,		 0, &cbChaRemain);
				::SQLGetData(pConn->hStmt,  11, SQL_C_LONG,	&nUserPoint,         0, &cbUserPoint);
				::SQLGetData(pConn->hStmt,  12, SQL_C_LONG,	&nUserPoint2,        0, &cbUserPoint2);
				::SQLGetData(pConn->hStmt,  13, SQL_C_LONG,	&nUserBlock,         0, &cbUserBlock);
				::SQLGetData(pConn->hStmt,  14, SQL_C_TYPE_TIMESTAMP, &sUserBlockDate, 0, &cbUserBlockDate);
				::SQLGetData(pConn->hStmt,  15, SQL_C_TYPE_TIMESTAMP, &sChatBlockDate, 0, &cbChatBlockDate);
				::SQLGetData(pConn->hStmt,  16, SQL_C_TYPE_TIMESTAMP, &sPremiumDate, 0, &cbPremiumDate);
				::SQLGetData(pConn->hStmt,  17, SQL_C_TYPE_TIMESTAMP, &sCreateDate, 0, &cbCreateDate);
				::SQLGetData(pConn->hStmt,  18, SQL_C_TYPE_TIMESTAMP, &sLastLoginDate, 0, &cbLastLoginDate);
				::SQLGetData(pConn->hStmt,  19, SQL_C_LONG,	&nSGNum,  0, &cbSGNum);
				::SQLGetData(pConn->hStmt,  20, SQL_C_LONG,	&nSVRNum, 0, &cbSVRNum);
				...
				if (cbUserSA != 0 && cbUserSA != -1)
					::StringCchCopy( pDATA->szSecurityKey, USR_PASS_LENGTH+1, (const char*) szSecurityKey);
				...
				pDATA->dwUserPoint2 = (DWORD) nUserPoint2;
...
int CGMToolOdbcBase::UserDataSave( SGMTOOL_DATA_USER* pDATA )
{
	...
	TCHAR szTemp[1024] = {0};
	_snprintf( szTemp, 1024, "UPDATE UserInfo SET UserPass ='%s', UserPass2 ='%s', UserEmail ='%s', UserSA ='%s',"
		"UserType = %u, UserLoginState = %u, UserAvailable = %u, ChaRemain = %u, UserPoint = %u, UserPoint2 = %u,"
		"UserBlock = %u, UserBlockDate = '%s', ChatBlockDate = '%s', PremiumDate = '%s' "
		"WHERE UserNum = %u ", 
		pDATA->szUserPass, pDATA->szUserPass2, pDATA->szUserEmail, pDATA->szSecurityKey
		, pDATA->wUserType, pDATA->bUserLoginState, pDATA->bUserAvailable, pDATA->wChaRemain, pDATA->dwUserPoint, pDATA->dwUserPoint2, pDATA->bUserBlock
