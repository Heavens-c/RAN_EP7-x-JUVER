...
HRESULT GLLandManClient::ClientUpdate(D3DXVECTOR3 myPos, D3DXVECTOR3 myDir,DWORD myPassenger,DWORD myID )
{
	PGLCHARCLIENT pChar = GLGaeaClient::GetInstance().GetChar( myPassenger );
	if( !pChar ) return S_FALSE;

	GLCharacter* pCharacter = GLGaeaClient::GetInstance().GetCharacter();
	if ( pCharacter )
	{
		if ( pChar->m_sVehicle.m_dwDriverID == myID )
		{
			pChar->SetPosition( pCharacter->GetPosition() );
			pChar->SetDirection ( myPos );
			if ( pCharacter->GetACTION() != GLAT_FALLING && pCharacter->GetACTION() != GLAT_DIE )
			pChar->SetACTION(pCharacter->GetACTION());
...
HRESULT GLLandManClient::ClientUpdate()
{
	GLCHARCLIENTNODE *pCharCur = m_GlobPCList.m_pHead;
	for ( ; pCharCur; pCharCur = pCharCur->pNext )
	{
		...
		if ( pChar->m_bPassenger )
		{
			if ( pChar->m_sVehicle.m_dwDriverID != 0 )
			{
				PGLCHARCLIENT pCharDriver = GLGaeaClient::GetInstance().GetChar( pChar->m_sVehicle.m_dwDriverID );
				if( !pCharDriver ) continue;
				if ( pCharDriver->GetPosition() == pChar->GetPosition() ) continue;
				pChar->SetDirection( pCharDriver->GetPosition() );
				if ( pChar->GetACTION() != GLAT_FALLING && pChar->GetACTION() != GLAT_DIE )
				pChar->SetACTION(pCharDriver->GetACTION());
