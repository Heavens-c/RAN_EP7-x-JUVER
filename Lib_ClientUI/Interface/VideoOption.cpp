...
void CBasicVideoOption::CreateSubControl ()
{
	...
	{	//	기본 텍스트		
		...
		pTextBox = CreateStaticControl ( "HWOPTION_VIDEO_CLICK_EFFECT_STATIC",m_pFont, dwFontColor, nAlign );
		pTextBox->SetOneLineText ( ID2GAMEWORD ( "HWOPTION_VIDEO_OPTION", 12 ) );

		pTextBox = CreateStaticControl ( "HWOPTION_VIDEO_TARGET_EFFECT_STATIC",m_pFont, dwFontColor, nAlign );
		pTextBox->SetOneLineText ( ID2GAMEWORD ( "HWOPTION_VIDEO_OPTION", 13 ) );

		pTextBox = CreateStaticControl ( "HWOPTION_VIDEO_MINE_EFFECT_STATIC",m_pFont, dwFontColor, nAlign );
		pTextBox->SetOneLineText ( ID2GAMEWORD ( "HWOPTION_VIDEO_OPTION", 14 ) );

		pTextBox = CreateStaticControl ( "HWOPTION_VIDEO_TEXFILTER_STATIC",m_pFont, dwFontColor, nAlign );
	...
	{	//	기능
		...
		//	콤보박스
		{
			...
			pComboBox = new CBasicComboBox;
			pComboBox->CreateSub ( this, "BASIC_COMBOBOX", UI_FLAG_XSIZE, HWOPTION_VIDEO_TEXTUREFILTER_COMBO_OPEN );
			pComboBox->CreateBaseComboBox ( "HWOPTION_VIDEO_TEXFILTER_COMBO_OPEN" );			
			RegisterControl ( pComboBox );
			m_pComboBoxTextureFilterOpen = pComboBox;
		}
		...
		//	롤 오버 ( 언제나 가장 마지막에 뿌려져야합니다. )
		{
			...
			pComboBoxRollOver = new CBasicComboBoxRollOver;
			pComboBoxRollOver->CreateSub ( this, "BASIC_COMBOBOX_ROLLOVER", UI_FLAG_XSIZE | UI_FLAG_YSIZE, HWOPTION_VIDEO_TEXTUREFILTER_COMBO_ROLLOVER );
			pComboBoxRollOver->CreateBaseComboBoxRollOver ( "HWOPTION_VIDEO_TEXFILTER_COMBO_ROLLOVER" );
			pComboBoxRollOver->SetVisibleSingle ( FALSE );
			RegisterControl ( pComboBoxRollOver );
			m_pComboBoxTextureFilterRollOver = pComboBoxRollOver;
		}
	}
}
...
void CBasicVideoOption::LoadCurrentOption()
{
	...
	LoadClickEffectButton();
	LoadTargetEffectButton();
	LoadMineEffectButton();
	LoadTextureFilter();
}
...
void CBasicVideoOption::LoadTextureFilter ()
{
	m_dwTextureFilter = DXPARAMSET::GetInstance().m_dwTextureFilter;
	SetTextTextureFilterCombo( m_dwTextureFilter );
}
...
{	
	...
	{	//texture filter
		m_pComboBoxTextureFilterRollOver->ClearText ();
		for ( int i = 0; i < RANPARAM::EMTEXTUREFILTER_NSIZE; i++ )
		{
			strTemp = ID2GAMEWORD ( "HWOPTION_VIDEO_TEXFILTER", i );
			m_pComboBoxTextureFilterRollOver->AddText ( strTemp );
		}
	}
	...
	{	//	해상도
		...
		{	
			...
			if ( dwWidth < 1024 )	continue;
			if ( dwHeight < 768 )	continue;
...
void CBasicVideoOption::SetTextTextureFilterCombo( DWORD dwIndex )
{
	if ( 0 <= dwIndex && dwIndex < RANPARAM::EMTEXTUREFILTER_NSIZE )
	{
		CString strTemp( ID2GAMEWORD ( "HWOPTION_VIDEO_TEXFILTER", dwIndex ) );
		m_pComboBoxTextureFilterOpen->SetText ( strTemp );
	}else{
		GASSERT ( 0 && "스킨 디테일 범위를 넘어섭니다." );
	}
}
