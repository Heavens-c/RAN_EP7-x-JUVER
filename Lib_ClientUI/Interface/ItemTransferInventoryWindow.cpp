...
CItemTransferInvenWindow::CItemTransferInvenWindow()
	: m_pPage( NULL )
	, m_nONE_VIEW_SLOT( 4 )
{
}
...
void CItemTransferInvenWindow::CreateSubControl()
{
	m_pPage = new CItemTransferInvenPage;
	m_pPage->CreateSub( this, "ITEM_TRANSFER_INVEN_PAGE", UI_FLAG_YSIZE, ITEM_TRANSFER_INVEN_PAGE );
	m_pPage->CreateSubControl();
	RegisterControl( m_pPage );

	CUIControl* pControl = CreateControl( "ITEM_TRANSFER_INVEN_MONEY_BACK" );
	pControl->SetAlignFlag( UI_FLAG_BOTTOM );

	CBasicButton* pButton = CreateFlipButton( "ITEM_TRANSFER_INVEN_MONEY_BUTTON", "ITEM_TRANSFER_INVEN_MONEY_BUTTON_F", ITEM_TRANSFER_INVEN_MONEY_BUTTON, CBasicButton::CLICK_FLIP );
	pButton->CreateMouseOver( "ITEM_TRANSFER_INVEN_MONEY_BUTTON_F" );
	pButton->SetUseDynamic( TRUE );
	pButton->SetAlignFlag( UI_FLAG_BOTTOM );

	CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
	m_pMoneyTextBox = CreateStaticControl( "ITEM_TRANSFER_INVEN_MONEYTEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
	m_pMoneyTextBox->SetAlignFlag( UI_FLAG_BOTTOM );
}

void CItemTransferInvenWindow::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
void CItemTransferInvenWindow::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	switch( ControlID )
	{
...
HRESULT CItemTransferInvenWindow::RestoreDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
void CItemTransferInvenWindow::SetOneViewSlot( const INT& nONE_VIEW_SLOT )
{
	CUIControl TempControl;
	TempControl.Create( 1, "ITEM_TRANSFER_INVENSLOT_ITEM_SLOT0" );

	CUIControl TempControl2;
	TempControl2.Create( 2, "ITEM_TRANSFER_INVENSLOT_ITEM_SLOT1" );
