...
protected:
	...
	TCHAR				 m_szRandomString[USR_CAPTCHA+2]; //userpanel
...
public:
	...
	//userpanel
	void SetUserPoint( USHORT uUserPoint );
	void SetBattlePoint( USHORT uBattlePoint );
	void SetUserEmail ( const TCHAR* szEmail );
 ...
 //   // 연결할 퍌드 서버의 정보를 설정한다
	...
	*/
	...
	//userpanel
	void SetRandomString(const TCHAR* szRandString );
	TCHAR* GetRandomString();
...
public:
	...
	/*register page, Juver, 2017/11/18 */
	bool	IsRegister();
	void	SetRegister( bool bSet );
	bool	IsRegisterWait();
	void	SetRegisterWait( bool bSet );
};
