	...
	if ( found!=m_listDISP_NAME.end() )
	{
		...
		if ( pDISP->DIFFERENT(sDISP_NAME) )
		{
			...
			//	Note : 새로 생성.
			{
				...
				if( sDISP_NAME.m_szBADGE[0] != _T('\0') )
					pDISP_NEW->SetName ( sDISP_NAME.m_szNAME, sDISP_NAME.m_szBADGE, sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY, sDISP_NAME.m_wCaptureTheFlagTeam );
				else
					pDISP_NEW->SetName ( sDISP_NAME.m_szNAME, "", sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY, sDISP_NAME.m_wCaptureTheFlagTeam );
...
void CNameDisplayMan::UpdateHold ( DWORD _dwCOUNT )
{
	...
	while ( pos!=end )
	{
		...
		if ( pDISP->GET_HIDENAME() )						bVISIBLE = FALSE;
