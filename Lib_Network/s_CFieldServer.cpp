...
int CFieldServer::Start()
{
	...
	CIPFilter::GetInstance()->AddIPKnown( CCfg::GetInstance()->GetAgentIP() );
...
void CFieldServer::PrintDebugMsg( DWORD dwUdpateTime )
{
	...
	{		
		...
		CConsoleMessage::GetInstance()->WriteStats( _T("INSTANCE %u %u Max:%u"), 
			dwNumInstance, dwNumInstance2, m_dwPeakInstance );

		StringCchPrintf( szLog, C_BUFFER_SIZE+1, _T("IPFILTER IPKnown :%u KnownRef:%I64u"),
			CIPFilter::GetInstance()->GetIPKnownNum(), CIPFilter::GetInstance()->GetRefKnown() );
		CDebugSet::ToLogFile( szLog );

		StringCchPrintf( szLog, C_BUFFER_SIZE+1, _T("IPFILTER IPBlock :%u BlockRef:%I64u"),
			CIPFilter::GetInstance()->GetIPBlockNum(), CIPFilter::GetInstance()->GetRefBlocked() );
		CDebugSet::ToLogFile( szLog );
		...
		StringCchPrintf( szLog, C_BUFFER_SIZE+1, _T("INSTANCE %u %u Max:%u"), 
			dwNumInstance, dwNumInstance2, m_dwPeakInstance );
		CDebugSet::ToLogFile( szLog );
...
void CFieldServer::IPFilterCleanIP( std::string _strIP )
{
