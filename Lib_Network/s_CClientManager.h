...
class CClientManager : public CLock
{
	...
	typedef std::set<DWORD>							SET_CLIENTID;
	typedef SET_CLIENTID::iterator					SET_CLIENTID_ITER;

	typedef std::map<std::string, SET_CLIENTID>		MAP_CLIENTIP;
	typedef MAP_CLIENTIP::iterator					MAP_CLIENTIP_ITER;

	typedef std::set<std::string>					SET_CLIENTIP;
	typedef SET_CLIENTIP::iterator					SET_CLIENTIP_ITER;
...
public:
	...
	//iplimit
	MAP_CLIENTIP	m_mapClientIP;
	SET_CLIENTIP	m_setExcludeIP;
	WORD			m_wClientIPMax;
	ULONGLONG		m_llDenyNum;
...
public:
	...
	//userpanel
	void		SetUserPoint (DWORD dwClient , USHORT	uUserPoints );
	void		SetBattlePoint (DWORD dwClient , USHORT	uBattlePoints );
	void		SetUserEmail (DWORD dwClient , const TCHAR* strEmail );

	
	int		GetUserPoint (DWORD dwClient );
	int		GetBattlePoint (DWORD dwClient );
	TCHAR*		GetUserEmail (DWORD dwClient );
	...
	//userpanel
	void SetRandomString(DWORD dwClient , const TCHAR* szRandString );

	//! 랜덤패스워드 번호를 가져온다.
	TCHAR* GetRandomString(DWORD dwClient);
	...
	//iplimit
public:
	void	IPSetClientMax( WORD _wMAX );
	BOOL	IPAllowConnect( SOCKET _s );
	BOOL	IPAllowConnect( std::string _strIP );
	void	IPAddClient( std::string _strIP, DWORD _dwClientID );
	void	IPRemoveClient( std::string _strIP, DWORD _dwClientID );
	void	IPRemove( std::string _strIP );
	BOOL	IPExcluded( std::string _strIP );
	void	IPAddExcluded( std::string _strIP );
	void	IPRemoveExcluded( std::string _strIP );
	void	IPDoCheck( std::string _strIP );
	DWORD	IPGetCount();
	DWORD	IPGetTotalClient();
	DWORD	IPGetExcludedCount();
	void	IPClear();
	ULONGLONG IPGetDenyNum();
};
