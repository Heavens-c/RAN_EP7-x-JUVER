...
int CSetVehicleBooster::Execute(CServer* pServer)
{
	return m_pDbManager->SetVehicleBooster( m_nVehicleNum, m_dwCharNum, m_bBooster );
}
...
CSetVehicleColor::CSetVehicleColor(	 DWORD dwClientID, DWORD dwCharNum, int nVehicleNum,  GLVEHICLE* pVehicle )
	: m_dwCharNum ( dwCharNum )
	, m_nVehicleNum(nVehicleNum)
{	
	m_dwClient = dwClientID;
	pVehicle->GETCOLORS_BYBUF( m_ByteStream );
}

int CSetVehicleColor::Execute(CServer* pServer)
{	
	return m_pDbManager->SetVehicleColor( m_nVehicleNum, m_dwCharNum, m_ByteStream);
}