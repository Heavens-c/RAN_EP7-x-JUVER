	...
	)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_InsertPet('%s',%d,%d,%d,%d,%d,%d,%d,%d,?)}",
		szPetName,
		nChaNum,
		nPetType,
		nPetMID,
	...
	const TCHAR* szPetName )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_RenamePet(%d,%d,'%s',?)}", nChaNum, nPetNum, szPetNameTemp ); 
	...
	int nPetNum )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_UpdatePetChaNum(%d,%d,?)}", nChaNum, nPetNum  );
	...
	int nPetColor)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_UpdatePetColor(%d,%d,%d,?)}", nChaNum, nPetNum, nPetColor );
	...
	int nPetStyle)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_UpdatePetStyle(%d,%d,%d,?)}", nChaNum, nPetNum, nPetStyle );
	...
	int nPetFull)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_UpdatePetFull(%d,%d,%d,?)}", nChaNum, nPetNum, nPetFull );
	...
	int nPetNum)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_DeletePet(%d,%d,?)}", nChaNum, nPetNum );
	...
	int nPetInvenAvailable)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_UpdatePetInven(%d,%d,%d,%d,%d,%d,%d,%d,?)}", 
		nChaNum,
		nPetNum,
		nPetInvenType,
		nPetInvenMID,
	...
	DWORD dwClientID)
{
	...
	while (true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
			...
			/*dual pet skill, Juver, 2017/12/29 */
			pPet->m_bDualSkill							= (BOOL)nDualSkill;

			VerifyTimeValid( "COdbcManager::GetPet", "sPetSkinStartDate", sPetSkinStartDate );
	...
	DWORD dwPetNum)
{
	...
	while (true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
			...
			/*dual pet skill, Juver, 2017/12/29 */
			if (nAvailable == 1)
				pPet->m_sActiveSkillID_A = PetSkill.sNativeID;

			if (nAvailable == 2)
				pPet->m_sActiveSkillID_B = PetSkill.sNativeID;
		}
		else
		{
			break;
		...
		DWORD dwPetNum )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_GetPetFull(%d,%u,?)}", nChaNum, dwPetNum );
	...
	std::vector<PET_RELIST> &vecPetReList )
{
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT PetNum, PetName, PetType, PetCardMID, PetCardSID "
...
{	
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_RestorePet(%u,%d,?)}", dwPetNum, nChaNum );
								 ...
								 int nPetSkinTime )
{
	...
	TCHAR szTemp[128];
	_snprintf( szTemp, 128, "{call sp_UpdatePetSkin(%d,%u,%d,%d,%d,%d,?)}",
		nChaNum,
		dwPetNum,
		nPetSkinMID,
		nPetSkinSID,
...
/*dual pet skill, Juver, 2017/12/29 */
int COdbcManager::SetPetDualSkill(
								  int nChaNum,
								  int nPetNum,
								  bool bDualSkill)
{
	if (nPetNum <= 0)
	{
		Print(_T("ERROR:COdbcManager::SetPetDualSkill"));
		return DB_ERROR;
	}

	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_UpdatePetDualSkill(%d,%d,%d,?)}", nChaNum, nPetNum, bDualSkill );
