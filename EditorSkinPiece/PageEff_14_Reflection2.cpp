// PageEff_14_Reflection2.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CPageEff_14_Reflection2, CPropertyPage)

CPageEff_14_Reflection2::CPageEff_14_Reflection2(LOGFONT logfont)
	: CPropertyPage(CPageEff_14_Reflection2::IDD)
	, m_pFont ( NULL )
	, m_pPiece( NULL )
	, m_pEff( NULL )
	, m_bADD( FALSE )
...
CPageEff_14_Reflection2::~CPageEff_14_Reflection2()
{
	SAFE_DELETE ( m_pFont );
}

void CPageEff_14_Reflection2::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPageEff_14_Reflection2, CPropertyPage)
	ON_BN_CLICKED(IDC_BUTTON_APPLY, OnBnClickedButtonApply)
	ON_BN_CLICKED(IDC_BUTTON_SAVE, OnBnClickedButtonSave)
	ON_BN_CLICKED(IDC_BUTTON_CLOSE, OnBnClickedButtonClose)
	ON_BN_CLICKED(IDC_BUTTON_CRF_COLOR, OnBnClickedButtonCrfColor)
	ON_BN_CLICKED(IDC_BUTTON_CRF_TEXTURE, OnBnClickedButtonCrfTexture)
...
// CPageEff_14_Reflection2 message handlers
BOOL CPageEff_14_Reflection2::OnInitDialog()
{
	...
	SetWin_Combo_Init( this, IDC_COMBO_CRF_COLOROP, GLOBAL_EFFTOOL::strEFFLEVEL_MODULATE, TOOL_EFFLEVEL_MODULATE_SIZE );
...
void CPageEff_14_Reflection2::DataSet( DxSkinPiece* pPiece, DxEffCharReflection2* pEff, BOOL bADD )
{
	m_pPiece = pPiece;
	m_pEff = pEff;
	m_bADD = bADD;
...
void CPageEff_14_Reflection2::ResetTool()
{
	SetWin_Combo_Sel( this, IDC_COMBO_CRF_COLOROP, 0 );
	SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_A, 1.0f );
	SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_R, 1.0f );
	SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_G, 1.0f );
	SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_B, 1.0f );
	SetWin_Text( this, IDC_EDIT_CRF_TEXTURE, "" );
...
void CPageEff_14_Reflection2::DataShow()
{
	if ( !m_pPiece )	return;
	if ( !m_pEff )		return;

	EFFCHAR_PROPERTY_REFLECTION2* pProp = (EFFCHAR_PROPERTY_REFLECTION2*)m_pEff->GetProperty();
	SetWin_Combo_Sel( this, IDC_COMBO_CRF_COLOROP, pProp->m_dwColorOP - 4 );
	SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_A, pProp->m_cSpecular.a );
	SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_R, pProp->m_cSpecular.r );
	SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_G, pProp->m_cSpecular.g );
	SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_B, pProp->m_cSpecular.b );
	SetWin_Text( this, IDC_EDIT_CRF_TEXTURE, pProp->m_szTexture );
}

BOOL CPageEff_14_Reflection2::DataSave()
{
	if ( !m_pPiece )	return FALSE;
	if ( !m_pEff )		return FALSE;

	std::string str = GetWin_Text( this, IDC_EDIT_CRF_TEXTURE );

	EFFCHAR_PROPERTY_REFLECTION2 sProperty;
	sProperty.m_dwColorOP = GetWin_Combo_Sel( this, IDC_COMBO_CRF_COLOROP) +4 ;
	sProperty.m_cSpecular.a = GetWin_Num_float( this, IDC_EDIT_CRF_COLOR_A );
	sProperty.m_cSpecular.r = GetWin_Num_float( this, IDC_EDIT_CRF_COLOR_R );
	sProperty.m_cSpecular.g = GetWin_Num_float( this, IDC_EDIT_CRF_COLOR_G );
	sProperty.m_cSpecular.b = GetWin_Num_float( this, IDC_EDIT_CRF_COLOR_B );
...
void CPageEff_14_Reflection2::MaterialShow()
{
...
void CPageEff_14_Reflection2::OnBnClickedButtonApply()
{
	DataSave();
}

void CPageEff_14_Reflection2::OnBnClickedButtonSave()
{
	if ( DataSave() )
	{
		m_pPiece = NULL;
...
void CPageEff_14_Reflection2::OnBnClickedButtonClose()
{
	if ( m_pSheetTab )
	{
		m_pPiece = NULL;
...
void CPageEff_14_Reflection2::OnBnClickedButtonCrfColor()
{
	float fColor_r = GetWin_Num_float( this, IDC_EDIT_CRF_COLOR_R );
	float fColor_g = GetWin_Num_float( this, IDC_EDIT_CRF_COLOR_G );
	float fColor_b = GetWin_Num_float( this, IDC_EDIT_CRF_COLOR_B );

	CSelColorDlg dlg( BYTE( fColor_r * 255.0f ), BYTE( fColor_g * 255.0f ), BYTE( fColor_b * 255.0f ) );
	if (dlg.DoModal () == IDOK)
	{
		COLORREF crNewColor = dlg.GetColor();
		SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_R, float( GetRValue( crNewColor ) / 255.0f ) );
		SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_G, float( GetGValue( crNewColor ) / 255.0f ) );
		SetWin_Num_float( this, IDC_EDIT_CRF_COLOR_B, float( GetBValue( crNewColor ) / 255.0f ) );
	}
}

void CPageEff_14_Reflection2::OnBnClickedButtonCrfTexture()
{
	...
	if ( dlg.DoModal() == IDOK )	
		SetWin_Text ( this, IDC_EDIT_CRF_TEXTURE, dlg.GetFileName().GetString() );
}

void CPageEff_14_Reflection2::OnNMDblclkListMaterial(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CPageEff_14_Reflection2::OnBnClickedButtonMaterialTexture()
{
	CString szFilter = "Texture Image (*.bmp,*.dds,*.tga,*.jpg)|*.bmp;*.dds;*.tga;*.jpg|";
	CFileDialog dlg(TRUE,".",NULL,OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
	dlg.m_ofn.lpstrInitialDir = TextureManager::GetTexturePath(); 
...
void CPageEff_14_Reflection2::OnBnClickedButtonMaterialOk()
{
	if ( !m_pPiece )	return;
	if ( !m_pEff )		return;
