	...
	case WM_KEYDOWN:
		{
			if ( pMsg->wParam == VK_ESCAPE )
			{
				...
				{
					m_PageEdit1.CrowClear();
					m_PageEdit2.CrowClear();
...
void CsheetWithTab::ActivePageEdit1( int nCurPage, PCROWDATA pCROW )
{
	m_PageEdit1.SetCallPage( nCurPage );
	m_PageEdit1.CrowSet( pCROW );
	SetActivePage ( PAGE_EDIT1 );
}

void CsheetWithTab::ActivePageEdit2( int nCurPage, PCROWDATA pCROW )
{
	m_PageEdit2.SetCallPage( nCurPage );
	m_PageEdit2.CrowSet( pCROW );
