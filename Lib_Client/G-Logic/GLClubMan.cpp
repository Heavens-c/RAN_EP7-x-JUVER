...
bool GLCLUB::SetMemberFlag ( DWORD dwCharID, DWORD dwFlags )
{
	...
	dwFlags = (dwFlags&EMCLUB_SUBMASTER_ALL);

	GLCLUBMEMBER* pMember = GetMember ( dwCharID );
	if ( !pMember )		return false;

	//	CDM 셋팅
	if ( !SetMemberCDMFlag( dwCharID, (dwFlags & EMCLUB_SUBMASTER_CDM) ) ) return false;

	//	Note : 선도클럽 인증 권한 유무.
	if ( dwFlags&EMCLUB_SUBMASTER_CD )
	{
		...
		if ( m_dwCDCertifior != 0 && m_dwCDCertifior != dwCharID )
		{
			...
			if ( pOldMember )
			{
				pOldMember->m_dwFlags &= ~EMCLUB_SUBMASTER_CD;
			}
		}
	...
	else if ( m_dwCDCertifior == dwCharID )
	{
		//	선도클럽 인증 권한 제거.
		m_dwCDCertifior = 0;
		pMember->m_dwFlags &= ~EMCLUB_SUBMASTER_CD;
	}
...
bool GLCLUB::IsSubMaster ( DWORD dwCharID )
{
	...
	return ( pMember->m_dwFlags&EMCLUB_SUBMASTER_ALL ) != NULL;
}
...
bool GLCLUB::IsMemberFlgJoin ( DWORD dwCharID )
{
	...
	return (pMember->m_dwFlags&EMCLUB_SUBMASTER_JOIN)!=NULL;
}
...
bool GLCLUB::IsMemberFlgKick ( DWORD dwCharID )
{
	...
	return (pMember->m_dwFlags&EMCLUB_SUBMASTER_KICK)!=NULL;
}
...
bool GLCLUB::IsMemberFlgNotice ( DWORD dwCharID )
{
	...
	return (pMember->m_dwFlags&EMCLUB_SUBMASTER_NOTICE)!=NULL;
}
...
bool GLCLUB::IsMemberFlgMarkChange ( DWORD dwCharID )
{
	...
	return (pMember->m_dwFlags&EMCLUB_SUBMASTER_MARK)!=NULL;
}
...
bool GLCLUB::IsMemberFlgCDM ( DWORD dwCharID )
{
	...
	return (pMember->m_dwFlags&EMCLUB_SUBMASTER_CDM)!=NULL;
}
...
HRESULT GLClubMan::FrameMoveAgent( float fTime, float fElapsedTime )
{
	...
	for ( ; pos!=end; )
	{
		...
		if ( cCLUB.ISCONFTING() )
		{
			...
			if ( bCONFT_END )
			{
				...
				if ( emEND_B==EMCONFRONT_END_CWIN )
				{
					...
					GLMSG::SNET_EMCONFRONT_END_CWIN NetMsgConftEndCwin;
					StringCchCopy ( NetMsgConftEndCwin.szMapName, CHAR_SZNAME, strCUR_MAP.c_str() );	
					StringCchCopy ( NetMsgConftEndCwin.szGuild1, CHAR_SZNAME, strCLUB_B.c_str() );	
					StringCchCopy ( NetMsgConftEndCwin.szGuild2, CHAR_SZNAME, strCLUB_A.c_str() );	
					StringCchCopy ( NetMsgConftEndCwin.szWinner, CHAR_SZNAME, strCLUB_B.c_str() );	
					cAgentServer.SENDTOALLCLIENT ( &NetMsgConftEndCwin );
				}
				else
				{
					/*strTEXT.Format ( ID2SERVERTEXT("EMCONFRONT_END_CDRAWN"),
						strCUR_MAP.c_str(), strCLUB_A.c_str(), strCLUB_B.c_str() );*/
					GLMSG::SNET_EMCONFRONT_END_CDRAWN NetMsgConftEndCDrawn;
					StringCchCopy ( NetMsgConftEndCDrawn.szMapName, CHAR_SZNAME, strCUR_MAP.c_str() );	
					StringCchCopy ( NetMsgConftEndCDrawn.szGuild1, CHAR_SZNAME, strCLUB_A.c_str() );	
					StringCchCopy ( NetMsgConftEndCDrawn.szGuild2, CHAR_SZNAME, strCLUB_B.c_str() );	
					cAgentServer.SENDTOALLCLIENT ( &NetMsgConftEndCDrawn );
				}
		...
		if ( cCLUB.ISCONFTING() )
		{
			...
			NetMsg.SETTEXT ( strTEXT.GetString() );
			cAgentServer.SENDTOALLCLIENT ( &NetMsg );*/
			GLMSG::SNET_EMCONFRONT_END_CDRAWN NetMsgCbOvrDraw;
			StringCchCopy ( NetMsgCbOvrDraw.szMapName, CHAR_SZNAME, strCUR_MAP.c_str() );	
			StringCchCopy ( NetMsgCbOvrDraw.szGuild1, CHAR_SZNAME, strCLUB_A.c_str() );	
			StringCchCopy ( NetMsgCbOvrDraw.szGuild2, CHAR_SZNAME, strCLUB_B.c_str() );	
			cAgentServer.SENDTOALLCLIENT ( &NetMsgCbOvrDraw );
		}
...
HRESULT GLClubMan::EndClubBattle( DWORD dwClubID_P, DWORD dwClubID_S )
{
	...
	if ( sClubBattle.m_wKillPoint == sClubBattle.m_wDeathPoint )
	{
		...
		GLMSG::SNET_CLUB_BATTLE_OVER_DRAW NetMsgCbOvrDraw;
		StringCchCopy ( NetMsgCbOvrDraw.szGuild1, CHAR_SZNAME, pClub_P->m_szName );	
		StringCchCopy ( NetMsgCbOvrDraw.szGuild2, CHAR_SZNAME, sClubBattle.m_szClubName );	
		cAgentServer.SENDTOALLCLIENT ( &NetMsgCbOvrDraw );
	...
	else if ( sClubBattle.m_wKillPoint > sClubBattle.m_wDeathPoint ) 
	{
		...
		GLMSG::SNET_CLUB_BATTLE_OVER_WIN NetMsgCbOvrWin1;
		StringCchCopy ( NetMsgCbOvrWin1.szGuild1, CHAR_SZNAME, pClub_P->m_szName );	
		StringCchCopy ( NetMsgCbOvrWin1.szGuild2, CHAR_SZNAME, sClubBattle.m_szClubName );	
		cAgentServer.SENDTOALLCLIENT ( &NetMsgCbOvrWin1 );
	...
	else
	{
		...
		GLMSG::SNET_CLUB_BATTLE_OVER_WIN NetMsgCbOvrWin2;
		StringCchCopy ( NetMsgCbOvrWin2.szGuild1, CHAR_SZNAME, sClubBattle.m_szClubName );	
		StringCchCopy ( NetMsgCbOvrWin2.szGuild2, CHAR_SZNAME, pClub_P->m_szName );	
		cAgentServer.SENDTOALLCLIENT ( &NetMsgCbOvrWin2 );
	...
	GLMSG::SNET_CLUB_BATTLE_RESULT NetMsgBattleResult1;
	StringCchCopy ( NetMsgBattleResult1.szString1, CHAR_SZNAME, pClub_P->m_szName );	
	NetMsgBattleResult1.dwWin = pClub_P->m_dwBattleWin;
	NetMsgBattleResult1.dwLose = pClub_P->m_dwBattleLose;
	NetMsgBattleResult1.dwDraw = pClub_P->m_dwBattleDraw;
	cAgentServer.SENDTOALLCLIENT ( &NetMsgBattleResult1 );

	GLMSG::SNET_CLUB_BATTLE_RESULT NetMsgBattleResult2;
	StringCchCopy ( NetMsgBattleResult2.szString1, CHAR_SZNAME, pClub_S->m_szName );	
	NetMsgBattleResult2.dwWin = pClub_S->m_dwBattleWin;
	NetMsgBattleResult2.dwLose = pClub_S->m_dwBattleLose;
	NetMsgBattleResult2.dwDraw = pClub_S->m_dwBattleDraw;
	cAgentServer.SENDTOALLCLIENT ( &NetMsgBattleResult2 );
...
HRESULT GLClubMan::EndAllianceBattle ( DWORD dwClubID_P, DWORD dwClubID_S )
{
	...
	if ( sClubBattle.m_wKillPoint == sClubBattle.m_wDeathPoint )
	{
		...
		GLMSG::SNET_ALLIANCE_BATTLE_OVER_DRAW NetMsgDraw;
		StringCchCopy ( NetMsgDraw.szParty1, CHAR_SZNAME, pClub_P->m_szName );	
		StringCchCopy ( NetMsgDraw.szParty2, CHAR_SZNAME, sClubBattle.m_szClubName );	
		cAgentServer.SENDTOALLCLIENT( &NetMsgDraw );
	...
	else if ( sClubBattle.m_wKillPoint > sClubBattle.m_wDeathPoint ) 
	{
		...
		GLMSG::SNET_ALLIANCE_BATTLE_OVER_WIN NetMsgWin;
		StringCchCopy ( NetMsgWin.szParty1, CHAR_SZNAME, pClub_P->m_szName );	
		StringCchCopy ( NetMsgWin.szParty2, CHAR_SZNAME, sClubBattle.m_szClubName );	
		cAgentServer.SENDTOALLCLIENT( &NetMsgWin );
	...
	else
	{
		...
		GLMSG::SNET_ALLIANCE_BATTLE_OVER_WIN NetMsgWin2;
		StringCchCopy ( NetMsgWin2.szParty1, CHAR_SZNAME, sClubBattle.m_szClubName );	
		StringCchCopy ( NetMsgWin2.szParty2, CHAR_SZNAME, pClub_P->m_szName );	
		cAgentServer.SENDTOALLCLIENT( &NetMsgWin2 );
	...
	GLMSG::SNET_ALLIANCE_BATTLE_RESULT NetMsgAllBatRes1;
	NetMsgAllBatRes1.dwWinner = pClub_P->m_dwAllianceBattleWin;
	NetMsgAllBatRes1.dwLose = pClub_P->m_dwAllianceBattleLose;
	NetMsgAllBatRes1.dwDraw = pClub_P->m_dwAllianceBattleDraw;
	StringCchCopy ( NetMsgAllBatRes1.szString1, CHAR_SZNAME, pClub_P->m_szName );	
	cAgentServer.SENDTOALLCLIENT( &NetMsgAllBatRes1 );

	GLMSG::SNET_ALLIANCE_BATTLE_RESULT NetMsgAllBatRes2;
	NetMsgAllBatRes2.dwWinner = pClub_S->m_dwAllianceBattleWin;
	NetMsgAllBatRes2.dwLose = pClub_S->m_dwAllianceBattleLose;
	NetMsgAllBatRes2.dwDraw = pClub_S->m_dwAllianceBattleDraw;
	StringCchCopy ( NetMsgAllBatRes2.szString1, CHAR_SZNAME, pClub_S->m_szName );	
	cAgentServer.SENDTOALLCLIENT( &NetMsgAllBatRes2 );
