                     ...
                     int       nCmdShow)
{
	...
	BUG_TRAP::BugTrapInstall( std::string(_T("ServerSession")));
...
void cmdStartOrStop(HWND hDlg)
{
	...
	{				
		g_pServer = new CSessionServer(hDlg, GetDlgItem(hDlg, IDC_EDITCONSOLE), GetDlgItem(hDlg, IDC_EDITCONSOLE_INFO), GetDlgItem(hDlg, IDC_EDITCONSOLE_STATS));
...
void cmdHideWindow(HWND hDlg)
{
	...
	lstrcpy(nid.szTip, "ServerSession");
