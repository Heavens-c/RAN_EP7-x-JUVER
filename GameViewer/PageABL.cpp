...
BEGIN_MESSAGE_MAP(CPageABL, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_BUTTON_PIECE_EFFSKIN_ABL, OnBnClickedButtonPieceEffskinAbl)
END_MESSAGE_MAP()
...
void CPageABL::DataReset()
{
	SetWin_Text( this, IDC_EDIT_FILENAME, "" );
	SetWin_Text( this, IDC_EDIT_MASKPIECE, "" );
...
{
	DxAttBone* pData = CGameViewerView::GetView()->m_pAttBone;
	if ( !pData )	return;

	SetWin_Text( this, IDC_EDIT_FILENAME, pData->GetFileName() );
	SetWin_Text( this, IDC_EDIT_MASKPIECE, pData->m_strMaskPiece.c_str() );

	DataShowAnim();
	DataShowPiece();
}

void CPageABL::DataShowAnim()
{
	m_List_Anim.DeleteAllItems();
...
void CPageABL::OnBnClickedButtonAbfLoad()
{
	...
	if ( dlg.DoModal() == IDOK )
	{
		...
		BOOL bLOAD = CGameViewerView::GetView()->DataLoadABL( strFile.c_str() );
		if ( bLOAD )
		{
			DataShow();
		}else{
...
void CPageABL::OnBnClickedButtonPieceCps()
{
	DxAttBone* pData = CGameViewerView::GetView()->m_pAttBone;
	if ( !pData )	return;
...
void CPageABL::OnBnClickedButtonPieceReset()
{
	DxAttBone* pData = CGameViewerView::GetView()->m_pAttBone;
	if ( !pData )	return;
...
void CPageABL::OnBnClickedButtonPieceEffskin()
{
	DxAttBone* pData = CGameViewerView::GetView()->m_pAttBone;
	if ( !pData )	return;
...
void CPageABL::OnBnClickedButtonAnimPlay()
{
	PlayAnim();
}

void CPageABL::OnNMDblclkListAnim(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CPageABL::OnBnClickedButtonPieceEffskinAbl()
{
	/*Item Self Effect, Juver, 2017/06/09 */
	DxAttBone* pData = CGameViewerView::GetView()->m_pAttBone;
	if ( !pData )	return;
