...
#include "../Lib_Client/G-Logic/GLCharAG.h"
#include "../Lib_Client/G-Logic/GLClubMan.h"
#include "../Lib_Client/G-Logic/GLGuidance.h"
#include "../Lib_Client/G-Logic/GLPet.h" // Pet Define
...
public:
	...
	*/
	...
	//itemmall
	int GetPoints ( int nChaNum , DWORD dwUserID , DWORD dwClientID , int* PPoints , int* VPoints );
	...
	*/
	...
	int SetPetDualSkill( int nChaNum, int nPetNum, bool bDualSkill );
	...
	*/
	...
	int GetPlayerRanking( PLAYER_RANKING_STATUS_MAP &m_pPlayerRanking );

	int GetVehicleColor( GLVEHICLE* pVehicle, int nVehicleNum, int nCharNum );
	int SetVehicleColor( int nVehicle, int nCharNum, CByteStream &ByteStream );	
    ...
    **************************************************************************/
	...
	void VerifyTimeValid( std::string strFunction, std::string strVariable, TIMESTAMP_STRUCT& time );

	/*register page, Juver, 2017/11/18 */
	int	UserRegister( const TCHAR* szUser, const TCHAR* szPass, const TCHAR* szPass2, const TCHAR* szSA, const TCHAR* szMail, int nSvrGrp, int nSvrNum );

	/*equipment lock, Juver, 2018/01/14 */
	int EquipmentLockEnable (DWORD dwChaNum, char* szChaName, char* szEPin );
	int EquipmentLockInput (DWORD dwChaNum, char* szChaName, char* szEPin );
	int EquipmentLockRecover (DWORD dwChaNum, char* szChaName, std::string& strEPin );
	int EquipmentLockRecoverChange (DWORD dwChaNum, char* szChaName, char* szEPin );
	int EquipmentLockRecoverDelete (DWORD dwChaNum, char* szChaName );

	/*inventory lock, DevArt22, 2020/02/11 */
	int InventoryLockEnable (DWORD dwChaNum, char* szChaName, char* szEPin );
	int InventoryLockInput (DWORD dwChaNum, char* szChaName, char* szEPin );

	/*storage lock, DevArt22, 2020/02/11 */
	int StorageLockEnable (DWORD dwChaNum, char* szChaName, char* szEPin );
	int StorageLockInput (DWORD dwChaNum, char* szChaName, char* szEPin );
	
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 GlobalRanking
	int GetTopKill(VEC_TOP_KILL& vecKill);
	int GetTopRich(VEC_TOP_RICH& vecRich);
	int GetTopGuild(VEC_TOP_GUILD& vecRich);
	/////////////////////////////////////////////////////////////////////////////
	
	/*re-coded LG-7 Global Ranking by DevArt22*/
	int GetTopKillBr(VEC_TOP_KILL_BR& vecKill);
	int GetTopKillSw(VEC_TOP_KILL_SW& vecKill);
	int GetTopKillAr(VEC_TOP_KILL_AR& vecKill);
	int GetTopKillSh(VEC_TOP_KILL_SH& vecKill);
	int GetTopKillEx(VEC_TOP_KILL_EX& vecKill);
	int GetTopKillSc(VEC_TOP_KILL_SC& vecKill);
	int GetTopKillAs(VEC_TOP_KILL_AS& vecKill);
};
