...
{	
	...
	if( RANPARAM::bFeatureDisplayCP )
		SetCP ( sCharData.m_sCombatPoint.wNow, sCharData.m_sCombatPoint.wMax ); /*combatpoint logic, Juver, 2017/05/28 */
}
	...
	case CP_TEXT:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if( RANPARAM::bFeatureDisplayCP )
				{
					const GLCHARLOGIC& sCharData = GLGaeaClient::GetInstance().GetCharacterLogic ();
					CString strText;
					strText.Format ( "%d/%d", sCharData.m_sCombatPoint.wNow, sCharData.m_sCombatPoint.wMax );
					CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO ( strText, NS_UITEXTCOLOR::DEFAULT );
				}
			}
		}
