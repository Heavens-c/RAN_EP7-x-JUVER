...
void GLPetField::UpdateSkillState_A ( float fElapsedTime )
{
	...
	{
		if ( m_pOwner->m_sPETSKILLFACT_A.emTYPE == SKILL::EMFOR_PET_PROTECTITEMS )
		{
		...
		GLMSG::SNETPET_REMOVE_SKILLFACT_A NetMsg;
		GLGaeaServer::GetInstance().SENDTOCLIENT ( m_pOwner->m_dwClientID, &NetMsg );
...
void GLPetField::UpdateSkillState_B ( float fElapsedTime )
{
	...
	{
		if ( m_pOwner->m_sPETSKILLFACT_B.emTYPE == SKILL::EMFOR_PET_PROTECTITEMS )
		{
			m_pOwner->m_bProtectPutOnItem = false;
		}
		...
		GLMSG::SNETPET_REMOVE_SKILLFACT_B NetMsg;
		GLGaeaServer::GetInstance().SENDTOCLIENT ( m_pOwner->m_dwClientID, &NetMsg );
