...
void CBasicQuickSkillSlotEx::CreateSubControl ( INT nTabIndex )
{
	...
	if ( m_pSkillBlock ) m_pSkillBlock->SetVisibleSingle ( FALSE ); /*prohibit skill logic, Juver, 2017/06/06 */
}
...
void CBasicQuickSkillSlotEx::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	BOOL bSkillBLOCK = BOOL( GLGaeaClient::GetInstance().GetCharacter()->m_bProhibitSkill || 
		GLGaeaClient::GetInstance().GetCharacter()->m_bCaptureTheFlagHoldFlag );
	...
	if ( m_pSkillBlock )
	{
		m_pSkillBlock->SetVisibleSingle ( bSkillBLOCK && !bSlotEmpty );
	}
}
