...
{	
	...
	//iplimit
	IPRemoveClient( m_pField[dwClient].GetIP(), dwClient );
...
bool CClientAgent::IsRegister(DWORD dwClient)
{
	if (dwClient >= (DWORD) m_nMaxClient)   return m_pClient[dwClient-m_nMaxClient].IsRegister();
	else                                    return m_pClient[dwClient].IsRegister();
}

/*register page, Juver, 2017/11/18 */
void CClientAgent::SetRegister(DWORD dwClient, bool bSet )
{
	if (dwClient >= (DWORD) m_nMaxClient)	m_pClient[dwClient-m_nMaxClient].SetRegister(bSet);
	else                                    m_pClient[dwClient].SetRegister(bSet);
}

/*register page, Juver, 2017/11/18 */
bool CClientAgent::IsRegisterWait(DWORD dwClient)
{
	if (dwClient >= (DWORD) m_nMaxClient)   return m_pClient[dwClient-m_nMaxClient].IsRegisterWait();
	else                                    return m_pClient[dwClient].IsRegisterWait();
}

/*register page, Juver, 2017/11/18 */
void CClientAgent::SetRegisterWait(DWORD dwClient, bool bSet )
{
	if (dwClient >= (DWORD) m_nMaxClient)	m_pClient[dwClient-m_nMaxClient].SetRegisterWait(bSet);
	else                                    m_pClient[dwClient].SetRegisterWait(bSet);
}