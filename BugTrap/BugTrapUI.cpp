 ...
 */
void GetDefaultMailSubject(PTSTR pszSubject)
{
	if (*g_szAppName)
		_stprintf(pszSubject, _T("\"%s\" Error Report"), g_szAppName);
	else
		_tcscpy(pszSubject, _T("Error Report"));
}
 ...
 */
void GetDefaultMailURL(PTSTR pszURLString)
{
	if (*g_szAppName)
		_stprintf(pszURLString, _T("mailto:%s?subject=%%22%s%%22%%20Error%%20Report"), g_szSupportEMail, g_szAppName);
	else
		_stprintf(pszURLString, _T("mailto:%s?subject=Error%%20Report"), g_szSupportEMail);
}
...
BOOL MailTempReport(HWND hwndParent)
{
	...
	GetDefaultMailSubject(szSubject);
...
#define CONVERT_INTERNET_STATE_TO_STRING(state) \
	...
	default:
		_stprintf(szInternetStatusBuffer, _T("0x%08lX"), dwInternetStatus);
	...
	if (dwInternetStatus == INTERNET_STATUS_STATE_CHANGE)
	{
		...
		default:
			_stprintf(szInternetStateBuffer, _T("0x%08lX"), *(PDWORD)pvStatusInformation);
...
BOOL SendTempReport(HWND hwndParent)
{
	...
	if (*g_szSupportHost && g_nSupportPort)
	{
		if (g_bShowUI)
		{
			bResult = DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_TRANSFERPROGRESS_DLG), hwndParent, TransferProgressDlgProc);
			EndDialog(hwndParent, FALSE);
		}
		else
		{
...
static BOOL CreateTempReport(void)
{
	...
	if (g_dwFlags & BTF_DETAILEDMODE)
	{
		...
		CreateTempFolder(g_szInternalReportFolder);
