// LockerPageItemP.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CLockerPageItemP, CPropertyPage)

CLockerPageItemP::CLockerPageItemP(LOGFONT logfont, SCHARDATA2* pData)
	: CPropertyPage(CLockerPageItemP::IDD)
	, m_pFont( NULL )
	, m_pData( NULL )
	, m_nLockerChannel(-1)
{
...
CLockerPageItemP::~CLockerPageItemP()
{
	SAFE_DELETE ( m_pFont );
}

void CLockerPageItemP::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CLockerPageItemP, CPropertyPage)
...
// CLockerPageItemP message handlers
BOOL CLockerPageItemP::OnInitDialog()
{
...
void CLockerPageItemP::ShowData()
{
...
void CLockerPageItemP::EditItem()
{
	if( !m_pData ){
		return;
	}
...
void CLockerPageItemP::OnBnClickedLockerPageButtonAdd()
{
	if ( !m_pData ){
		return;
	}
...
void CLockerPageItemP::OnBnClickedLockerPageButtonEdit()
{
	EditItem();
}

void CLockerPageItemP::OnBnClickedLockerPageButtonDelete()
{
	if ( !m_pData ){
		return;
	}
...
void CLockerPageItemP::OnBnClickedLockerPageButtonClear()
{
	if ( !m_pData ){
		return;
	}
...
void CLockerPageItemP::OnNMDblclkLockerPageList(NMHDR *pNMHDR, LRESULT *pResult)
{
