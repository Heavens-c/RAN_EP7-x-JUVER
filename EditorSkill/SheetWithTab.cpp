	 ...
	 m_pFont ( NULL )
{
	...
	m_PageTree.SetSheetTab ( this );
	m_PageEdit1.SetSheetTab ( this );
	m_PageEdit2.SetSheetTab ( this );
	...
	case WM_KEYDOWN:
		{
			...
			{
				if ( GetActivePage() != &m_PageTree )
				{
					m_PageEdit1.SkillClear();
					m_PageEdit2.SkillClear();
					ActivePageTree( FALSE );
				}
			}
		}break;
	}
...
void CsheetWithTab::ActivePageTree( bool bREFRESH )
{
	if ( bREFRESH )
	{
		m_PageTree.UpdateTree();
	}else{
		m_PageTree.UpdateName ();
	}

	SetActivePage ( PAGE_TREE );
}

void CsheetWithTab::ActivePageEdit1( int nCurPage, PGLSKILL pSKILL )
{
	m_PageEdit1.SetCallPage( nCurPage );
	m_PageEdit1.SkillSet( pSKILL );
	SetActivePage ( PAGE_EDIT1 );
}

void CsheetWithTab::ActivePageEdit2( int nCurPage, PGLSKILL pSKILL )
{
	m_PageEdit2.SetCallPage( nCurPage );
	m_PageEdit2.SkillSet( pSKILL );
	SetActivePage ( PAGE_EDIT2 );
}
