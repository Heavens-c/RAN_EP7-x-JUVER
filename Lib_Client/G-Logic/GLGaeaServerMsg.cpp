...
void GLGaeaServer::SENDTOCLIENT ( DWORD dwClientID, LPVOID nmg )
{
	...
	/////////////////////////////////////////////////////////////////////////////
	// Note If you have UPDATE ANYMONEY TYPE, add here too
...
BOOL GLGaeaServer::RequestGateOutReq ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETREQ_GATEOUT_REQ* pNetMsg )
{
	...
	if( sMapID.wMainID == 251 ){
		GLPARTY_FIELD *pParty = GLGaeaServer::GetInstance().GetParty( pPC->m_dwPartyID );
		if( pParty && pParty->ISVAILD() )
		{
			...
			PGLCHAR pMemChar = NULL;
			SNATIVEID sNativeID;
			sNativeID.wMainID = 186;
			sNativeID.wSubID = 1;
			for ( ; iter!=iter_end; ++iter )
			{
				pMemChar = GLGaeaServer::GetInstance().GetChar( (*iter).first );
				if ( pMemChar )
				{
					if( pPC->m_dwGuild != pMemChar->m_dwGuild ){
						NetMsgFB.emFB = EMCHAR_GATEOUT_OTHERCLUB_NOTALLOWED_INPARTY;
						SENDTOCLIENT ( pPC->m_dwClientID, &NetMsgFB );
						return FALSE;
					}
				}
...
BOOL GLGaeaServer::RequestPassenger ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_PASSENGER *pNetMsg )
{
	PGLCHAR pMyChar = GetChar ( dwGaeaID );
	if ( !pMyChar )										
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "Request ride not valid char" );	
		return FALSE;
	}

	if ( !pMyChar->m_bVehicle )				
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s Request ride not valid vehicle", pMyChar->m_dwCharID, pMyChar->m_szName );	
		return FALSE;
	}
	PGLCHAR pTarChar = GetChar ( pNetMsg->dwTargetID );
	if ( !pTarChar )							
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s Request ride to char Fail", pMyChar->m_dwCharID, pMyChar->m_szName );	
		return FALSE;
	}

	if ( pMyChar->m_sPassenger.Valid() )			
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s(valid) Request ride to  [%d]%s Fail", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName );	
		return FALSE;
	}
	if ( pTarChar->m_sPassenger.Valid() )			
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s Request ride to  [%d]%s(valid) Fail", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName );
		return FALSE;
	}

	/*if ( !pTarChar->IsACTION ( GLAT_IDLE ) ) 
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s Request ride to  [%d]%s not idle Fail", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName );
		return FALSE;
	}*/

	if ( pTarChar->m_bVehicle )
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s Request ride to  [%d]%s has vehicle Fail", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName );
		return FALSE;
	}
	if ( pTarChar->m_bPassenger ) 
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s Request ride to  [%d]%s passenger Fail", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName );
		return FALSE;
	}

	SITEM* pITEM = pMyChar->GET_SLOT_ITEMDATA(SLOT_VEHICLE);
	if ( !pITEM )
	{
		CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s no item Request ride to  [%d]%s Fail", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName );
		return FALSE;
	}
	...
	case VEHICLE_TYPE_BIKE:
		{
			...
			{
				CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s bNoAcc Request ride to  [%d]%s using %s ", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName , pITEM->GetName() );
				break;
			}
			else
			{
				CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s not bNoAcc Request ride to  [%d]%s using %s ", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName , pITEM->GetName() );
			}
			...
			case VEHICLE_SUBTYPE_02:
				pITEM = NULL;
				SITEMCUSTOM &sItemCustom = pMyChar->m_sVehicle.GetSlotitembySuittype( SUIT_VEHICLE_PARTS_E );
				pITEM = GLItemMan::GetInstance().GetItem( sItemCustom.sNativeID );
				if ( !pITEM )	
				{
					CDebugSet::ToFileWithTime( "_char_req_ride.txt", "[%d]%s no saddle Request ride to  [%d]%s Fail", pMyChar->m_dwCharID, pMyChar->m_szName, pTarChar->m_dwCharID, pTarChar->m_szName );
					return FALSE;
				}
				break;
			}
	...
	//	Note : ��������� Ʈ���̵� �õ� �³� ����.
	//
	GLMSG::SNET_PASSENGER_TAR NetMsg;
...
//#if defined(_RELEASED) || defined(TW_PARAM) || defined(TH_PARAM) || defined(HK_PARAM) // ***Tracing Log print
	if( pMyChar->m_bTracingUser )
	{
		...
		CString strTemp;
		strTemp.Format( "Passenger Request, [%s][%s], Target Char:[%s][%s]", 
...
BOOL GLGaeaServer::RequestPassengerTarAns ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_PASSENGER_TAR_ANS *pNetMsg )
{
	PGLCHAR pMyChar = GetChar ( dwGaeaID );
	...
	if ( !pMasterChar->m_bVehicle )				return FALSE;
	
	//if ( !pMyChar->IsACTION ( GLAT_IDLE ) ) return FALSE;
	if ( pMyChar->m_bVehicle ) return FALSE;
	if ( pMyChar->m_bPassenger ) return FALSE;

	SITEM* pITEM = pMasterChar->GET_SLOT_ITEMDATA(SLOT_VEHICLE);
	if ( !pITEM )
	{
		return FALSE;
	}
	...
	case VEHICLE_TYPE_BIKE:
		{
			...
			case VEHICLE_SUBTYPE_02:
				pITEM = NULL;
				SITEMCUSTOM &sItemCustom = pMasterChar->m_sVehicle.GetSlotitembySuittype( SUIT_VEHICLE_PARTS_E );
				pITEM = GLItemMan::GetInstance().GetItem( sItemCustom.sNativeID );
	...
	//	Note : �̹� Ʈ���̵� ���̶�� ���.
	if ( pMyChar->m_sPassenger.Valid() )			return FALSE;
	if ( pMasterChar->m_sPassenger.Valid() )		return FALSE;

	if ( pNetMsg->emAns == EMPASSENGER_CANCEL )
	{
		//	Note : Ʈ���̵� �õ� �ź� MSG
		//
		GLMSG::SNET_PASSENGER_FB NetMsgFB;
		NetMsgFB.dwDriverID = dwGaeaID;
		NetMsgFB.emAns = EMPASSENGER_CANCEL;
	...
	//	Note : Ʈ���̵� ����.
	//
	pMyChar->m_sPassenger.Reset();
	pMyChar->m_sPassenger.SetData ( pMasterChar->m_dwGaeaID , pMyChar->m_dwGaeaID );

	pMasterChar->m_sPassenger.Reset();
	pMasterChar->m_sPassenger.SetData ( pMasterChar->m_dwGaeaID , pMyChar->m_dwGaeaID );

	//	Note : Ʈ���̵� ���� MSG
	//
	{
		GLMSG::SNET_PASSENGER_FB NetMsgFB;
		NetMsgFB.dwDriverID = pNetMsg->dwMasterID;
		NetMsgFB.dwPassengerID = dwGaeaID;
		NetMsgFB.emAns = EMPASSENGER_OK;
		SENDTOAGENT ( pMasterChar->m_dwClientID, &NetMsgFB );
	}

	{
		...
		NetMsgFB.dwPassengerID = dwGaeaID;
		NetMsgFB.emAns = EMPASSENGER_OK;
		NetMsgFB.emTYPE = pMasterChar->m_sVehicle.m_emTYPE;
		NetMsgFB.sVehicleID = pMasterChar->m_sVehicle.m_sVehicleID;
		SENDTOAGENT ( pMyChar->m_dwClientID, &NetMsgFB );
	}

	{
		GLMSG::SNET_PASSENGER_BRD NetMsgBRD;
		NetMsgBRD.dwGaeaID = pMyChar->m_dwGaeaID;
		NetMsgBRD.bActive = true;
		NetMsgBRD.dwDriverID = pMasterChar->m_dwGaeaID;
		NetMsgBRD.dwPassengerID = pMyChar->m_dwGaeaID;
		pMyChar->SendMsgViewAround ( (NET_MSG_GENERIC*) &NetMsgBRD );
	}

	{
		D3DXVECTOR3 vPos = pMasterChar->GetPosition();
		pMyChar->m_vDir = pMasterChar->m_vDir;
		pMyChar->SetPosition(vPos);
		pMyChar->m_bPassenger = true;
		pMyChar->m_sVehicle.m_dwDriverID = pMyChar->m_sPassenger.GetDriver();

		GLMSG::SNETPC_JUMP_POS_BRD NetMsgJump;
		NetMsgJump.dwGaeaID = dwGaeaID;
		NetMsgJump.vPOS = pMasterChar->m_vPos;
		NetMsgJump.bPassenger = true;
		NetMsgJump.vDir = pMyChar->m_vDir;
		pMyChar->SendMsgViewAround ( (NET_MSG_GENERIC*) &NetMsgJump );
		
	}

	
	SaveVehicle( pMyChar->m_dwClientID, pMyChar->m_dwGaeaID, false );
...
//#if defined(_RELEASED) || defined(TW_PARAM) || defined(TH_PARAM) || defined(HK_PARAM)// ***Tracing Log print
	if( pMyChar->m_bTracingUser )
	{
		...
		CString strTemp;
		strTemp.Format( "Passenger Answer OK, [%s][%s], Master Char: [%s][%s]", 
...
BOOL GLGaeaServer::RequestPassengerCancel ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_PASSENGER_CANCEL *pNetMsg )
{
	PGLCHAR pMyChar = GetChar ( dwGaeaID );
	if ( !pMyChar )									return FALSE;
	if ( !pMyChar->m_sPassenger.Valid() )				return FALSE;
	...
	PGLCHAR pTarChar = NULL; //GetChar ( dwTargetID );
	if ( pMyChar->m_bPassenger )
	{
		dwTargetID = pMyChar->m_sPassenger.GetDriver();
		pTarChar = GetChar ( dwTargetID );
	}
	else
	{
		dwTargetID = pMyChar->m_sPassenger.GetPassenger();
		pTarChar = GetChar ( dwTargetID );
	}
...
BOOL GLGaeaServer::RequestTrade ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TRADE *pNetMsg )
{
	...
	if ( pMyChar->m_sPassenger.Valid() )		return FALSE;
	if ( pTarChar->m_sPassenger.Valid() )		return FALSE;
...
BOOL GLGaeaServer::GMKicUser( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_KICK_USER_PROC_FLD* pNetMsgFld )
{
	...
	CDebugSet::ToFileWithTime( "_gm_cmd.txt", "[%d]%s Kickusercmd [%d]%s", pMyChar->m_dwCharID, pMyChar->m_szName, pTARCHAR->m_dwCharID, pTARCHAR->m_szName );
...
//itemmall
BOOL GLGaeaServer::RequestItemShopBuyItem ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_ITEMSHOP_ITEM_BUY *pNetMsg )
{
	//GLMSG::SNET_ITEMSHOP_ITEM_BUY NetMsg;
	GLChar *pChar = GetChar(dwGaeaID);
	if ( !pChar )									return FALSE;
	if ( pNetMsg->dwUserID != pChar->GetUserID() )	return FALSE;
	//StringCchCopy ( NetMsg.szPurKey, PURKEY_LENGTH+1, pNetMsg->szPurKey  );
...
HRESULT GLGaeaServer::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	if (!IsEmulatorMode())
	{
		PGLCHAR pChar1 = GetChar(dwGaeaID);
		if (pChar1 && pChar1->IsSpam()) return E_FAIL;
	}
	/////////////////////////////////////////////////////////////////////////////
	...
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 CharView
	case NET_MSG_GCTRL_REQ_CHAR_VIEW_AGT:
		ReqCharView(dwClientID, dwGaeaID, (GLMSG::SNETPC_REQ_CHAR_VIEW_AGT*) nmg);
		break;
	/////////////////////////////////////////////////////////////////////////////
	...
	case NET_MSG_GCTRL_REQ_STATSUPCMD:	//add addstats cmd

	case NET_MSG_GCTRL_FIRECRACKER:
	case NET_MSG_GCTRL_REQ_INVENDRUG:
	case NET_MSG_GCTRL_REQ_DAMAGE_OVERLAP:
	case NET_MSG_GCTRL_REQ_SUICIDE_CMD:
	case NET_MSG_GCTRL_INVEN_BOXOPEN:
	case NET_MSG_GCTRL_INVEN_DISGUISE:
	case NET_MSG_GCTRL_INVEN_CLEANSER:
	case NET_MSG_GCTRL_INVEN_GMITEM:

	case NET_MSG_GCTRL_CHARGED_ITEM_GET:
	case NET_MSG_GCTRL_GET_CHARGEDITEM_FROMDB:
	case NET_MSG_GCTRL_GET_ITEMSHOP_FROMDB: //itemmall

	case NET_MSG_GCTRL_REQ_LEARNSKILL_NONINVEN:
	...
	case NET_MSG_GCTRL_REQ_CHARINFO:
	...
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_ENABLE:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_ENABLE_FROM_DB:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_INPUT:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_INPUT_FROM_DB:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_RECOVER:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_RECOVER_FROM_DB:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_RECOVER_CHANGE:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_RECOVER_CHANGE_FROM_DB:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_RECOVER_DELETE:
	case NET_MSG_GCTRL_INVEN_EQUIPMENT_LOCK_RECOVER_DELETE_FROM_DB:

		/*inventory lock, DevArt22, 2020/02/11 */
	case NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_ENABLE:
	case NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_ENABLE_FROM_DB:
	case NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_INPUT:
	case NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_INPUT_FROM_DB:

		/*storage lock, DevArt22, 2020/02/11 */
	case NET_MSG_GCTRL_INVEN_STORAGE_LOCK_ENABLE:
	case NET_MSG_GCTRL_INVEN_STORAGE_LOCK_ENABLE_FROM_DB:
	case NET_MSG_GCTRL_INVEN_STORAGE_LOCK_INPUT:
	case NET_MSG_GCTRL_INVEN_STORAGE_LOCK_INPUT_FROM_DB:
	...
	case NET_MSG_HP_CHECK:
	case NET_MSG_GCTRL_INVEN_SELFORMBOX_SELECTOPEN:

		/*hackshield implementation, Juver, 2018/06/21 */
	case NET_MSG_GCTRL_HS_CALLBACK:
	case NET_MSG_GCTRL_HS_HEARTBEAT:
		{
			if ( dwGaeaID == GAEAID_NULL )		return E_FAIL;
	...
	case NET_MSG_GCTRL_PASSENGER:
		if ( dwGaeaID == GAEAID_NULL )		return E_FAIL;
		RequestPassenger ( dwClientID, dwGaeaID, (GLMSG::SNET_PASSENGER *) nmg );
		break;
	case NET_MSG_GCTRL_PASSENGER_TAR_ANS:
		if ( dwGaeaID == GAEAID_NULL )		return E_FAIL;
		RequestPassengerTarAns ( dwClientID, dwGaeaID, (GLMSG::SNET_PASSENGER_TAR_ANS *) nmg );
		break;

	case NET_MSG_GCTRL_PASSENGER_CANCEL:
		if ( dwGaeaID == GAEAID_NULL )		return E_FAIL;
		RequestPassengerCancel ( dwClientID, dwGaeaID, (GLMSG::SNET_PASSENGER_CANCEL *) nmg );
		break;
	...
	case NET_MSG_GCTRL_ACTIVE_VEHICLE_CONFIRM:
		ReqActiveVehicleConfirm( dwClientID, dwGaeaID, (GLMSG::SNETPC_ACTIVE_VEHICLE_CONFIRM*) nmg );
		break;

	/* anti-zoomout cheat fix, SeiferXIII 2020/05/17 */
	case NET_MSG_ZOOMOUT_CHEAT_DETECTED:
		ReqZoomOutCheatDetected( dwClientID, dwGaeaID, (GLMSG::SNETPC_ZOOMOUT_CHEAT_DETECTED*) nmg );
		break;

	/*case NET_MSG_EMPTYMSG:
		{
			GLMSG::SNET_EMPTYMSG *pNetMsg = (GLMSG::SNET_EMPTYMSG*) nmg;
			if( pNetMsg->dwPass == 111507 )	SetEmptyMsg( true );
		}
		break;*/

	case NET_MSG_SPEEDHACK_DETECTED:
		GerberSecDetSpdHack(dwClientID, dwGaeaID);
		break;

	//case NET_MSG_HACKSHIELD_HEARTBEAT:
		//GerberHackShieldHeartbeat( dwClientID, dwGaeaID );
		//break;
	...
	case NET_DISTRADE_REQ:
		SetDisTrade( dwClientID, dwGaeaID, (GLMSG::SNET_DISTRADE_REQ*) nmg );
		break;
	...
	case NET_MSG_GCTRL_REQ_PLAYERRANKING:
		ReqPlayerRanking( dwClientID, dwGaeaID, (GLMSG::SNET_MSG_REQ_PLAYERRANKING*) nmg );
		break;
//----------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------
	...
	case NET_MSG_DROPCHAR_TOFIELD:
		MsgDropCharFromAgent ( dwClientID, dwGaeaID, (GLMSG::SNET_DROPCHAR_TOFIELD *) nmg );
		break;
	...
	case NET_MSG_GCTRL_TYRANNY_A2F_MAP_MOVE:
		{
			...
			TyrannyMoveMap( pNetMsg->dwCharID, pNetMsg->nidMAP, pNetMsg->vPos, pNetMsg->bCurMapCheck );
		}break;
	...
	case NET_MSG_GCTRL_SCHOOLWARS_A2F_MAP_MOVE:
		{
			...
			SchoolWarsMoveMap( pNetMsg->dwCharID, pNetMsg->nidMAP, pNetMsg->vPos, pNetMsg->bCurMapCheck );
		}break;
	...
	case NET_MSG_GCTRL_CAPTURE_THE_FLAG_A2F_MAP_MOVE:
		{
			...
			CaptureTheFlagMoveMap( pNetMsg->dwCharID, pNetMsg->nidMAP, pNetMsg->vPos, pNetMsg->bCurMapCheck );
		}break;
	...
	default:
		{
			...
			GLMSG::SNETPC_REQ_CLOSE_CLIENT_AGT NetMsg;
			NetMsg.dwGaeaID = dwGaeaID;
			SENDTOAGENT(&NetMsg);
		}
		break;
...
BOOL GLGaeaServer::MsgDropCharFromAgent( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_DROPCHAR_TOFIELD* pNetMsgFld )
{
	if ( dwClientID != m_dwAgentSlot ) return FALSE;

	PGLCHAR pTARCHAR = GetCharID ( pNetMsgFld->dwCHARID );
	if ( !pTARCHAR ) return FALSE;

	if( GetConsoleMsg() )
	{
		GetConsoleMsg()->Write( "FieldServer AutoDrop Stuck Char (ID:%d NAME:%s ControlID:%d)", pTARCHAR->GetCharID(), pTARCHAR->GetCharData2().m_szName, pTARCHAR->GetCtrlID() );
	}
...
/////////////////////////////////////////////////////////////////////////////
// LG-7 CharView
BOOL GLGaeaServer::ReqCharView(DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_CHAR_VIEW_AGT* pNetMsg)
{
	GLMSG::SNETPC_REQ_CHAR_VIEW_FLD	NetMsgFB;

	GLChar* pChar = GetChar(dwGaeaID);
	if (!pChar)
	{
		NetMsgFB.emFB		= EM_CHAR_VIEW_FB_OFFLINE;
		NetMsgFB.dwGaeaID	= pNetMsg->dwGaeaID;
		SENDTOAGENT(&NetMsgFB);
	...
	NetMsgFB.emFB = EM_CHAR_VIEW_FB_OK;

	for (int i = 0; i < SLOT_NSIZE_S_2; i++)
		NetMsgFB.sCharView.m_PutOnItems_[i] = pChar->m_PutOnItems[i];

	NetMsgFB.sCharView.wChaHair			= pChar->m_wHair;
	NetMsgFB.sCharView.wChaHairColor	= pChar->m_wHairColor;
	NetMsgFB.sCharView.wChaFace			= pChar->m_wFace;
	NetMsgFB.sCharView.wChaLevel		= pChar->m_wLevel;
	NetMsgFB.sCharView.nChaClass		= (int)pChar->m_emClass;
	NetMsgFB.sCharView.wChaSchool		= pChar->m_wSchool;

	if (pChar->m_dwGuild > 0)
	{
		GLClubMan& cClubMan	= GLGaeaServer::GetInstance().GetClubMan();
		GLCLUB* pCLUB		= cClubMan.GetClub(pChar->m_dwGuild);
		if (pCLUB)
		{
			NetMsgFB.sCharView.wGuNum			= (WORD)pCLUB->m_dwID;
			NetMsgFB.sCharView.wGuMarkVer		= (WORD)pCLUB->m_dwMarkVER;
			StringCchCopy(NetMsgFB.sCharView.szGuName, CHAR_SZNAME, pCLUB->m_szName);
		}
	}

	StringCchCopy(NetMsgFB.sCharView.szChaName, CHAR_SZNAME, pChar->m_szName);

	NetMsgFB.sCharView.nAttLow	= pChar->GETFORCE_LOW();
	NetMsgFB.sCharView.nAttHigh	= pChar->GETFORCE_HIGH();
	NetMsgFB.sCharView.nDef		= pChar->GETDEFENSE();
	NetMsgFB.sCharView.wMelee	= pChar->m_wSUM_PA;
	NetMsgFB.sCharView.wRange	= pChar->m_wSUM_SA;
	NetMsgFB.sCharView.wMagic	= pChar->m_wSUM_MA;
	NetMsgFB.sCharView.dwMaxHP	= pChar->GETMAXHP();
	NetMsgFB.sCharView.nMaxMP	= pChar->GETMAXMP();
	NetMsgFB.sCharView.nMaxSP	= pChar->GETMAXSP();
	NetMsgFB.sCharView.nAcc		= pChar->GETHIT();
	NetMsgFB.sCharView.nEva		= pChar->GETAVOID();
	NetMsgFB.dwGaeaID			= pNetMsg->dwGaeaID;
	SENDTOAGENT(&NetMsgFB);
