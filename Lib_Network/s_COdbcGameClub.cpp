...
{    
 ...
 //   strTemp << ",'" << strClubName.GetString() << "', ?)}";	
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_create_guild(%u,'%s',?)}",
							dwChaNum,
...
{    
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "UPDATE GuildInfo SET GuExpireTime='%s' WHERE GuNum=%u",
							strTime.GetString(),
							dwClub );
	...
	__time64_t tSec )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "UPDATE GuildInfo SET GuAllianceSec='%s' WHERE GuNum=%u",
							strTime.GetString(),
							dwClub );
	...
	__time64_t tDis )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "UPDATE GuildInfo SET GuAllianceDis='%s' WHERE GuNum=%u",
							strTime.GetString(),
							dwClub );
...
int COdbcManager::SetClubAuthorityTime(DWORD dwClub, __time64_t tAuthority)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "UPDATE GuildInfo SET GuAuthorityTime='%s' WHERE GuNum=%u",
							strTime.GetString(),
							dwClub );
...
int COdbcManager::GetClubMember(DWORD dwClub, std::vector<GLCLUBMEMBER> &vMember)
{
 ...
 //   strTemp << " ORDER BY ChaName";
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "SELECT ChaNum, ChaName, GuPosition FROM viewGuildMember "
...
// GuildInfo Table 컬럼 추가, viewGuildInfo 수정 필요
	...
	TCHAR szTemp[512] = {0};
	_snprintf( szTemp, 512, "SELECT GuNum, ChaNum, GuDeputy, ChaName, GuName, "
	...
	while (true)
	{
		...
		{	
			...
			VerifyTimeValid( "COdbcManager::GetClubInfo", "sMakeTime", sMakeTime );
			VerifyTimeValid( "COdbcManager::GetClubInfo", "sExpireTime", sExpireTime );
			VerifyTimeValid( "COdbcManager::GetClubInfo", "sAuthorityTime", sAuthorityTime );
			VerifyTimeValid( "COdbcManager::GetClubInfo", "sAllianceSec", sAllianceSec );
			VerifyTimeValid( "COdbcManager::GetClubInfo", "sAllianceDis", sAllianceDis );
			VerifyTimeValid( "COdbcManager::GetClubInfo", "sBattleLastTime", sBattleLastTime );
...
int COdbcManager::ReadClubMarkImage(DWORD dwClub, CByteStream &ByteStream)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "SELECT GuildInfo.GuMarkImage FROM GuildInfo where (GuNum=%u)", dwClub );
...
{    
 ...
 //   strTempVer << std::ends;
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "UPDATE GuildInfo SET GuMarkVer=%u WHERE GuNum=%u", dwMarkVer, dwClub );
	...
	_snprintf( szTemp, 128, "UPDATE GuildInfo SET GuMarkImage=? WHERE (GuNum=%u)", dwClub );
...
int COdbcManager::GetChaGuildNum(DWORD dwChaNum)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "SELECT GuNum FROM ChaInfo WHERE ChaNum=%u", dwChaNum );
...
LONGLONG COdbcManager::GetClubIncomeMoney(DWORD dwClub)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "SELECT GuIncomeMoney From GuildInfo WHERE GuNum=%u", dwClub );
...
LONGLONG COdbcManager::GetClubMoney(DWORD dwClub)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "SELECT GuMoney From GuildInfo WHERE GuNum=%u", dwClub );
...
int COdbcManager::ReadClubStorage(DWORD dwClub, CByteStream &ByteStream)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "SELECT GuildInfo.GuStorage FROM GuildInfo where (GuNum=%u)", dwClub );
	...
	int nSize )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "UPDATE GuildInfo SET GuStorage=? WHERE (GuNum=%u)", dwClub );
...
int COdbcManager::SetClubMasterFlags(DWORD dwClub, DWORD dwChaNum, DWORD dwSubMasterFlags)
{
 ...
 //   strTemp << std::ends;
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "UPDATE ChaInfo SET GuPosition=%u"
...
int COdbcManager::SetClubBattle( DWORD dwClubP, DWORD dwClubS, DWORD dwEndTime, int nAlliance )
{
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 128, "{call sp_RequestGuBattle(%u,%u,%u,%u,?)}", dwClubP, dwClubS, dwEndTime, nAlliance );
...
int COdbcManager::EndClubBattle( DWORD dwClubP, DWORD dwClubS, int nGuFlag, int nGuKillNum, int nGuDeathNum, bool bAlliance )
{
	...
	{
		TCHAR szTemp[128] = {0};
		_snprintf( szTemp, 128, "{call sp_EndGuAllianceBattle(%u,%u,%u,%u,%u,?)}", dwClubP, dwClubS, nGuFlag, nGuKillNum, nGuDeathNum );
	...
	{
		TCHAR szTemp[128] = {0};
		_snprintf( szTemp, 128, "{call sp_EndGuBattle(%u,%u,%u,%u,%u,?)}", dwClubP, dwClubS, nGuFlag, nGuKillNum, nGuDeathNum );
...
int COdbcManager::SaveClubBattle( DWORD dwClubP, DWORD dwClubS, int nGuKillNum, int nGuDeathNum )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_SaveGuBattle(%u,%u,%u,%u,?)}", dwClubP, dwClubS, nGuKillNum, nGuDeathNum );
...
int COdbcManager::ReSetAllianceBattle( DWORD dwClub )
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_ResetAllianceBattle(%u,?)}", dwClub );
...
int COdbcManager::GetClubBattleInfo( DWORD dwClub, std::vector<GLCLUBBATTLE> &vBattleInfo )
{
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT GuSNum, GuFlag, GuKillNum, GuDeathNum, GuBattleStartDate,"
	...
	while (true)
	{
 		...
 		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
			...
			VerifyTimeValid( "COdbcManager::GetClubBattleInfo", "sStartDate", sStartDate );
			VerifyTimeValid( "COdbcManager::GetClubBattleInfo", "sEndDate", sEndDate );
