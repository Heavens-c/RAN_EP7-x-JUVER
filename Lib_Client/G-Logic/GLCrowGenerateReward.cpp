...
void GLCrow::GenerateRewardItem(DWORD dwOWNER /* GAEAID_NULL */)
{
	...
	if ( m_pCrowData->m_sAction.m_dwActFlag&EMCROWACT_KNOCK )
	{
		...
		if ( pChar && szMAP_NAME )
		{
			...
			GLMSG::SNET_EMCROWACT_REPULSE NetMsgCrowRepulse;
			NetMsgCrowRepulse.nidMapID = m_sMapID;
			NetMsgCrowRepulse.nidBossID = m_pCrowData->sNativeID;
			//StringCchCopy ( NetMsgCrowRepulse.szMapName, CHAR_SZNAME, szMAP_NAME );	
			StringCchCopy ( NetMsgCrowRepulse.szKiller, CHAR_SZNAME, pChar->m_szName );	
			//StringCchCopy ( NetMsgCrowRepulse.szBossName, CHAR_SZNAME, m_pCrowData->GetName() );	
			GLGaeaServer::GetInstance().SENDTOALLCLIENT ( &NetMsgCrowRepulse );
		}
	}


	/*crow notice, Juver, 2017/11/25 */
	if ( m_pCrowData->m_dwNotice&EMCROW_NOTICE_DEFEAT ||
		m_pCrowData->m_dwNotice&EMCROW_NOTICE_DEFEAT2 )
	{
		const char *szMAP_NAME = GLGaeaServer::GetInstance().GetMapName ( m_sMapID );
		PGLCHAR pChar = GLGaeaServer::GetInstance().GetChar ( dwOWNER );
		if ( szMAP_NAME && pChar )
		{
			CString strKey;
			CString strTEXT;
			strKey.Format ( "CROW_DEFEAT_%03d_%03d", m_pCrowData->sNativeID.wMainID, m_pCrowData->sNativeID.wSubID );
			strTEXT.Format ( ID2SERVERTEXT(strKey.GetString()), m_pCrowData->GetName(), pChar->m_szName, szMAP_NAME );

			GLMSG::SNET_SERVER_GENERALCHAT NetMsg;
			NetMsg.SETTEXT ( strTEXT.GetString() );
			//NetMsg.bUseAdminDisplay = BOOL( m_pCrowData->m_dwNotice&EMCROW_NOTICE_DEFEAT2 );
