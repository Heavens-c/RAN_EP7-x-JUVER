...
/////////////////////////////////////////////////////////////////////////////
// LG-7 CharView
#include "../Lib_ClientUI/Interface/CharViewWindow/CharViewWindow.h"
/////////////////////////////////////////////////////////////////////////////
...
void GLCharacter::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_KILL_AGT:
	{
		...
		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopKill_[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingKill(pNetMsg->wPart, pNetMsg->sTopKill_[i], i);
	}
	break;

	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_BR_AGT:
	{
		GLMSG::SNETPC_REQ_GLOBAL_RANKING_BR_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_BR_AGT*)nmg;

		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopKill_Br[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingKillBr(pNetMsg->wPart, pNetMsg->sTopKill_Br[i], i);
	}
	break;

	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_SW_AGT:
	{
		GLMSG::SNETPC_REQ_GLOBAL_RANKING_SW_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_SW_AGT*)nmg;

		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopKill_Sw[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingKillSw(pNetMsg->wPart, pNetMsg->sTopKill_Sw[i], i);
	}
	break;

	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_AR_AGT:
	{
		GLMSG::SNETPC_REQ_GLOBAL_RANKING_AR_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_AR_AGT*)nmg;

		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopKill_Ar[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingKillAr(pNetMsg->wPart, pNetMsg->sTopKill_Ar[i], i);
	}
	break;

	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_SH_AGT:
	{
		GLMSG::SNETPC_REQ_GLOBAL_RANKING_SH_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_SH_AGT*)nmg;

		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopKill_Sh[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingKillSh(pNetMsg->wPart, pNetMsg->sTopKill_Sh[i], i);
	}
	break;

	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_EX_AGT:
	{
		GLMSG::SNETPC_REQ_GLOBAL_RANKING_EX_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_EX_AGT*)nmg;

		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopKill_Ex[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingKillEx(pNetMsg->wPart, pNetMsg->sTopKill_Ex[i], i);
	}
	break;

	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_SC_AGT:
	{
		GLMSG::SNETPC_REQ_GLOBAL_RANKING_SC_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_SC_AGT*)nmg;

		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopKill_Sc[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingKillSc(pNetMsg->wPart, pNetMsg->sTopKill_Sc[i], i);
	}
	break;

	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_AS_AGT:
	{
		GLMSG::SNETPC_REQ_GLOBAL_RANKING_AS_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_AS_AGT*)nmg;

		if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
			for (int i = 0; i < 10; i++)
				if (pNetMsg->sTopKill_As[i].dwChaNum > 0)
					CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingKillAs(pNetMsg->wPart, pNetMsg->sTopKill_As[i], i);
	}
	break;
	...
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 CharView
	case NET_MSG_GCTRL_REQ_CHAR_VIEW_FB:
	{
		GLMSG::SNETPC_REQ_CHAR_VIEW_FB* pNetMsg = (GLMSG::SNETPC_REQ_CHAR_VIEW_FB*)nmg;

		if (pNetMsg->emFB > EM_CHAR_VIEW_FB_NONE)
		{
			switch (pNetMsg->emFB)
			{
			case EM_CHAR_VIEW_FB_DELAY:
			{
				CString strTemp;
				strTemp.Format((char*)ID2GAMEWORD("CHAR_VIEW_STAGE", 0), pNetMsg->fDelay);

				DoModal(strTemp);
			}
			break;

			case EM_CHAR_VIEW_FB_OFFLINE:
			{
				CString strTemp;
				strTemp.Format((char*)ID2GAMEWORD("CHAR_VIEW_STAGE", 1));

				DoModal(strTemp);
			}
			break;

			case EM_CHAR_VIEW_FB_OK:
			{
				CInnerInterface::GetInstance().GetCharViewWindow()->AddCharView(pNetMsg->sCharView);
				CInnerInterface::GetInstance().SetCharViewWindowOpen(TRUE);
			}
			break;
			}
		}
...
// ����ġȸ��_����_Npc
	...
	case NET_MSG_GCTRL_INVEN_SELFORMBOX_SELECTOPEN_FB:
	{
		GLMSG::SNET_INVEN_SELFORMBOX_SELECTOPEN_FB* pNetMsgFB = (GLMSG::SNET_INVEN_SELFORMBOX_SELECTOPEN_FB*)nmg;
		switch ( pNetMsgFB->emFB )
		{
		case EMINVEN_SELFORMBOX_FB_OK:
		case EMINVEN_SELFORMBOX_FB_INVENNOSPACE:
		case EMINVEN_SELFORMBOX_FB_FAIL:
			CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEWORD("SELFORMBOX_MSG", (int)pNetMsgFB->emFB));
			break;
		}

		CInnerInterface::GetInstance().HideGroup( SELECTIVEFORMBOX_WINDOW );
	}
	break;
...
//				CInnerInterface::GetInstance().WAITSERVER_DIALOGUE_OPEN ( ID2GAMEINTEXT("CHINA_GAIN_CLOSEGAME"), WAITSERVER_CLOSEGAME, 30 );
	...
	case NET_MSG_GM_LIMIT_EVENT_END_FB:
		{
			//CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, "�̺�Ʈ ����!!" );
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	case NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_ENABLE_FB:
		{
			GLMSG::SNETPC_INVEN_INVENTORY_LOCK_ENABLE_FB* pNetMsg = ( GLMSG::SNETPC_INVEN_INVENTORY_LOCK_ENABLE_FB* ) nmg;
		...
		/*inventory lock, DevArt22, 2020/02/11 */
	case NET_MSG_GCTRL_INVEN_INVENTORY_LOCK_INPUT_FB:
		{
			GLMSG::SNETPC_INVEN_INVENTORY_LOCK_INPUT_FB* pNetMsg = ( GLMSG::SNETPC_INVEN_INVENTORY_LOCK_INPUT_FB* ) nmg;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	case NET_MSG_GCTRL_INVEN_STORAGE_LOCK_ENABLE_FB:
		{
			GLMSG::SNETPC_INVEN_STORAGE_LOCK_ENABLE_FB* pNetMsg = ( GLMSG::SNETPC_INVEN_STORAGE_LOCK_ENABLE_FB* ) nmg;
		...
		/*storage lock, DevArt22, 2020/02/11 */
	case NET_MSG_GCTRL_INVEN_STORAGE_LOCK_INPUT_FB:
		{
			GLMSG::SNETPC_INVEN_STORAGE_LOCK_INPUT_FB* pNetMsg = ( GLMSG::SNETPC_INVEN_STORAGE_LOCK_INPUT_FB* ) nmg;
