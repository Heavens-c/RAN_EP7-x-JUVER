...
#include "./G-Logic/GLQUEST.h"
#include "./G-Logic/GLQUESTMAN.h"
...
#include "../Lib_Engine/DxCommon/DxWeatherMan.h"
...
#include "../Lib_Engine/DxCommon/DxSkyMan.h"
#include "../Lib_Engine/DxCommon/DxDynamicVB.h"	// 삭제 해야한다.
...
#include "../Lib_Client/G-Logic/GLPVPSchoolWarsClient.h"
...
HRESULT DxGameStage::Render ()
{
	...
	PROFILE_BEGIN2("DxEffExtra::Render");
	DxEffExtra::GetInstance().Render ( m_pd3dDevice );
	PROFILE_END2("DxEffExtra::Render");
...
void DxGameStage::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_LOBBY_CHAR_JOIN:
		{
			...
			/*activity system, Juver, 2017/10/30 */
			m_CharJoinData.m_dwNumActivityProg = pCharJoin->dwNumActivityProg;
			m_CharJoinData.m_dwNumActivityDone = pCharJoin->dwNumActivityDone;
	...
	case NET_MSG_LOBBY_CHAR_ITEMFOOD:
		{
			GLMSG::SNETLOBBY_ITEMFOOD *pNetMsg = (GLMSG::SNETLOBBY_ITEMFOOD *) nmg;

			for( int i=0; i<FITEMFACT_SIZE; ++i )
			{
				m_CharJoinData.m_CharData2.m_sFITEMFACT[i] = pNetMsg->sFITEMFACT[i];
			}

			m_CharJoinData.m_bReceiveItemFood = true;

			CDebugSet::ToListView ( "NET_MSG_LOBBY_CHAR_ITEMFOOD" );
			if ( m_CharJoinData.CheckReceive() )
			{
				NET_MSG_GENERIC nmg = { sizeof(NET_MSG_GENERIC), NET_MSG_LOBBY_GAME_COMPLETE };
				COuterInterface::GetInstance().MsgProcess ( &nmg );
	...
	case NET_MSG_LOBBY_CHAR_ACTIVITY_PROG:
		{
			GLMSG::SNETLOBBY_ACTIVITY_PROG *pNetMsg = (GLMSG::SNETLOBBY_ACTIVITY_PROG *) nmg;

			m_CharJoinData.m_dwReceiveActivityProg += pNetMsg->dwNum;
			for ( DWORD i=0; i<pNetMsg->dwNum; ++i )
			{
				m_CharJoinData.m_CharData2.m_mapActivityProg.insert ( std::make_pair ( pNetMsg->sActivity[i].dwActivityID, pNetMsg->sActivity[i] ) );
			}

			CDebugSet::ToListView ( "NET_MSG_LOBBY_CHAR_ACTIVITY_PROG" );
			if ( m_CharJoinData.CheckReceive() )
			{
				NET_MSG_GENERIC nmg = { sizeof(NET_MSG_GENERIC), NET_MSG_LOBBY_GAME_COMPLETE };
				COuterInterface::GetInstance().MsgProcess ( &nmg );
	...
	case NET_MSG_LOBBY_CHAR_ACTIVITY_DONE:
		{
			GLMSG::SNETLOBBY_ACTIVITY_DONE *pNetMsg = (GLMSG::SNETLOBBY_ACTIVITY_DONE *) nmg;

			m_CharJoinData.m_dwReceiveActivityDone += pNetMsg->dwNum;
			for ( DWORD i=0; i<pNetMsg->dwNum; ++i )
			{
				m_CharJoinData.m_CharData2.m_mapActivityDone.insert ( std::make_pair ( pNetMsg->sActivity[i].dwActivityID, pNetMsg->sActivity[i] ) );
			}

			CDebugSet::ToListView ( "NET_MSG_LOBBY_CHAR_ACTIVITY_DONE" );
			if ( m_CharJoinData.CheckReceive() )
			{
				NET_MSG_GENERIC nmg = { sizeof(NET_MSG_GENERIC), NET_MSG_LOBBY_GAME_COMPLETE };
				COuterInterface::GetInstance().MsgProcess ( &nmg );
