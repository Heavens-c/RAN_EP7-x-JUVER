...
void CBasicHWOptionWindow::SetVisibleSingle ( BOOL bVisible )
{
	...
	if( bVisible )
	{
		...
		if( m_bFirstSetting )
		{
			...
			GLGaeaClient::GetInstance().GetCharacter()->ReqPrivateStats( RANPARAM::bPrivateStats );
		}
...
void CBasicHWOptionWindow::GamePlayOption_OK()
{
	...
	DXPARAMSET::GetInstance().m_bDIS_RIDE		= m_pGamePlayOption->m_bRide;
	...
	RANPARAM::bCharacterSimple  = m_pGamePlayOption->m_bCharacterSimple;	/*character simple, Juver, 2017/10/01 */
	RANPARAM::bHideSkillEffect	= m_pGamePlayOption->m_bHideSkillEffect;	/*skill effect setting, Juver, 2017/10/01 */
	RANPARAM::bPrivateStats		= m_pGamePlayOption->m_bPrivateStats;		/*charinfoview , Juver, 2017/11/12 */
	RANPARAM::bShowRankName		= m_pGamePlayOption->m_bRankName; //MMR Rank
	RANPARAM::bShowRankMark		= m_pGamePlayOption->m_bRankMark; //MMR Rank

	GLGaeaClient::GetInstance().SetCharacterSimple( RANPARAM::bCharacterSimple );	/*character simple, Juver, 2017/10/01 */

	GLGaeaClient::GetInstance().GetCharacter()->ReqNonRebirth( RANPARAM::bNON_Rebirth );

	GLGaeaClient::GetInstance().GetCharacter()->ReqPrivateStats( RANPARAM::bPrivateStats );	/*charinfoview , Juver, 2017/11/12 */
	GLGaeaClient::GetInstance().GetCharacter()->ReqRankName( RANPARAM::bShowRankName ); //MMR Rank
	GLGaeaClient::GetInstance().GetCharacter()->ReqRankMark( RANPARAM::bShowRankMark ); //MMR Rank
...
void CBasicHWOptionWindow::VideoOption_OK()
{
	...
	DXPARAMSET::GetInstance().m_bFrameLimit		= FALSE;
