...
#include "../../Lib_Engine/NaviMesh/NavigationMesh.h"
#include "../../Lib_Engine/DxCommon/DxWeatherMan.h"
#include "../../Lib_Engine/DxOctree/CollisionMap.h"
...
struct SSKILLACTEX
{
	...
	bool		bDebuff; /*debuff skill logic, Juver, 2017/06/10 */
	...
	SSKILLACTEX () :
		...
		bDebuff(false), /*debuff skill logic, Juver, 2017/06/10 */
	...
	bool VALID() const
	{
		...
		if ( bDebuff == true)					return true; /*debuff skill logic, Juver, 2017/06/10 */
		if ( bDash == true)						return true; /*dash skill logic, Juver, 2017/06/17 */
...
{
	enum{ INSTANTMAP_DELETE_TIME = 60, INSTANT_MAP_CHAR_DROP_TIME = 10 }; //30초
...
public:
	BOOL LoadFile ( const char *szFile, const char *szWLDFile = NULL );
...
public:
	...
	BOOL DropItem ( D3DXVECTOR3 &vPos, SITEMCUSTOM *pItemCustom, EMGROUP emGroup=EMGROUP_ONE, DWORD dwHoldGID=GAEAID_NULL, BOOL bMobGen = FALSE );
...
public:
	...
	/*push pull skill logic, Juver, 2017/06/04 */
	void PushAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fDISTANCE );
	void PullAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED );

	/*dash skill logic, Juver, 2017/06/17 */
	void DashAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED, const WORD _wActionAnim );
...
public:
	/*pvp tyranny, Juver, 2017/07/16 */
	void ResetTyrannyTower();
