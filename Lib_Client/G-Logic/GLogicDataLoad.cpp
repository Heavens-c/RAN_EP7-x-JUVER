...
//		cFILE.getflag( "fDISPRICE", 1, 1, fDISPRICE );
		...
		for ( i=0; i<GLCI_NUM_8CLASS; ++i )
			cFILE.getflag( "fCHAR_HEIGHT", i+1, GLCI_NUM_8CLASS, fCHAR_HEIGHT[i] );
		...
		//userpanel
		cFILE.getflag( "wGameTimeDiv", 1, 1, wGameTimeDiv );
		cFILE.getflag( "wGameTimeRes", 1, 1, wGameTimeRes );
		...
		fEXP_SCALE_BACKUP = fEXP_SCALE;
		fITEM_DROP_SCALE_BACKUP = fITEM_DROP_SCALE;
		fMONEY_DROP_SCALE_BACKUP = fMONEY_DROP_SCALE;
		...
		BYTE cA, cR, cG, cB;
		BYTE cA1, cR1, cG1, cB1;	//gm color name - devart22
		...
		//gm color name - devart22 
		cFILE.getflag( "GM_NORMAL_NAME_COLOR", 1, 4, cA1 );
		cFILE.getflag( "GM_NORMAL_NAME_COLOR", 2, 4, cR1 );
		cFILE.getflag( "GM_NORMAL_NAME_COLOR", 3, 4, cG1 );
		cFILE.getflag( "GM_NORMAL_NAME_COLOR", 4, 4, cB1 );
		dwGM_NORMAL_NAME_COLOR = D3DCOLOR_RGBA(cR1,cG1,cB1,cA1);
		...
		const char* szSETFILE[GLCI_NUM_8CLASS] = 
		{
			"BRAWLER_M.SETFILE",
			"SWORDSMAN_M.SETFILE",
			"ARCHER_W.SETFILE",
		...
		for ( i=0; i<wSCHOOLNUM; ++i )
		{
			for ( WORD j=0; j<GLCI_NUM_8CLASS; ++j )
			{
				...
				cFILE.getflag( strKEY.GetString(), 1+j, GLCI_NUM_8CLASS, strCLASS_INIT[i][j] );	
		...
		const char* szSKILFILE[GLCI_NUM_8CLASS] = 
		{
			"BRAWLER_M.SKINFILE",
			"SWORDSMAN_M.SKINFILE",
			"ARCHER_W.SKINFILE",
			"SHAMAN_W.SKINFILE",
			"EXTREME_M.SKINFILE",
			"EXTREME_W.SKINFILE",
			"BRAWLER_W.SKINFILE",
			"SWORDSMAN_W.SKINFILE",
			"ARCHER_M.SKINFILE",
			"SHAMAN_M.SKINFILE",
			"GUNNER_M.SKINFILE",
			"GUNNER_W.SKINFILE",
		...
		cFILE.getflag( "strCOMPLETE_NPC_EFFECT",	1, 1, strCOMPLETE_NPC_EFFECT );

		/*combatpoint logic, Juver, 2017/05/27 */
		cFILE.getflag( "wCombatPoint_MAX",	1, 1, wCombatPoint_MAX );
		cFILE.getflag( "wCombatPoint_PK",	1, 1, wCombatPoint_PK );

		/*vehicle booster system, Juver, 2017/08/11 */
		cFILE.getflag( "fVehicleBoosterChargeTime",	1, 1, fVehicleBoosterChargeTime );
		cFILE.getflag( "fVehicleBoosterBoostTime",	1, 1, fVehicleBoosterBoostTime );
		cFILE.getflag( "fVehicleBoosterAddSpeed",	1, 1, fVehicleBoosterAddSpeed );

		/*Money Luck, Juver, 2017/08/24 */
		cFILE.getflag( "fMONEY_LUCK_RATE",	1, 1, fMONEY_LUCK_RATE );

		/*pvp tyranny, Juver, 2017/08/25 */
		cFILE.getflag( "TYRANNY_FILE",	1, 1, strTyrannyFile );

		for ( i=0; i<TYRANNY_SCHOOL_SIZE; ++i )
		{
			CString strschooleff;
			strschooleff.Format ( "strSCHOOLEFF%02d", i );
			cFILE.getflag( strschooleff.GetString(), 1, 1, strSchoolEffect[i] );
		}

		/*quest rate, Juver, 2017/09/28 */
		cFILE.getflag( "QUEST_RATE_EXP",	1, 1, fQuestExpRate );
		cFILE.getflag( "QUEST_RATE_MONEY",	1, 1, fQuestMoneyRate );

		/*character simple, Juver, 2017/10/01 */
		for ( i=0; i<SLOT_WEAR_SIMPLE; ++i )
		{
			cFILE.getflag( "strSimpleWearM", 1+i, SLOT_WEAR_SIMPLE, strSimpleWearM[i] );
			cFILE.getflag( "strSimpleWearF", 1+i, SLOT_WEAR_SIMPLE, strSimpleWearF[i] );
		}
	
		/*contribution/activity bonus, Juver, 2017/10/14 */
		cFILE.getflag( "bContributionPointBonusTimer",	1, 1, bContributionPointBonusTimer );
		cFILE.getflag( "fContributionPointBonusTime",	1, 1, fContributionPointBonusTime );
		cFILE.getflag( "wContributionPointBonusAdd",	1, 1, wContributionPointBonusAdd );
		cFILE.getflag( "bActivityPointBonusTimer",	1, 1, bActivityPointBonusTimer );
		cFILE.getflag( "fActivityPointBonusTime",	1, 1, fActivityPointBonusTime );
		cFILE.getflag( "wActivityPointBonusAdd",	1, 1, wActivityPointBonusAdd );

		/*activity system, Juver, 2017/10/30 */
		cFILE.getflag( "strCOMPLETE_ACTIVITY_EFFECT",	1, 1, strCOMPLETE_ACTIVITY_EFFECT );

		/*school wars, Juver, 2018/01/19 */
		cFILE.getflag( "SCHOOLWARS_FILE",	1, 1, strSchoolWarsFile );

		/*pvp capture the flag, Juver, 2018/01/23 */
		cFILE.getflag( "CAPTURE_THE_FLAG_FILE",	1, 1, strCaptureTheFlagFile );
		for ( i=0; i<CAPTURE_THE_FLAG_TEAM_SIZE; ++i )
		{
			CString str_capturetheflag_effect;
			str_capturetheflag_effect.Format ( "strCTF_EFF%02d", i );
			cFILE.getflag( str_capturetheflag_effect.GetString(), 1, 1, strCaptureTheFlagEffect[i] );

			CString str_capturetheflag_effect_direction;
			str_capturetheflag_effect_direction.Format ( "strCTF_DIR_EFF%02d", i );
			cFILE.getflag( str_capturetheflag_effect_direction.GetString(), 1, 1, strCaptureTheFlagDirectionEffect[i] );
		}
...
BOOL GLCONST_CHARCLASS::LOADFILE ( const char* szFileName )
{
	...
	if ( dwHAIRNUM_SELECT >= MAX_HAIR )		
		dwHAIRNUM_SELECT = MAX_HAIR;
