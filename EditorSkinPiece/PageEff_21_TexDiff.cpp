// PageEff_21_TexDiff.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CPageEff_21_TexDiff, CPropertyPage)

CPageEff_21_TexDiff::CPageEff_21_TexDiff(LOGFONT logfont)
	: CPropertyPage(CPageEff_21_TexDiff::IDD)
	, m_pFont ( NULL )
	, m_pPiece( NULL )
	, m_pEff( NULL )
	, m_bADD( FALSE )
...
CPageEff_21_TexDiff::~CPageEff_21_TexDiff()
{
	SAFE_DELETE ( m_pFont );
}

void CPageEff_21_TexDiff::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPageEff_21_TexDiff, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_BUTTON_CTD_TEXTURE, OnBnClickedButtonCtdTexture)
END_MESSAGE_MAP()


// CPageEff_21_TexDiff message handlers
BOOL CPageEff_21_TexDiff::OnInitDialog()
{
	...
	SetWin_Combo_Init( this, IDC_COMBO_CTD_INTENSITY, GLOBAL_EFFTOOL::strEFFLEVEL_MODULATE, TOOL_EFFLEVEL_MODULATE_SIZE );
...
void CPageEff_21_TexDiff::DataSet( DxSkinPiece* pPiece, DxEffCharTexDiff* pEff, BOOL bADD )
{
	m_pPiece = pPiece;
	m_pEff = pEff;
	m_bADD = bADD;
...
void CPageEff_21_TexDiff::ResetTool()
{
	SetWin_Check( this, IDC_CHECK_CTD_USE_UPDOWN, 0 );
	SetWin_Num_float( this, IDC_EDIT_CTD_SPEED, 1.0f );
	SetWin_Combo_Sel( this, IDC_COMBO_CTD_INTENSITY, 0 );
...
void CPageEff_21_TexDiff::DataShow()
{
	if ( !m_pPiece )	return;
	if ( !m_pEff )		return;

	EFFCHAR_PROPERTY_TEXDIFF* pProp = (EFFCHAR_PROPERTY_TEXDIFF*)m_pEff->GetProperty();
	SetWin_Check( this, IDC_CHECK_CTD_USE_UPDOWN, pProp->m_dwFlag&NO_UPDOWN);
	SetWin_Num_float( this, IDC_EDIT_CTD_SPEED, pProp->m_fSpeed );
	SetWin_Combo_Sel( this, IDC_COMBO_CTD_INTENSITY, GLOBAL_EFFTOOL::strEFFLEVEL_MODULATE[pProp->m_dwIntensity].c_str() );
	SetWin_Text( this, IDC_EDIT_CTD_TEXTURE, pProp->m_szTexture );
}

BOOL CPageEff_21_TexDiff::DataSave()
{
	if ( !m_pPiece )	return FALSE;
	if ( !m_pEff )		return FALSE;

	EFFCHAR_PROPERTY_TEXDIFF sProperty;
	SetCheck_Flags ( GetWin_Check ( this, IDC_CHECK_CTD_USE_UPDOWN ), sProperty.m_dwFlag, NO_UPDOWN );
	sProperty.m_fSpeed = GetWin_Num_float( this, IDC_EDIT_CTD_SPEED );
	sProperty.m_dwIntensity = GetWin_Combo_Sel( this, IDC_COMBO_CTD_INTENSITY );
	
	std::string strTexture = GetWin_Text( this, IDC_EDIT_CTD_TEXTURE );
...
void CPageEff_21_TexDiff::MaterialShow()
{
...
void CPageEff_21_TexDiff::OnBnClickedButtonApply()
{
	DataSave();
}

void CPageEff_21_TexDiff::OnBnClickedButtonSave()
{
	if ( DataSave() )
	{
		m_pPiece = NULL;
...
void CPageEff_21_TexDiff::OnBnClickedButtonClose()
{
	if ( m_pSheetTab )
	{
		m_pPiece = NULL;
...
void CPageEff_21_TexDiff::OnBnClickedButtonMaterialTexture()
{
...
void CPageEff_21_TexDiff::OnBnClickedButtonMaterialOk()
{
	if ( !m_pPiece )	return;
	if ( !m_pEff )		return;
...
void CPageEff_21_TexDiff::OnNMDblclkListMaterial(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CPageEff_21_TexDiff::OnBnClickedButtonCtdTexture()
{
	CString szFilter = "Texture Image (*.bmp,*.dds,*.tga,*.jpg)|*.bmp;*.dds;*.tga;*.jpg|";
	CFileDialog dlg(TRUE,".",NULL,OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
	dlg.m_ofn.lpstrInitialDir = TextureManager::GetTexturePath(); 
	if ( dlg.DoModal() == IDOK )	
		SetWin_Text ( this, IDC_EDIT_CTD_TEXTURE, dlg.GetFileName().GetString() );
}
