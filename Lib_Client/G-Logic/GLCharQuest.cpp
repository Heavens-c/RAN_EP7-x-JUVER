...
void GLChar::DoQuestGenerateReward ( GLQUEST* pQUEST )
{
	...
	if ( pQUEST->m_llGiftEXP>0 )
	{

		LONGLONG llMemExp = pQUEST->m_llGiftEXP;
#if defined(VN_PARAM) //vietnamtest%%%
		...
		{
			m_lVNGainSysExp += (pQUEST->m_llGiftEXP - llMemExp);
		...
		llMemExp = LONGLONG ( GLCONST_CHAR::fQuestExpRate * llMemExp );

		m_sExperience.lnNow += llMemExp;
	...
	if ( pQUEST->m_dwGiftMONEY>0 )
	{
		...
		/*quest rate, Juver, 2017/09/28 */
		lnAmount = LONGLONG ( GLCONST_CHAR::fQuestMoneyRate * lnAmount );
		...
		NetMsgMoney.lnPickUp = (LONGLONG)lnAmount;
	...
	DoActivityCompleteQuest( pQUEST->m_sNID.dwID );
}
