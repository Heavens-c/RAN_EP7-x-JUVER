	...
	NET_LOGIN_FEEDBACK_DATA2* pMsg )
{
	...
	SQLINTEGER nUserPoint	  = 0, cbUserPoint	   = SQL_NTS;
	SQLINTEGER nBattlePoint	  = 0, cbBattlePoint	   = SQL_NTS;
	SQLCHAR vUserEmail[USR_EMAIL+RSA_ADD+1]	  = {0}; SQLINTEGER cbUserEmail	   = SQL_NTS; //itemmall userpanel
 ...
 //   std::strstream strTemp;
	...
	/*TCHAR szTemp[256] = {0}; 
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, "
							"ChaTestRemain, ChatBlockDate, PlayTime, OfflineTime, LastLoginDate, UserAge FROM UserInfo WHERE UserID='%s'",
							szUserID );*/
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, "
							"ChaTestRemain, ChatBlockDate, PlayTime, OfflineTime, LastLoginDate, UserAge FROM UserInfo WHERE UserID='%s'",
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, "
		"ChaTestRemain, ChatBlockDate, LastLoginDate , UserEmail , UserPoint , UserPoint2 , PlayTime FROM UserInfo WHERE UserID='%s'",
		szUserID );
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, "
		"ChaTestRemain, ChatBlockDate , UserEmail , UserPoint , UserPoint2 , PlayTime FROM UserInfo WHERE UserID='%s'",
		szUserID );
#endif
	...
	else
	{
		...
		while(true)
		{
			...
			{	
				...
				::SQLGetData(pConn->hStmt, 11, SQL_C_CHAR, vUserEmail  ,	 USR_EMAIL+RSA_ADD+1 , &cbUserEmail);
				::SQLGetData(pConn->hStmt, 12, SQL_C_LONG,           &nUserPoint, 0, &cbUserPoint);
				::SQLGetData(pConn->hStmt, 13, SQL_C_LONG,           &nBattlePoint, 0, &cbBattlePoint); //userpanel
				...
				::StringCchCopy(pMsg->szUserEmail, USR_EMAIL+1, (const char*) vUserEmail); //userpanel
			}
			else
			{
				break;
		...
		while(true)
		{
			...
			{	
				...
				::SQLGetData(pConn->hStmt, 8, SQL_C_CHAR, vUserEmail  ,	 USR_EMAIL+RSA_ADD+1 , &cbUserEmail);
				::SQLGetData(pConn->hStmt, 9, SQL_C_LONG,           &nUserPoint, 0, &cbUserPoint);
				::SQLGetData(pConn->hStmt, 10, SQL_C_LONG,           &nBattlePoint, 0, &cbBattlePoint);
				::SQLGetData(pConn->hStmt, 11, SQL_C_LONG,           &nChinaGameTime, 0, &cbChinaGameTime);
#else
				
				::SQLGetData(pConn->hStmt, 7, SQL_C_CHAR, vUserEmail  ,	 USR_EMAIL+RSA_ADD+1 , &cbUserEmail);
				...
				::SQLGetData(pConn->hStmt, 10, SQL_C_LONG,           &nChinaGameTime, 0, &cbChinaGameTime); //itemmall userpanel
#endif
				...
				::StringCchCopy(pMsg->szUserEmail, USR_ID_LENGTH+1, (const char*) vUserEmail);
#else
				...
				::StringCchCopy(pMsg->szUserEmail, USR_ID_LENGTH+1, (const char*) vUserEmail); //itemmall userpanel
#endif
	...
	GSP_NET_LOGIN_FEEDBACK_DATA2* pMsg )
{
 ...
 //   std::strstream strTemp;
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, ChaTestRemain, ChatBlockDate "
